...
public class Char2IntMapPutAllArrayTester extends AbstractChar2IntMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Char2IntMap.Entry> iterator = getMap().char2IntEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (int[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (Integer[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2IntMap.Entry> entries) {
		Char2IntMap map = new Char2IntLinkedOpenHashMap();
		for (Char2IntMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toIntArray(new int[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2IntMap.Entry> entries) {
		Map<Character, Integer> map = new Char2IntLinkedOpenHashMap();
		for (Char2IntMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray(new Integer[map.size()]));
	}
}