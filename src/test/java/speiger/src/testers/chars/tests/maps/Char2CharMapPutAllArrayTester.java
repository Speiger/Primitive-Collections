...
public class Char2CharMapPutAllArrayTester extends AbstractChar2CharMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Char2CharMap.Entry> iterator = getMap().char2CharEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (char[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (Character[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2CharMap.Entry> entries) {
		Char2CharMap map = new Char2CharLinkedOpenHashMap();
		for (Char2CharMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toCharArray(new char[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2CharMap.Entry> entries) {
		Map<Character, Character> map = new Char2CharLinkedOpenHashMap();
		for (Char2CharMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray(new Character[map.size()]));
	}
}