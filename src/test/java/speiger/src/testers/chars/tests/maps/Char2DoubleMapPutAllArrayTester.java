...
public class Char2DoubleMapPutAllArrayTester extends AbstractChar2DoubleMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (double[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (Double[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2DoubleMap.Entry> entries) {
		Char2DoubleMap map = new Char2DoubleLinkedOpenHashMap();
		for (Char2DoubleMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2DoubleMap.Entry> entries) {
		Map<Character, Double> map = new Char2DoubleLinkedOpenHashMap();
		for (Char2DoubleMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}