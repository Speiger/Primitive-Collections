...
public class Char2ShortMapAddToTester extends AbstractChar2ShortMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2ShortMap.Entry> iterator = getMap().char2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Char2ShortMap.Entry entry) {
		return getMap().addTo(entry.getCharKey(), entry.getShortValue());
	}
}