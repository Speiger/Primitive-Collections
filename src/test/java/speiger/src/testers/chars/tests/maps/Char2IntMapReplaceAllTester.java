...
@Ignore
public class Char2IntMapReplaceAllTester extends AbstractChar2IntMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Char2IntMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Char2IntMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getCharKey());
			expectedEntries.add(entry(entry.getCharKey(), values().asList().getInt(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Char2IntMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
