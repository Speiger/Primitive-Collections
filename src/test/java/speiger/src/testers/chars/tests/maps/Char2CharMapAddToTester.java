...
@Ignore
public class Char2CharMapAddToTester extends AbstractChar2CharMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2CharMap.Entry> iterator = getMap().char2CharEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private char addTo(Char2CharMap.Entry entry) {
		return getMap().addTo(entry.getCharKey(), entry.getCharValue());
	}
}