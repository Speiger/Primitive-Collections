...
public class Char2ObjectMapToStringTester<V> extends AbstractChar2ObjectMapTester<V>
{
	...
	public void testToString_formatting() {
		assertEquals("map.toString() incorrect", expectedToString(getMap().char2ObjectEntrySet()), getMap().toString());
	}

	private String expectedToString(Set<Char2ObjectMap.Entry<V>> entries) {
		Char2ObjectMap<V> reference = new Char2ObjectLinkedOpenHashMap<>();
		for (Char2ObjectMap.Entry<V> entry : entries) {
			reference.put(entry.getCharKey(), entry.getValue());
		}
