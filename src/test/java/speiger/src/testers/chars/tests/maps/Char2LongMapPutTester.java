...
@Ignore
public class Char2LongMapPutTester extends AbstractChar2LongMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2LongMap.Entry> iterator = getMap().char2LongEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private long put(Char2LongMap.Entry entry) {
		return getMap().put(entry.getCharKey(), entry.getLongValue());
	}
}