...
public class Char2BooleanMapPutAllArrayTester extends AbstractChar2BooleanMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Char2BooleanMap.Entry> iterator = getMap().char2BooleanEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (boolean[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (Boolean[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2BooleanMap.Entry> entries) {
		Char2BooleanMap map = new Char2BooleanLinkedOpenHashMap();
		for (Char2BooleanMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toBooleanArray(new boolean[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2BooleanMap.Entry> entries) {
		Map<Character, Boolean> map = new Char2BooleanLinkedOpenHashMap();
		for (Char2BooleanMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray(new Boolean[map.size()]));
	}
}