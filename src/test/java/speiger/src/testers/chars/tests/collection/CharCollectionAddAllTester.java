...
public class CharCollectionAddAllTester extends AbstractCharCollectionTester
{
	...
	public void testAddAll_supportedSomePresent() {
		assertTrue("addAll(somePresent) should return true", collection.addAll(MinimalCharCollection.of(e3(), e0())));
	...
	public void testAddAll_unsupportedSomePresent() {
		try {
			collection.addAll(MinimalCharCollection.of(e3(), e0()));
	...
	public void testAddAllConcurrentWithIteration() {
		try {
			...
			assertTrue(collection.addAll(MinimalCharCollection.of(e3(), e0())));
			iterator.nextChar();
	...
	public void testAddAll_unsupportedAllPresent() {
		try {
			assertFalse("addAll(allPresent) should return false or throw", collection.addAll(MinimalCharCollection.of(e0())));
		} catch (UnsupportedOperationException tolerated) {
		}
		expectUnchanged();
	}
