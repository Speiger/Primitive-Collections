...
public class Char2BooleanMapPutTester extends AbstractChar2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2BooleanMap.Entry> iterator = getMap().char2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Char2BooleanMap.Entry entry) {
		return getMap().put(entry.getCharKey(), entry.getBooleanValue());
	}
}