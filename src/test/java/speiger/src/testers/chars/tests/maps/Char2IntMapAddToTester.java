...
@Ignore
public class Char2IntMapAddToTester extends AbstractChar2IntMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2IntMap.Entry> iterator = getMap().char2IntEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private int addTo(Char2IntMap.Entry entry) {
		return getMap().addTo(entry.getCharKey(), entry.getIntValue());
	}
}