...
public class Char2ByteMapReplaceAllTester extends AbstractChar2ByteMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Char2ByteMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Char2ByteMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getCharKey());
			expectedEntries.add(entry(entry.getCharKey(), values().asList().getByte(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Char2ByteMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
