...
@Ignore
public class Char2ByteMapAddToTester extends AbstractChar2ByteMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Char2ByteMap.Entry> iterator = getMap().char2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Char2ByteMap.Entry entry) {
		return getMap().addTo(entry.getCharKey(), entry.getByteValue());
	}
}