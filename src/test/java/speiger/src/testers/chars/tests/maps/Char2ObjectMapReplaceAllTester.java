...
public class Char2ObjectMapReplaceAllTester<V> extends AbstractChar2ObjectMapTester<V>
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Char2ObjectMap.Entry<V>> expectedEntries = new ObjectArrayList<>();
		for (Char2ObjectMap.Entry<V> entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getCharKey());
			expectedEntries.add(entry(entry.getCharKey(), values().asList().get(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Char2ObjectMap.Entry<V>> orderedEntries = getOrderedElements();
		int index = 0;
