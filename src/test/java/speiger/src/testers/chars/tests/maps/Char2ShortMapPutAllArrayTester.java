...
public class Char2ShortMapPutAllArrayTester extends AbstractChar2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Char2ShortMap.Entry> iterator = getMap().char2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (short[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (Short[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2ShortMap.Entry> entries) {
		Char2ShortMap map = new Char2ShortLinkedOpenHashMap();
		for (Char2ShortMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2ShortMap.Entry> entries) {
		Map<Character, Short> map = new Char2ShortLinkedOpenHashMap();
		for (Char2ShortMap.Entry entry : entries) {
			map.put(entry.getCharKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}