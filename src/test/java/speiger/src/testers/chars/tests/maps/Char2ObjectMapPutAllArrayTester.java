...
public class Char2ObjectMapPutAllArrayTester<V> extends AbstractChar2ObjectMapTester<V>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Char2ObjectMap.Entry<V>> iterator = getMap().char2ObjectEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((char[])null, (V[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Character[])null, (V[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Char2ObjectMap.Entry<V>> entries) {
		Char2ObjectMap<V> map = new Char2ObjectLinkedOpenHashMap<>();
		for (Char2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getCharKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toCharArray(new char[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Char2ObjectMap.Entry<V>> entries) {
		Map<Character, V> map = new Char2ObjectLinkedOpenHashMap<>();
		for (Char2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getCharKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray(new Character[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
}