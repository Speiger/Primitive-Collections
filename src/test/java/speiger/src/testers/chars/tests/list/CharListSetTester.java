...
public class CharListSetTester extends AbstractCharListTester {
	...
	private void doTestSet(char newValue) {
		int index = aValidIndex();
		char initialValue = getList().getChar(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getChar(index));
