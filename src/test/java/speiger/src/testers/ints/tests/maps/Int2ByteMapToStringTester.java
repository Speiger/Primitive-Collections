...
public class Int2ByteMapToStringTester extends AbstractInt2ByteMapTester
{
	...
	public void testToString_formatting() {
		assertEquals("map.toString() incorrect", expectedToString(getMap().int2ByteEntrySet()), getMap().toString());
	}

	private String expectedToString(Set<Int2ByteMap.Entry> entries) {
		Int2ByteMap reference = new Int2ByteLinkedOpenHashMap();
		for (Int2ByteMap.Entry entry : entries) {
			reference.put(entry.getIntKey(), entry.getByteValue());
		}
