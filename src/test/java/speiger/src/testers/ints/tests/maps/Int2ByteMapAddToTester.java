...
public class Int2ByteMapAddToTester extends AbstractInt2ByteMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Int2ByteMap.Entry> iterator = getMap().int2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Int2ByteMap.Entry entry) {
		return getMap().addTo(entry.getIntKey(), entry.getByteValue());
	}
}