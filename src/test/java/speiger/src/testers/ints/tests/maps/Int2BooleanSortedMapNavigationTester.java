...
@Ignore
public class Int2BooleanSortedMapNavigationTester extends AbstractInt2BooleanMapTester
{
	private Int2BooleanSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Int2BooleanSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getIntKey(), entries.get(j).getIntKey()).int2BooleanEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.int2BooleanEntrySet().iterator();
