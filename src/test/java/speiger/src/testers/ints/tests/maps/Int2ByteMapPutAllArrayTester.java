...
@Ignore
public class Int2ByteMapPutAllArrayTester extends AbstractInt2ByteMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2ByteMap.Entry> iterator = getMap().int2ByteEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (byte[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Byte[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2ByteMap.Entry> entries) {
		Int2ByteMap map = new Int2ByteLinkedOpenHashMap();
		for (Int2ByteMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2ByteMap.Entry> entries) {
		Map<Integer, Byte> map = new Int2ByteLinkedOpenHashMap();
		for (Int2ByteMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}