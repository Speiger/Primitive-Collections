...
@Ignore
public class Int2FloatMapPutAllArrayTester extends AbstractInt2FloatMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2FloatMap.Entry> iterator = getMap().int2FloatEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (float[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Float[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2FloatMap.Entry> entries) {
		Int2FloatMap map = new Int2FloatLinkedOpenHashMap();
		for (Int2FloatMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2FloatMap.Entry> entries) {
		Map<Integer, Float> map = new Int2FloatLinkedOpenHashMap();
		for (Int2FloatMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}