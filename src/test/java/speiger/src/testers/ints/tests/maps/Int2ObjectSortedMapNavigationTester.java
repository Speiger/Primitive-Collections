...
public class Int2ObjectSortedMapNavigationTester<V> extends AbstractInt2ObjectMapTester<V>
{
	private Int2ObjectSortedMap<V> sortedMap;
	private Entry<V> a;
	private Entry<V> c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Int2ObjectSortedMap<V>) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getIntKey(), entries.get(j).getIntKey()).int2ObjectEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry<V>> entryItr = sortedMap.int2ObjectEntrySet().iterator();
