...
@Ignore
public class Int2ObjectMapPutAllArrayTester<V> extends AbstractInt2ObjectMapTester<V>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2ObjectMap.Entry<V>> iterator = getMap().int2ObjectEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (V[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2ObjectMap.Entry<V>> entries) {
		Int2ObjectMap<V> map = new Int2ObjectLinkedOpenHashMap<>();
		for (Int2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getIntKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2ObjectMap.Entry<V>> entries) {
		Map<Integer, V> map = new Int2ObjectLinkedOpenHashMap<>();
		for (Int2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getIntKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
}