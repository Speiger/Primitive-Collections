...
@Ignore
public class Int2IntMapPutAllArrayTester extends AbstractInt2IntMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2IntMap.Entry> iterator = getMap().int2IntEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (int[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Integer[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2IntMap.Entry> entries) {
		Int2IntMap map = new Int2IntLinkedOpenHashMap();
		for (Int2IntMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toIntArray(new int[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2IntMap.Entry> entries) {
		Map<Integer, Integer> map = new Int2IntLinkedOpenHashMap();
		for (Int2IntMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Integer[map.size()]));
	}
}