...
public class Int2ShortMapPutAllArrayTester extends AbstractInt2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2ShortMap.Entry> iterator = getMap().int2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (short[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Short[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2ShortMap.Entry> entries) {
		Int2ShortMap map = new Int2ShortLinkedOpenHashMap();
		for (Int2ShortMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2ShortMap.Entry> entries) {
		Map<Integer, Short> map = new Int2ShortLinkedOpenHashMap();
		for (Int2ShortMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}