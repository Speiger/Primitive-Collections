...
@Ignore
public class Int2ObjectMapReplaceAllTester<V> extends AbstractInt2ObjectMapTester<V>
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Int2ObjectMap.Entry<V>> expectedEntries = new ObjectArrayList<>();
		for (Int2ObjectMap.Entry<V> entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getIntKey());
			expectedEntries.add(entry(entry.getIntKey(), values().asList().get(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Int2ObjectMap.Entry<V>> orderedEntries = getOrderedElements();
		int index = 0;
