...
public class IntCollectionRetainAllTester extends AbstractIntCollectionTester {
	...
	private class Target {
		...
		private final IntCollection inverse = IntArrayList.wrap(createSamplesArray());
		private final String description;

		private Target(IntCollection toRetain, String description) {
	...
	public void setUp() throws Exception {
		...
		disjoint = new Target(IntArrayList.wrap(e3(), e4()), "disjoint");
		superset = new Target(MinimalIntCollection.of(e0(), e1(), e2(), e3(), e4()), "superset");
		nonEmptyProperSubset = new Target(MinimalIntCollection.of(e1()), "subset");
		sameElements = new Target(IntArrayList.wrap(createSamplesArray()), "sameElements");
		containsDuplicates = new Target(MinimalIntCollection.of(e0(), e0(), e3(), e3()), "containsDuplicates");
		partialOverlap = new Target(MinimalIntCollection.of(e2(), e3()), "partialOverlap");
	}
	...
	private void expectReturnsTrueExtra(Target target) {
		...
		IntHelpers.assertEqualIgnoringOrder(target.inverse, list);
	}
