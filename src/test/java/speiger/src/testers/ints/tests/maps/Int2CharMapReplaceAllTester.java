...
public class Int2CharMapReplaceAllTester extends AbstractInt2CharMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Int2CharMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Int2CharMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getIntKey());
			expectedEntries.add(entry(entry.getIntKey(), values().asList().getChar(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Int2CharMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
