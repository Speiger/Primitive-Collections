...
@Ignore
public class Int2BooleanMapPutTester extends AbstractInt2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Int2BooleanMap.Entry> iterator = getMap().int2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Int2BooleanMap.Entry entry) {
		return getMap().put(entry.getIntKey(), entry.getBooleanValue());
	}
}