...
public class Int2ObjectMapPutTester<V> extends AbstractInt2ObjectMapTester<V>
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Int2ObjectMap.Entry<V>> iterator = getMap().int2ObjectEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private V put(Int2ObjectMap.Entry<V> entry) {
		return getMap().put(entry.getIntKey(), entry.getValue());
	}
}