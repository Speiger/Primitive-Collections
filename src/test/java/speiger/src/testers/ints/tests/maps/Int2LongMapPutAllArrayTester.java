...
@Ignore
public class Int2LongMapPutAllArrayTester extends AbstractInt2LongMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Int2LongMap.Entry> iterator = getMap().int2LongEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (long[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Long[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Int2LongMap.Entry> entries) {
		Int2LongMap map = new Int2LongLinkedOpenHashMap();
		for (Int2LongMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toLongArray(new long[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2LongMap.Entry> entries) {
		Map<Integer, Long> map = new Int2LongLinkedOpenHashMap();
		for (Int2LongMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Long[map.size()]));
	}
}