...
public class IntListSetTester extends AbstractIntListTester {
	...
	private void doTestSet(int newValue) {
		int index = aValidIndex();
		int initialValue = getList().getInt(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getInt(index));
