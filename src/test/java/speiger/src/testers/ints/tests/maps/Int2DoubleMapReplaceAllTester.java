...
public class Int2DoubleMapReplaceAllTester extends AbstractInt2DoubleMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Int2DoubleMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Int2DoubleMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getIntKey());
			expectedEntries.add(entry(entry.getIntKey(), values().asList().getDouble(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Int2DoubleMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
