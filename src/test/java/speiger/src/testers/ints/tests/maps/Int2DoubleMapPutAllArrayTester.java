...
public class Int2DoubleMapPutAllArrayTester extends AbstractInt2DoubleMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((int[])null, (double[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Integer[])null, (Double[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	...
	private void putAll(ObjectIterable<Int2DoubleMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toIntArray(new int[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Int2DoubleMap.Entry> entries) {
		Map<Integer, Double> map = new Int2DoubleLinkedOpenHashMap();
		for (Int2DoubleMap.Entry entry : entries) {
			map.put(entry.getIntKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Integer[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}