...
public class Int2FloatMapAddToTester extends AbstractInt2FloatMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Int2FloatMap.Entry> iterator = getMap().int2FloatEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private float addTo(Int2FloatMap.Entry entry) {
		return getMap().addTo(entry.getIntKey(), entry.getFloatValue());
	}
}