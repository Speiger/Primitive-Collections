...
public class Int2CharSortedMapNavigationTester extends AbstractInt2CharMapTester
{
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Int2CharSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getIntKey(), entries.get(j).getIntKey()).int2CharEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.int2CharEntrySet().iterator();
