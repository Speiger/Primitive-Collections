...
public class Int2FloatMapReplaceAllTester extends AbstractInt2FloatMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Int2FloatMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Int2FloatMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getIntKey());
			expectedEntries.add(entry(entry.getIntKey(), values().asList().getFloat(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Int2FloatMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
