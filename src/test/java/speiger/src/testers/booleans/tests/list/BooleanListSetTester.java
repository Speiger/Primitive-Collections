...
public class BooleanListSetTester extends AbstractBooleanListTester {
	...
	private void doTestSet(boolean newValue) {
		int index = aValidIndex();
		boolean initialValue = getList().getBoolean(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getBoolean(index));
