...
@Ignore
public class Byte2BooleanMapPutTester extends AbstractByte2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2BooleanMap.Entry> iterator = getMap().byte2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Byte2BooleanMap.Entry entry) {
		return getMap().put(entry.getByteKey(), entry.getBooleanValue());
	}
}