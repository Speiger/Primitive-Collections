...
public class Byte2DoubleMapReplaceAllTester extends AbstractByte2DoubleMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Byte2DoubleMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Byte2DoubleMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getByteKey());
			expectedEntries.add(entry(entry.getByteKey(), values().asList().getDouble(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Byte2DoubleMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
