...
public class Byte2ByteMapPutAllArrayTester extends AbstractByte2ByteMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (byte[])null);
	...
	private void putAll(ObjectIterable<Byte2ByteMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2ByteMap.Entry> entries) {
		Map<Byte, Byte> map = new Byte2ByteLinkedOpenHashMap();
		for (Byte2ByteMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}