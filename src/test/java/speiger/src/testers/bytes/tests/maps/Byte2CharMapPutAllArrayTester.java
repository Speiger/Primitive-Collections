...
@Ignore
public class Byte2CharMapPutAllArrayTester extends AbstractByte2CharMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Byte2CharMap.Entry> iterator = getMap().byte2CharEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (char[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Byte[])null, (Character[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Byte2CharMap.Entry> entries) {
		Byte2CharMap map = new Byte2CharLinkedOpenHashMap();
		for (Byte2CharMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toCharArray(new char[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2CharMap.Entry> entries) {
		Map<Byte, Character> map = new Byte2CharLinkedOpenHashMap();
		for (Byte2CharMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Character[map.size()]));
	}
}