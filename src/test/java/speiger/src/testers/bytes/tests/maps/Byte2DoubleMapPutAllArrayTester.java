...
@Ignore
public class Byte2DoubleMapPutAllArrayTester extends AbstractByte2DoubleMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Byte2DoubleMap.Entry> entries) {
		Byte2DoubleMap map = new Byte2DoubleLinkedOpenHashMap();
		for (Byte2DoubleMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2DoubleMap.Entry> entries) {
		Map<Byte, Double> map = new Byte2DoubleLinkedOpenHashMap();
		for (Byte2DoubleMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}