...
public class Byte2FloatMapAddToTester extends AbstractByte2FloatMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2FloatMap.Entry> iterator = getMap().byte2FloatEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private float addTo(Byte2FloatMap.Entry entry) {
		return getMap().addTo(entry.getByteKey(), entry.getFloatValue());
	}
}