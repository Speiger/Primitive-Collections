...
public class Byte2ObjectMapPutTester<V> extends AbstractByte2ObjectMapTester<V>
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2ObjectMap.Entry<V>> iterator = getMap().byte2ObjectEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private V put(Byte2ObjectMap.Entry<V> entry) {
		return getMap().put(entry.getByteKey(), entry.getValue());
	}
}