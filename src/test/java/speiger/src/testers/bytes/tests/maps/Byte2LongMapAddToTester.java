...
public class Byte2LongMapAddToTester extends AbstractByte2LongMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2LongMap.Entry> iterator = getMap().byte2LongEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private long addTo(Byte2LongMap.Entry entry) {
		return getMap().addTo(entry.getByteKey(), entry.getLongValue());
	}
}