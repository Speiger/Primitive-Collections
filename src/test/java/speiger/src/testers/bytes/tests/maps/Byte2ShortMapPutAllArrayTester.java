...
public class Byte2ShortMapPutAllArrayTester extends AbstractByte2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Byte2ShortMap.Entry> iterator = getMap().byte2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (short[])null);
	...
	private void putAll(ObjectIterable<Byte2ShortMap.Entry> entries) {
		Byte2ShortMap map = new Byte2ShortLinkedOpenHashMap();
		for (Byte2ShortMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2ShortMap.Entry> entries) {
		Map<Byte, Short> map = new Byte2ShortLinkedOpenHashMap();
		for (Byte2ShortMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}