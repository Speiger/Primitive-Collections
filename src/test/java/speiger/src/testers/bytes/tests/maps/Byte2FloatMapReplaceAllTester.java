...
@Ignore
public class Byte2FloatMapReplaceAllTester extends AbstractByte2FloatMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Byte2FloatMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Byte2FloatMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getByteKey());
			expectedEntries.add(entry(entry.getByteKey(), values().asList().getFloat(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Byte2FloatMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
