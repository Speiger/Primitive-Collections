...
public class Byte2LongSortedMapNavigationTester extends AbstractByte2LongMapTester
{
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Byte2LongSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getByteKey(), entries.get(j).getByteKey()).byte2LongEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.byte2LongEntrySet().iterator();
