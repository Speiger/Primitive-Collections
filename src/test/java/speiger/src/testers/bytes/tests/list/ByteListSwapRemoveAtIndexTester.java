...
public class ByteListSwapRemoveAtIndexTester extends AbstractByteListTester
{
	...
	private void runRemoveTest(int index) {
		assertEquals(String.format("remove(%d) should return the element at index %d", index, index), getList().getByte(index), getList().swapRemove(index));
		ByteList expected = ByteHelpers.copyToList(createSamplesArray());
		expected.set(index, expected.getByte(expected.size()-1));
		expected.removeByte(expected.size()-1);
