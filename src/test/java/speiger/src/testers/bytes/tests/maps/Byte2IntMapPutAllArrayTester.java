...
public class Byte2IntMapPutAllArrayTester extends AbstractByte2IntMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (int[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Byte[])null, (Integer[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	...
	private void putAll(ObjectIterable<Byte2IntMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toIntArray(new int[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2IntMap.Entry> entries) {
		Map<Byte, Integer> map = new Byte2IntLinkedOpenHashMap();
		for (Byte2IntMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Integer[map.size()]));
	}
}