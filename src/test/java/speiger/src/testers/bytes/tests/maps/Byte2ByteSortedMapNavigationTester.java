...
public class Byte2ByteSortedMapNavigationTester extends AbstractByte2ByteMapTester
{
	private Byte2ByteSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Byte2ByteSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getByteKey(), entries.get(j).getByteKey()).byte2ByteEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.byte2ByteEntrySet().iterator();
