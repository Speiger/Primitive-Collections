...
@Ignore
public class Byte2ShortMapAddToTester extends AbstractByte2ShortMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2ShortMap.Entry> iterator = getMap().byte2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Byte2ShortMap.Entry entry) {
		return getMap().addTo(entry.getByteKey(), entry.getShortValue());
	}
}