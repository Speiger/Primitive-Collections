...
public class Byte2ByteMapAddToTester extends AbstractByte2ByteMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2ByteMap.Entry> iterator = getMap().byte2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Byte2ByteMap.Entry entry) {
		return getMap().addTo(entry.getByteKey(), entry.getByteValue());
	}
}