...
public class Byte2BooleanMapReplaceAllTester extends AbstractByte2BooleanMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Byte2BooleanMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Byte2BooleanMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getByteKey());
			expectedEntries.add(entry(entry.getByteKey(), values().asList().getBoolean(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Byte2BooleanMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
