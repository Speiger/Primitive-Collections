...
public class Byte2FloatSortedMapNavigationTester extends AbstractByte2FloatMapTester
{
	private Byte2FloatSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Byte2FloatSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getByteKey(), entries.get(j).getByteKey()).byte2FloatEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.byte2FloatEntrySet().iterator();
