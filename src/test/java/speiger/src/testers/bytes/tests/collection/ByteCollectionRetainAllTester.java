...
public class ByteCollectionRetainAllTester extends AbstractByteCollectionTester
{
	...
	private class Target {
		...
		private final ByteCollection inverse = ByteArrayList.wrap(createSamplesArray());
		private final String description;

		private Target(ByteCollection toRetain, String description) {
	...
	public void setUp() throws Exception {
		...
		disjoint = new Target(ByteArrayList.wrap(e3(), e4()), "disjoint");
		superset = new Target(MinimalByteCollection.of(e0(), e1(), e2(), e3(), e4()), "superset");
		nonEmptyProperSubset = new Target(MinimalByteCollection.of(e1()), "subset");
		sameElements = new Target(ByteArrayList.wrap(createSamplesArray()), "sameElements");
		containsDuplicates = new Target(MinimalByteCollection.of(e0(), e0(), e3(), e3()), "containsDuplicates");
		partialOverlap = new Target(MinimalByteCollection.of(e2(), e3()), "partialOverlap");
	}
	...
	private void expectReturnsTrueExtra(Target target) {
		...
		ByteHelpers.assertEqualIgnoringOrder(target.inverse, list);
	}
