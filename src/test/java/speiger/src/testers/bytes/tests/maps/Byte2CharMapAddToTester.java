...
public class Byte2CharMapAddToTester extends AbstractByte2CharMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Byte2CharMap.Entry> iterator = getMap().byte2CharEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private char addTo(Byte2CharMap.Entry entry) {
		return getMap().addTo(entry.getByteKey(), entry.getCharValue());
	}
}