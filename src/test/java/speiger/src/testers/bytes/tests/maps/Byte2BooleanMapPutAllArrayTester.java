...
public class Byte2BooleanMapPutAllArrayTester extends AbstractByte2BooleanMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Byte2BooleanMap.Entry> iterator = getMap().byte2BooleanEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (boolean[])null);
	...
	private void putAll(ObjectIterable<Byte2BooleanMap.Entry> entries) {
		Byte2BooleanMap map = new Byte2BooleanLinkedOpenHashMap();
		for (Byte2BooleanMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toBooleanArray(new boolean[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2BooleanMap.Entry> entries) {
		Map<Byte, Boolean> map = new Byte2BooleanLinkedOpenHashMap();
		for (Byte2BooleanMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Boolean[map.size()]));
	}
}