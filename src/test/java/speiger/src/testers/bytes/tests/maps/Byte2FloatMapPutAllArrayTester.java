...
public class Byte2FloatMapPutAllArrayTester extends AbstractByte2FloatMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((byte[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Byte2FloatMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toByteArray(new byte[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Byte2FloatMap.Entry> entries) {
		Map<Byte, Float> map = new Byte2FloatLinkedOpenHashMap();
		for (Byte2FloatMap.Entry entry : entries) {
			map.put(entry.getByteKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Byte[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}