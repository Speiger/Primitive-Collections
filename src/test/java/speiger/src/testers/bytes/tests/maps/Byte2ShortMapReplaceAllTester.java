...
public class Byte2ShortMapReplaceAllTester extends AbstractByte2ShortMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Byte2ShortMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Byte2ShortMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getByteKey());
			expectedEntries.add(entry(entry.getByteKey(), values().asList().getShort(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Byte2ShortMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
