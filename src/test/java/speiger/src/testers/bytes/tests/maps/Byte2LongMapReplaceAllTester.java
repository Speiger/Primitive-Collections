...
public class Byte2LongMapReplaceAllTester extends AbstractByte2LongMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Byte2LongMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Byte2LongMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getByteKey());
			expectedEntries.add(entry(entry.getByteKey(), values().asList().getLong(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Byte2LongMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
