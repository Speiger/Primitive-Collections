...
public class Float2BooleanSortedMapNavigationTester extends AbstractFloat2BooleanMapTester
{
	private Float2BooleanSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Float2BooleanSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getFloatKey(), entries.get(j).getFloatKey()).float2BooleanEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.float2BooleanEntrySet().iterator();
