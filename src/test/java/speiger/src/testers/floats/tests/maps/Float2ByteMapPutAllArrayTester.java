...
public class Float2ByteMapPutAllArrayTester extends AbstractFloat2ByteMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Float2ByteMap.Entry> iterator = getMap().float2ByteEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((float[])null, (byte[])null);
	...
	private void putAll(ObjectIterable<Float2ByteMap.Entry> entries) {
		Float2ByteMap map = new Float2ByteLinkedOpenHashMap();
		for (Float2ByteMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toFloatArray(new float[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Float2ByteMap.Entry> entries) {
		Map<Float, Byte> map = new Float2ByteLinkedOpenHashMap();
		for (Float2ByteMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray(new Float[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}