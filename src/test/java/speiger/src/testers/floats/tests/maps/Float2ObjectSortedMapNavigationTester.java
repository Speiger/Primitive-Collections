...
public class Float2ObjectSortedMapNavigationTester<V> extends AbstractFloat2ObjectMapTester<V>
{
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Float2ObjectSortedMap<V>) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getFloatKey(), entries.get(j).getFloatKey()).float2ObjectEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry<V>> entryItr = sortedMap.float2ObjectEntrySet().iterator();
