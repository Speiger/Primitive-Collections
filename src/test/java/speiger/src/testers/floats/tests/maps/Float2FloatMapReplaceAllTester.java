...
@Ignore
public class Float2FloatMapReplaceAllTester extends AbstractFloat2FloatMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Float2FloatMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Float2FloatMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getFloatKey());
			expectedEntries.add(entry(entry.getFloatKey(), values().asList().getFloat(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Float2FloatMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
