...
public class Float2LongMapPutTester extends AbstractFloat2LongMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2LongMap.Entry> iterator = getMap().float2LongEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private long put(Float2LongMap.Entry entry) {
		return getMap().put(entry.getFloatKey(), entry.getLongValue());
	}
}