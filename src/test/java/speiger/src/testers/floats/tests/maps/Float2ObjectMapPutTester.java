...
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2ObjectMap.Entry<V>> iterator = getMap().float2ObjectEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private V put(Float2ObjectMap.Entry<V> entry) {
		return getMap().put(entry.getFloatKey(), entry.getValue());
	}
}