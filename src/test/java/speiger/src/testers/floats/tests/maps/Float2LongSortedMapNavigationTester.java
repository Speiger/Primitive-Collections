...
public class Float2LongSortedMapNavigationTester extends AbstractFloat2LongMapTester
{
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Float2LongSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getFloatKey(), entries.get(j).getFloatKey()).float2LongEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.float2LongEntrySet().iterator();
