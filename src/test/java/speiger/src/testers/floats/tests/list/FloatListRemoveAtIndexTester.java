...
public class FloatListRemoveAtIndexTester extends AbstractFloatListTester {
	...
	public void testRemoveAtIndex_tooLarge() {
		try {
			getList().removeFloat(getNumElements());
	...
	public void testRemoveAtIndexConcurrentWithIteration() {
		try {
			...
			getList().removeFloat(getNumElements() / 2);
			iterator.nextFloat();
	...
	private void runRemoveTest(int index) {
		assertEquals(String.format("remove(%d) should return the element at index %d", index, index), getList().getFloat(index), getList().removeFloat(index));
		FloatList expected = FloatHelpers.copyToList(createSamplesArray());
		expected.removeFloat(index);
