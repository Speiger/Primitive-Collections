...
@Ignore
public class Float2DoubleMapAddToTester extends AbstractFloat2DoubleMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2DoubleMap.Entry> iterator = getMap().float2DoubleEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private double addTo(Float2DoubleMap.Entry entry) {
		return getMap().addTo(entry.getFloatKey(), entry.getDoubleValue());
	}
}