...
public class Float2IntMapReplaceAllTester extends AbstractFloat2IntMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Float2IntMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Float2IntMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getFloatKey());
			expectedEntries.add(entry(entry.getFloatKey(), values().asList().getInt(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Float2IntMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
