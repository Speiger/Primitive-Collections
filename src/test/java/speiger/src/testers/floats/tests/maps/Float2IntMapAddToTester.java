...
public class Float2IntMapAddToTester extends AbstractFloat2IntMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2IntMap.Entry> iterator = getMap().float2IntEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private int addTo(Float2IntMap.Entry entry) {
		return getMap().addTo(entry.getFloatKey(), entry.getIntValue());
	}
}