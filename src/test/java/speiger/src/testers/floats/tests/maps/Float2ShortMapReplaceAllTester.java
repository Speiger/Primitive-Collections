...
@Ignore
public class Float2ShortMapReplaceAllTester extends AbstractFloat2ShortMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Float2ShortMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Float2ShortMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getFloatKey());
			expectedEntries.add(entry(entry.getFloatKey(), values().asList().getShort(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Float2ShortMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
