...
public class Float2ShortMapAddToTester extends AbstractFloat2ShortMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2ShortMap.Entry> iterator = getMap().float2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Float2ShortMap.Entry entry) {
		return getMap().addTo(entry.getFloatKey(), entry.getShortValue());
	}
}