...
@Ignore
public class Float2BooleanMapPutAllArrayTester extends AbstractFloat2BooleanMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Float2BooleanMap.Entry> iterator = getMap().float2BooleanEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((float[])null, (boolean[])null);
	...
	private void putAll(ObjectIterable<Float2BooleanMap.Entry> entries) {
		Float2BooleanMap map = new Float2BooleanLinkedOpenHashMap();
		for (Float2BooleanMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toFloatArray(new float[map.size()]), map.values().toBooleanArray(new boolean[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Float2BooleanMap.Entry> entries) {
		Map<Float, Boolean> map = new Float2BooleanLinkedOpenHashMap();
		for (Float2BooleanMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toArray(new Float[map.size()]), map.values().toArray(new Boolean[map.size()]));
	}
}