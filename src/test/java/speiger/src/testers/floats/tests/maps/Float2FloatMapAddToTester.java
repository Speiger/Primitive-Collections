...
@Ignore
public class Float2FloatMapAddToTester extends AbstractFloat2FloatMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2FloatMap.Entry> iterator = getMap().float2FloatEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private float addTo(Float2FloatMap.Entry entry) {
		return getMap().addTo(entry.getFloatKey(), entry.getFloatValue());
	}
}