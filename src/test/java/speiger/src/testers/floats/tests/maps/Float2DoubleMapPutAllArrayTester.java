...
@Ignore
public class Float2DoubleMapPutAllArrayTester extends AbstractFloat2DoubleMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Float2DoubleMap.Entry> iterator = getMap().float2DoubleEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((float[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Float2DoubleMap.Entry> entries) {
		Float2DoubleMap map = new Float2DoubleLinkedOpenHashMap();
		for (Float2DoubleMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toFloatArray(new float[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Float2DoubleMap.Entry> entries) {
		Map<Float, Double> map = new Float2DoubleLinkedOpenHashMap();
		for (Float2DoubleMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Float[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}