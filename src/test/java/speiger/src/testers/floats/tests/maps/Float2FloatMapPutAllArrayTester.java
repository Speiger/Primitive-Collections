...
public class Float2FloatMapPutAllArrayTester extends AbstractFloat2FloatMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Float2FloatMap.Entry> iterator = getMap().float2FloatEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((float[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Float2FloatMap.Entry> entries) {
		Float2FloatMap map = new Float2FloatLinkedOpenHashMap();
		for (Float2FloatMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toFloatArray(new float[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Float2FloatMap.Entry> entries) {
		Map<Float, Float> map = new Float2FloatLinkedOpenHashMap();
		for (Float2FloatMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Float[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}