...
public class Float2ByteMapAddToTester extends AbstractFloat2ByteMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2ByteMap.Entry> iterator = getMap().float2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Float2ByteMap.Entry entry) {
		return getMap().addTo(entry.getFloatKey(), entry.getByteValue());
	}
}