...
public class FloatListSetTester extends AbstractFloatListTester
{
	...
	private void doTestSet(float newValue) {
		int index = aValidIndex();
		float initialValue = getList().getFloat(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getFloat(index));
