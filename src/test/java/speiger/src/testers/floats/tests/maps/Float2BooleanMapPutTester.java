...
public class Float2BooleanMapPutTester extends AbstractFloat2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2BooleanMap.Entry> iterator = getMap().float2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Float2BooleanMap.Entry entry) {
		return getMap().put(entry.getFloatKey(), entry.getBooleanValue());
	}
}