...
@Ignore
public class Float2CharMapPutTester extends AbstractFloat2CharMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Float2CharMap.Entry> iterator = getMap().float2CharEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private char put(Float2CharMap.Entry entry) {
		return getMap().put(entry.getFloatKey(), entry.getCharValue());
	}
}