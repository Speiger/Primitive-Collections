...
public class Float2BooleanMapReplaceAllTester extends AbstractFloat2BooleanMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Float2BooleanMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Float2BooleanMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getFloatKey());
			expectedEntries.add(entry(entry.getFloatKey(), values().asList().getBoolean(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Float2BooleanMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
