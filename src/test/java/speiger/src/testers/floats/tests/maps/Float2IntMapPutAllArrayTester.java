...
@Ignore
public class Float2IntMapPutAllArrayTester extends AbstractFloat2IntMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((float[])null, (int[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Float[])null, (Integer[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	...
	private void putAll(ObjectIterable<Float2IntMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toFloatArray(new float[map.size()]), map.values().toIntArray(new int[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Float2IntMap.Entry> entries) {
		Map<Float, Integer> map = new Float2IntLinkedOpenHashMap();
		for (Float2IntMap.Entry entry : entries) {
			map.put(entry.getFloatKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toArray(new Float[map.size()]), map.values().toArray(new Integer[map.size()]));
	}
}