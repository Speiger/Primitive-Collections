...
public abstract class AbstractFloatIteratorTester{
	...
	private final FloatList expectedElements;
	...
	protected final class MultiExceptionListIterator implements FloatListIterator {
		final FloatArrayList nextElements = new FloatArrayList();
		final FloatArrayList previousElements = new FloatArrayList();
		FloatArrayList stackWithLastReturnedElementAtTop = null;

		MultiExceptionListIterator(FloatList expectedElements) {
			FloatHelpers.addAll(nextElements, FloatHelpers.reverse(expectedElements));
		...
		void promoteToNext(float e) {
			if (nextElements.remFloat(e)) {
				nextElements.push(e);
			} else {
		...
		private float transferElement(FloatArrayList source, FloatArrayList destination) {
			if (source.isEmpty()) {
		...
		private FloatList getElements() {
			FloatList elements = new FloatArrayList();
			FloatHelpers.addAll(elements, previousElements);
			FloatHelpers.addAll(elements, FloatHelpers.reverse(nextElements));
			return elements;
		}
	}
	...
	AbstractFloatIteratorTester(int steps, FloatIterable elementsToInsertIterable,
			Iterable<? extends IteratorFeature> features, FloatIterable expectedElements, KnownOrder knownOrder, int startIndex) {
		stimuli = new Stimulus[steps];
		if (!elementsToInsertIterable.iterator().hasNext()) {
			throw new IllegalArgumentException();
		}
		elementsToInsert = FloatHelpers.cycle(elementsToInsertIterable);
		this.features = Helpers.copyToSet(features);
		this.expectedElements = FloatHelpers.copyToList(expectedElements);
		this.knownOrder = knownOrder;
		this.startIndex = startIndex;
	}

	protected abstract Iterable<? extends Stimulus<FloatIterator>> getStimulusValues();

	protected abstract FloatIterator newTargetIterator();
	...
	public void testForEachRemaining() {
		for (int i = 0; i < expectedElements.size() - 1; i++) {
			...
			FloatList targetElements = new FloatArrayList();
			FloatIterator iterator = newTargetIterator();
			for (int j = 0; j < i; j++) {
				targetElements.add(iterator.nextFloat());
			}
			...
			if (knownOrder == KnownOrder.KNOWN_ORDER) {
				...
				FloatHelpers.assertEqualIgnoringOrder(expectedElements, targetElements);
			}
		}
	}
	...
	private void recurse(int level) {
		if (level == stimuli.length) {
			...
			for (Stimulus<FloatIterator> stimulus : getStimulusValues()) {
	...
	private void compareResultsForThisListOfStimuli() {
		...
		FloatIterator target = newTargetIterator();
	...
	private interface IteratorOperation {
		float execute(FloatIterator iterator);
	}
	...
	private final IteratorOperation newAddMethod() {
		final float toInsert = elementsToInsert.nextFloat();
		return new IteratorOperation() {
			@Override
			public float execute(FloatIterator iterator) {
				@SuppressWarnings("unchecked")
				FloatListIterator rawIterator = (FloatListIterator) iterator;
	...
	private final IteratorOperation newSetMethod() {
		...
		return new IteratorOperation() {
			...
			public float execute(FloatIterator iterator) {
				@SuppressWarnings("unchecked")
				FloatListIterator li = (FloatListIterator) iterator;
				li.set(toInsert);
				return -1F;
			}
		};
	...
	abstract static class Stimulus<E extends FloatIterator> {
		...
		abstract void executeAndCompare(FloatListIterator reference, E target);
	...
	Stimulus<FloatIterator> hasNext = new Stimulus<FloatIterator>("hasNext") {
		@Override
	...
	@SuppressWarnings("unchecked")
	List<Stimulus<FloatIterator>> iteratorStimuli() {
		return Arrays.asList(hasNext, next, remove);
	}
	
	Stimulus<FloatBidirectionalIterator> hasPrevious = new Stimulus<FloatBidirectionalIterator>("hasPrevious") {
		@Override
	...
	List<Stimulus<FloatBidirectionalIterator>> biIteratorStimuli() {
		return Arrays.asList(hasPrevious, previous);
	}
	
	Stimulus<FloatListIterator> nextIndex = new Stimulus<FloatListIterator>("nextIndex") {
		@Override
		void executeAndCompare(FloatListIterator reference, FloatListIterator target) {
			Assert.assertEquals(reference.nextIndex(), target.nextIndex());
		}
	};
	Stimulus<FloatListIterator> previousIndex = new Stimulus<FloatListIterator>("previousIndex") {
		@Override
		void executeAndCompare(FloatListIterator reference, FloatListIterator target) {
			Assert.assertEquals(reference.previousIndex(), target.previousIndex());
		}
	};
	Stimulus<FloatListIterator> add = new Stimulus<FloatListIterator>("add") {
		@Override
		void executeAndCompare(FloatListIterator reference, FloatListIterator target) {
			internalExecuteAndCompare(reference, target, newAddMethod());
		}
	};
	Stimulus<FloatListIterator> set = new Stimulus<FloatListIterator>("set") {
		@Override
		void executeAndCompare(FloatListIterator reference, FloatListIterator target) {
	...
	List<Stimulus<FloatListIterator>> listIteratorStimuli() {
