...
@Ignore
public class Short2DoubleMapAddToTester extends AbstractShort2DoubleMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2DoubleMap.Entry> iterator = getMap().short2DoubleEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private double addTo(Short2DoubleMap.Entry entry) {
		return getMap().addTo(entry.getShortKey(), entry.getDoubleValue());
	}
}