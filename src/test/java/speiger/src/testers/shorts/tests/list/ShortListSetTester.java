...
public class ShortListSetTester extends AbstractShortListTester {
	...
	private void doTestSet(short newValue) {
		int index = aValidIndex();
		short initialValue = getList().getShort(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getShort(index));
