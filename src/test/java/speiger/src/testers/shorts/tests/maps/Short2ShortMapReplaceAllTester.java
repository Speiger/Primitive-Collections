...
@Ignore
public class Short2ShortMapReplaceAllTester extends AbstractShort2ShortMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Short2ShortMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Short2ShortMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getShortKey());
			expectedEntries.add(entry(entry.getShortKey(), values().asList().getShort(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Short2ShortMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
