...
@Ignore
public class Short2BooleanMapPutTester extends AbstractShort2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2BooleanMap.Entry> iterator = getMap().short2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Short2BooleanMap.Entry entry) {
		return getMap().put(entry.getShortKey(), entry.getBooleanValue());
	}
}