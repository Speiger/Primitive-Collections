...
@Ignore
public class Short2ObjectMapPutAllArrayTester<V> extends AbstractShort2ObjectMapTester<V>
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (V[])null);
	...
	private void putAll(ObjectIterable<Short2ObjectMap.Entry<V>> entries) {
		Short2ObjectMap<V> map = new Short2ObjectLinkedOpenHashMap<>();
		for (Short2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getShortKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2ObjectMap.Entry<V>> entries) {
		Map<Short, V> map = new Short2ObjectLinkedOpenHashMap<>();
		for (Short2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getShortKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
}