...
public class Short2LongMapReplaceAllTester extends AbstractShort2LongMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Short2LongMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Short2LongMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getShortKey());
			expectedEntries.add(entry(entry.getShortKey(), values().asList().getLong(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Short2LongMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
