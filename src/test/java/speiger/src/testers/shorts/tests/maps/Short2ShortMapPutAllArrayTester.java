...
@Ignore
public class Short2ShortMapPutAllArrayTester extends AbstractShort2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2ShortMap.Entry> iterator = getMap().short2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (short[])null);
	...
	private void putAll(ObjectIterable<Short2ShortMap.Entry> entries) {
		Short2ShortMap map = new Short2ShortLinkedOpenHashMap();
		for (Short2ShortMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2ShortMap.Entry> entries) {
		Map<Short, Short> map = new Short2ShortLinkedOpenHashMap();
		for (Short2ShortMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}