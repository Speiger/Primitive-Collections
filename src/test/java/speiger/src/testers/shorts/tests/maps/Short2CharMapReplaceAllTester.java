...
@Ignore
public class Short2CharMapReplaceAllTester extends AbstractShort2CharMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Short2CharMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Short2CharMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getShortKey());
			expectedEntries.add(entry(entry.getShortKey(), values().asList().getChar(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Short2CharMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
