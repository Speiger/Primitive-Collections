...
@Ignore
public class Short2FloatMapAddToTester extends AbstractShort2FloatMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2FloatMap.Entry> iterator = getMap().short2FloatEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private float addTo(Short2FloatMap.Entry entry) {
		return getMap().addTo(entry.getShortKey(), entry.getFloatValue());
	}
}