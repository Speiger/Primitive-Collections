...
@Ignore
public class Short2BooleanMapPutAllArrayTester extends AbstractShort2BooleanMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2BooleanMap.Entry> iterator = getMap().short2BooleanEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (boolean[])null);
	...
	private void putAll(ObjectIterable<Short2BooleanMap.Entry> entries) {
		Short2BooleanMap map = new Short2BooleanLinkedOpenHashMap();
		for (Short2BooleanMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toBooleanArray(new boolean[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2BooleanMap.Entry> entries) {
		Map<Short, Boolean> map = new Short2BooleanLinkedOpenHashMap();
		for (Short2BooleanMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Boolean[map.size()]));
	}
}