...
@Ignore
public class Short2ShortSortedMapNavigationTester extends AbstractShort2ShortMapTester
{
	private Short2ShortSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Short2ShortSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getShortKey(), entries.get(j).getShortKey()).short2ShortEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.short2ShortEntrySet().iterator();
