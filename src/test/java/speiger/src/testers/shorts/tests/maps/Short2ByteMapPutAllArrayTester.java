...
public class Short2ByteMapPutAllArrayTester extends AbstractShort2ByteMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2ByteMap.Entry> iterator = getMap().short2ByteEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (byte[])null);
	...
	private void putAll(ObjectIterable<Short2ByteMap.Entry> entries) {
		Short2ByteMap map = new Short2ByteLinkedOpenHashMap();
		for (Short2ByteMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2ByteMap.Entry> entries) {
		Map<Short, Byte> map = new Short2ByteLinkedOpenHashMap();
		for (Short2ByteMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}