...
@Ignore
public class Short2ShortMapAddToTester extends AbstractShort2ShortMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2ShortMap.Entry> iterator = getMap().short2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Short2ShortMap.Entry entry) {
		return getMap().addTo(entry.getShortKey(), entry.getShortValue());
	}
}