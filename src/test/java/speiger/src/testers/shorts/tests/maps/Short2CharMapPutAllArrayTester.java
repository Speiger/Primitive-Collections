...
public class Short2CharMapPutAllArrayTester extends AbstractShort2CharMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2CharMap.Entry> iterator = getMap().short2CharEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (char[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Short[])null, (Character[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Short2CharMap.Entry> entries) {
		Short2CharMap map = new Short2CharLinkedOpenHashMap();
		for (Short2CharMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toCharArray(new char[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2CharMap.Entry> entries) {
		Map<Short, Character> map = new Short2CharLinkedOpenHashMap();
		for (Short2CharMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Character[map.size()]));
	}
}