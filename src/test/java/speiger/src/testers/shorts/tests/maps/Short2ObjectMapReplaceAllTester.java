...
public class Short2ObjectMapReplaceAllTester<V> extends AbstractShort2ObjectMapTester<V>
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Short2ObjectMap.Entry<V>> expectedEntries = new ObjectArrayList<>();
		for (Short2ObjectMap.Entry<V> entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getShortKey());
			expectedEntries.add(entry(entry.getShortKey(), values().asList().get(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Short2ObjectMap.Entry<V>> orderedEntries = getOrderedElements();
		int index = 0;
