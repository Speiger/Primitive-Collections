...
public class ShortCollectionRetainAllTester extends AbstractShortCollectionTester
{
	...
	private class Target {
		...
		private final ShortCollection inverse = ShortArrayList.wrap(createSamplesArray());
		private final String description;

		private Target(ShortCollection toRetain, String description) {
	...
	public void setUp() throws Exception {
		...
		disjoint = new Target(ShortArrayList.wrap(e3(), e4()), "disjoint");
		superset = new Target(MinimalShortCollection.of(e0(), e1(), e2(), e3(), e4()), "superset");
		nonEmptyProperSubset = new Target(MinimalShortCollection.of(e1()), "subset");
		sameElements = new Target(ShortArrayList.wrap(createSamplesArray()), "sameElements");
		containsDuplicates = new Target(MinimalShortCollection.of(e0(), e0(), e3(), e3()), "containsDuplicates");
		partialOverlap = new Target(MinimalShortCollection.of(e2(), e3()), "partialOverlap");
	}
	...
	private void expectReturnsTrueExtra(Target target) {
		...
		ShortHelpers.assertEqualIgnoringOrder(target.inverse, list);
	}
