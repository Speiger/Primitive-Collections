...
public class Short2LongMapPutAllArrayTester extends AbstractShort2LongMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2LongMap.Entry> iterator = getMap().short2LongEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (long[])null);
	...
	private void putAll(ObjectIterable<Short2LongMap.Entry> entries) {
		Short2LongMap map = new Short2LongLinkedOpenHashMap();
		for (Short2LongMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toLongArray(new long[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2LongMap.Entry> entries) {
		Map<Short, Long> map = new Short2LongLinkedOpenHashMap();
		for (Short2LongMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Long[map.size()]));
	}
}