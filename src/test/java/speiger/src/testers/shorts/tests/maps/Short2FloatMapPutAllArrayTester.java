...
public class Short2FloatMapPutAllArrayTester extends AbstractShort2FloatMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2FloatMap.Entry> iterator = getMap().short2FloatEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Short2FloatMap.Entry> entries) {
		Short2FloatMap map = new Short2FloatLinkedOpenHashMap();
		for (Short2FloatMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2FloatMap.Entry> entries) {
		Map<Short, Float> map = new Short2FloatLinkedOpenHashMap();
		for (Short2FloatMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}