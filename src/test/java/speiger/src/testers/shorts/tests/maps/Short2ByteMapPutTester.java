...
public class Short2ByteMapPutTester extends AbstractShort2ByteMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2ByteMap.Entry> iterator = getMap().short2ByteEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private byte put(Short2ByteMap.Entry entry) {
		return getMap().put(entry.getShortKey(), entry.getByteValue());
	}
}