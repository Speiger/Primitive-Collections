...
@Ignore
public class Short2DoubleMapPutAllArrayTester extends AbstractShort2DoubleMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Short2DoubleMap.Entry> iterator = getMap().short2DoubleEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((short[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Short2DoubleMap.Entry> entries) {
		Short2DoubleMap map = new Short2DoubleLinkedOpenHashMap();
		for (Short2DoubleMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toShortArray(new short[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Short2DoubleMap.Entry> entries) {
		Map<Short, Double> map = new Short2DoubleLinkedOpenHashMap();
		for (Short2DoubleMap.Entry entry : entries) {
			map.put(entry.getShortKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Short[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}