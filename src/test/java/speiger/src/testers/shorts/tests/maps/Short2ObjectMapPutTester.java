...
public class Short2ObjectMapPutTester<V> extends AbstractShort2ObjectMapTester<V>
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2ObjectMap.Entry<V>> iterator = getMap().short2ObjectEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private V put(Short2ObjectMap.Entry<V> entry) {
		return getMap().put(entry.getShortKey(), entry.getValue());
	}
}