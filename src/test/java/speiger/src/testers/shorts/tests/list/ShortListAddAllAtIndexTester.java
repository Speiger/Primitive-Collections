...
public class ShortListAddAllAtIndexTester extends AbstractShortListTester
{
	...
	public void testAddAllAtIndex_supportedAllPresent() {
		assertTrue("addAll(n, allPresent) should return true", getList().addAll(0, MinimalShortCollection.of(e0())));
	...
	public void testAddAllAtIndex_unsupportedAllPresent() {
		try {
			getList().addAll(0, MinimalShortCollection.of(e0()));
	...
	public void testAddAllAtIndex_supportedSomePresent() {
		assertTrue("addAll(n, allPresent) should return true", getList().addAll(0, MinimalShortCollection.of(e0(), e3())));
	...
	public void testAddAllAtIndex_unsupportedSomePresent() {
		try {
			getList().addAll(0, MinimalShortCollection.of(e0(), e3()));
			fail("addAll(n, allPresent) should throw");
		} catch (UnsupportedOperationException expected) {
		}
		expectUnchanged();
	...
	public void testAddAllAtIndex_nullCollectionReference() {
		try {
			getList().addAll(0, (ShortCollection)null);
	...
	public void testAddAllAtIndex_nullListReference() {
		try {
			getList().addAll(0, (ShortList)null);
			fail("addAll(n, null) should throw");
		} catch (NullPointerException expected) {
		}
		expectUnchanged();
	...
	public void testAddAllAtIndex_negative() {
		try {
			getList().addAll(-1, MinimalShortCollection.of(e3()));
	...
	public void testAddAllAtIndex_tooLarge() {
		try {
			getList().addAll(getNumElements() + 1, MinimalShortCollection.of(e3()));
