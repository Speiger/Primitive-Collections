...
public class Short2IntMapAddToTester extends AbstractShort2IntMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Short2IntMap.Entry> iterator = getMap().short2IntEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private int addTo(Short2IntMap.Entry entry) {
		return getMap().addTo(entry.getShortKey(), entry.getIntValue());
	}
}