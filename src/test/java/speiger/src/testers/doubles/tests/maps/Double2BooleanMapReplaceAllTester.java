...
@Ignore
public class Double2BooleanMapReplaceAllTester extends AbstractDouble2BooleanMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Double2BooleanMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Double2BooleanMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getDoubleKey());
			expectedEntries.add(entry(entry.getDoubleKey(), values().asList().getBoolean(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Double2BooleanMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
