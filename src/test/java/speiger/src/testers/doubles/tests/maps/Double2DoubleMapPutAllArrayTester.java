...
@Ignore
public class Double2DoubleMapPutAllArrayTester extends AbstractDouble2DoubleMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((double[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Double2DoubleMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toDoubleArray(new double[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Double2DoubleMap.Entry> entries) {
		Map<Double, Double> map = new Double2DoubleLinkedOpenHashMap();
		for (Double2DoubleMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Double[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}