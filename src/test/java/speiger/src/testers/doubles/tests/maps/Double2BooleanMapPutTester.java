...
public class Double2BooleanMapPutTester extends AbstractDouble2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Double2BooleanMap.Entry> iterator = getMap().double2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Double2BooleanMap.Entry entry) {
		return getMap().put(entry.getDoubleKey(), entry.getBooleanValue());
	}
}