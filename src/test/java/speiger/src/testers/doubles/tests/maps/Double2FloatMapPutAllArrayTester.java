...
@Ignore
public class Double2FloatMapPutAllArrayTester extends AbstractDouble2FloatMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Double2FloatMap.Entry> iterator = getMap().double2FloatEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((double[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Double2FloatMap.Entry> entries) {
		Double2FloatMap map = new Double2FloatLinkedOpenHashMap();
		for (Double2FloatMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toDoubleArray(new double[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Double2FloatMap.Entry> entries) {
		Map<Double, Float> map = new Double2FloatLinkedOpenHashMap();
		for (Double2FloatMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Double[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}