...
public class DoubleCollectionRetainAllTester extends AbstractDoubleCollectionTester {
	...
	private class Target {
		...
		private final DoubleCollection inverse = DoubleArrayList.wrap(createSamplesArray());
		private final String description;

		private Target(DoubleCollection toRetain, String description) {
	...
	public void setUp() throws Exception {
		...
		disjoint = new Target(DoubleArrayList.wrap(e3(), e4()), "disjoint");
		superset = new Target(MinimalDoubleCollection.of(e0(), e1(), e2(), e3(), e4()), "superset");
		nonEmptyProperSubset = new Target(MinimalDoubleCollection.of(e1()), "subset");
		sameElements = new Target(DoubleArrayList.wrap(createSamplesArray()), "sameElements");
		containsDuplicates = new Target(MinimalDoubleCollection.of(e0(), e0(), e3(), e3()), "containsDuplicates");
		partialOverlap = new Target(MinimalDoubleCollection.of(e2(), e3()), "partialOverlap");
	}
	...
	private void expectReturnsTrueExtra(Target target) {
		...
		DoubleHelpers.assertEqualIgnoringOrder(target.inverse, list);
	}
