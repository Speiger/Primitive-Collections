...
public class Double2ByteMapPutAllArrayTester extends AbstractDouble2ByteMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Double2ByteMap.Entry> iterator = getMap().double2ByteEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((double[])null, (byte[])null);
	...
	private void putAll(ObjectIterable<Double2ByteMap.Entry> entries) {
		Double2ByteMap map = new Double2ByteLinkedOpenHashMap();
		for (Double2ByteMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toDoubleArray(new double[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Double2ByteMap.Entry> entries) {
		Map<Double, Byte> map = new Double2ByteLinkedOpenHashMap();
		for (Double2ByteMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray(new Double[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}