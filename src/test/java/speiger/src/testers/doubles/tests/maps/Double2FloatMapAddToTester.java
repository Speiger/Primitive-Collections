...
@Ignore
public class Double2FloatMapAddToTester extends AbstractDouble2FloatMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Double2FloatMap.Entry> iterator = getMap().double2FloatEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private float addTo(Double2FloatMap.Entry entry) {
		return getMap().addTo(entry.getDoubleKey(), entry.getFloatValue());
	}
}