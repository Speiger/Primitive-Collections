...
public class Double2LongMapAddToTester extends AbstractDouble2LongMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Double2LongMap.Entry> iterator = getMap().double2LongEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private long addTo(Double2LongMap.Entry entry) {
		return getMap().addTo(entry.getDoubleKey(), entry.getLongValue());
	}
}