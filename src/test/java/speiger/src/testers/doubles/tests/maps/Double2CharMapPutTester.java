...
@Ignore
public class Double2CharMapPutTester extends AbstractDouble2CharMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Double2CharMap.Entry> iterator = getMap().double2CharEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private char put(Double2CharMap.Entry entry) {
		return getMap().put(entry.getDoubleKey(), entry.getCharValue());
	}
}