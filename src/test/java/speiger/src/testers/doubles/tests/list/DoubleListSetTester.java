...
public class DoubleListSetTester extends AbstractDoubleListTester {
	...
	private void doTestSet(double newValue) {
		int index = aValidIndex();
		double initialValue = getList().getDouble(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getDouble(index));
