...
public class DoubleListSwapRemoveAtIndexTester extends AbstractDoubleListTester
{
	...
	private void runRemoveTest(int index) {
		assertEquals(String.format("remove(%d) should return the element at index %d", index, index), getList().getDouble(index), getList().swapRemove(index));
		DoubleList expected = DoubleHelpers.copyToList(createSamplesArray());
		expected.set(index, expected.getDouble(expected.size()-1));
		expected.removeDouble(expected.size()-1);
