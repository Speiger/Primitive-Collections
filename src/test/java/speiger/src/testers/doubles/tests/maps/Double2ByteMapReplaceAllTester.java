...
@Ignore
public class Double2ByteMapReplaceAllTester extends AbstractDouble2ByteMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Double2ByteMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Double2ByteMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getDoubleKey());
			expectedEntries.add(entry(entry.getDoubleKey(), values().asList().getByte(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Double2ByteMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
