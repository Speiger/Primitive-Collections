...
public class Double2ShortMapPutAllArrayTester extends AbstractDouble2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Double2ShortMap.Entry> iterator = getMap().double2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((double[])null, (short[])null);
	...
	private void putAll(ObjectIterable<Double2ShortMap.Entry> entries) {
		Double2ShortMap map = new Double2ShortLinkedOpenHashMap();
		for (Double2ShortMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toDoubleArray(new double[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Double2ShortMap.Entry> entries) {
		Map<Double, Short> map = new Double2ShortLinkedOpenHashMap();
		for (Double2ShortMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Double[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}