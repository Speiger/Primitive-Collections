...
@Ignore
public class Double2CharMapPutAllArrayTester extends AbstractDouble2CharMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Double2CharMap.Entry> iterator = getMap().double2CharEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((double[])null, (char[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Double[])null, (Character[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Double2CharMap.Entry> entries) {
		Double2CharMap map = new Double2CharLinkedOpenHashMap();
		for (Double2CharMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toDoubleArray(new double[map.size()]), map.values().toCharArray(new char[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Double2CharMap.Entry> entries) {
		Map<Double, Character> map = new Double2CharLinkedOpenHashMap();
		for (Double2CharMap.Entry entry : entries) {
			map.put(entry.getDoubleKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toArray(new Double[map.size()]), map.values().toArray(new Character[map.size()]));
	}
}