...
public class Double2DoubleSortedMapNavigationTester extends AbstractDouble2DoubleMapTester
{
	private Double2DoubleSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Double2DoubleSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getDoubleKey(), entries.get(j).getDoubleKey()).double2DoubleEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.double2DoubleEntrySet().iterator();
