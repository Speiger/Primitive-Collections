...
public class Object2ShortMapAddToTester<T> extends AbstractObject2ShortMapTester<T>
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddTo_supportedNotPresent() {
		assertEquals("addTo(notPresent, value) should return (short)-1", (short)-1, addTo(e3()));
		expectAdded(e3());
	}
	...
	public void testAddToSum_supportedNotPresent() {
		...
		expectAdded(entry(k3(), (short)(v3()+v3())));
	}
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Object2ShortMap.Entry<T>> iterator = getMap().object2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Object2ShortMap.Entry<T> entry) {
		return getMap().addTo(entry.getKey(), entry.getShortValue());
	}
}