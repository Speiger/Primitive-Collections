...
public class Object2IntMapPutAllArrayTester<T> extends AbstractObject2IntMapTester<T>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Object2IntMap.Entry<T>> iterator = getMap().object2IntEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (Integer[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Object2IntMap.Entry<T>> entries) {
		...
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toIntArray(new int[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2IntMap.Entry<T>> entries) {
		Map<T, Integer> map = new Object2IntLinkedOpenHashMap<>();
		for (Object2IntMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getIntValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Integer[map.size()]));
	}
}