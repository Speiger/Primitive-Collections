...
@Ignore
public class Object2LongMapPutAllArrayTester<T> extends AbstractObject2LongMapTester<T>
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (long[])null);
	...
	private void putAll(ObjectIterable<Object2LongMap.Entry<T>> entries) {
		...
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toLongArray(new long[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2LongMap.Entry<T>> entries) {
		Map<T, Long> map = new Object2LongLinkedOpenHashMap<>();
		for (Object2LongMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Long[map.size()]));
	}
}