...
@Ignore
public class Object2DoubleMapPutAllArrayTester<T> extends AbstractObject2DoubleMapTester<T>
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Object2DoubleMap.Entry<T>> entries) {
		...
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2DoubleMap.Entry<T>> entries) {
		Map<T, Double> map = new Object2DoubleLinkedOpenHashMap<>();
		for (Object2DoubleMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}