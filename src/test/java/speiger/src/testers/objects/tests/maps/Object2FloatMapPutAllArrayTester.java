...
public class Object2FloatMapPutAllArrayTester<T> extends AbstractObject2FloatMapTester<T>
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Object2FloatMap.Entry<T>> entries) {
		...
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2FloatMap.Entry<T>> entries) {
		Map<T, Float> map = new Object2FloatLinkedOpenHashMap<>();
		for (Object2FloatMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}