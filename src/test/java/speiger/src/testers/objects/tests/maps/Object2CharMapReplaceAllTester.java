...
public class Object2CharMapReplaceAllTester<T> extends AbstractObject2CharMapTester<T>
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Object2CharMap.Entry<T>> expectedEntries = new ObjectArrayList<>();
		for (Object2CharMap.Entry<T> entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getKey());
			expectedEntries.add(entry(entry.getKey(), values().asList().getChar(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Object2CharMap.Entry<T>> orderedEntries = getOrderedElements();
		int index = 0;
