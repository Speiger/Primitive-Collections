...
@Ignore
public class ObjectCollectionRetainAllTester<T> extends AbstractObjectCollectionTester<T>
{

	private class Target {
		private final ObjectCollection<T> toRetain;
		private final ObjectCollection<T> inverse = ObjectArrayList.wrap(createSamplesArray());
		private final String description;

		private Target(ObjectCollection<T> toRetain, String description) {
	...
	public void setUp() throws Exception {
		...
		disjoint = new Target(ObjectArrayList.wrap(e3(), e4()), "disjoint");
		superset = new Target(MinimalObjectCollection.of(e0(), e1(), e2(), e3(), e4()), "superset");
		nonEmptyProperSubset = new Target(MinimalObjectCollection.of(e1()), "subset");
		sameElements = new Target(ObjectArrayList.wrap(createSamplesArray()), "sameElements");
		containsDuplicates = new Target(MinimalObjectCollection.of(e0(), e0(), e3(), e3()), "containsDuplicates");
		partialOverlap = new Target(MinimalObjectCollection.of(e2(), e3()), "partialOverlap");
	}
	...
	private void expectReturnsTrueExtra(Target target) {
		...
		ObjectHelpers.assertEqualIgnoringOrder(target.inverse, list);
	}
