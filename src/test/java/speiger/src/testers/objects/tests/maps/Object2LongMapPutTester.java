...
public class Object2LongMapPutTester<T> extends AbstractObject2LongMapTester<T>
{	
	...
	public void testPut_supportedNotPresent() {
		assertEquals("put(notPresent, value) should return -1L", -1L, put(e3()));
		expectAdded(e3());
	}
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Object2LongMap.Entry<T>> iterator = getMap().object2LongEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private long put(Object2LongMap.Entry<T> entry) {
		return getMap().put(entry.getKey(), entry.getLongValue());
	}
}