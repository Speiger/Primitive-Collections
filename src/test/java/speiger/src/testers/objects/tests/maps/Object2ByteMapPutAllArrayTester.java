...
public class Object2ByteMapPutAllArrayTester<T> extends AbstractObject2ByteMapTester<T>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Object2ByteMap.Entry<T>> iterator = getMap().object2ByteEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (byte[])null);
	...
	private void putAll(ObjectIterable<Object2ByteMap.Entry<T>> entries) {
		Object2ByteMap<T> map = new Object2ByteLinkedOpenHashMap<>();
		for (Object2ByteMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toByteArray(new byte[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2ByteMap.Entry<T>> entries) {
		Map<T, Byte> map = new Object2ByteLinkedOpenHashMap<>();
		for (Object2ByteMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getByteValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Byte[map.size()]));
	}
}