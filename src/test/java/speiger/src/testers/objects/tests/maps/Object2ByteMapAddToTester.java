...
@Ignore
public class Object2ByteMapAddToTester<T> extends AbstractObject2ByteMapTester<T>
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddTo_supportedNotPresent() {
		assertEquals("addTo(notPresent, value) should return (byte)-1", (byte)-1, addTo(e3()));
		expectAdded(e3());
	}
	...
	public void testAddToSum_supportedNotPresent() {
		...
		expectAdded(entry(k3(), (byte)(v3()+v3())));
	}
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Object2ByteMap.Entry<T>> iterator = getMap().object2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Object2ByteMap.Entry<T> entry) {
		return getMap().addTo(entry.getKey(), entry.getByteValue());
	}
}