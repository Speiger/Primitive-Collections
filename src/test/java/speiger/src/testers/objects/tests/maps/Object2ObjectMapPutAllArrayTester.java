...
public class Object2ObjectMapPutAllArrayTester<T, V> extends AbstractObject2ObjectMapTester<T, V>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Object2ObjectMap.Entry<T, V>> iterator = getMap().object2ObjectEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	private void putAll(ObjectIterable<Object2ObjectMap.Entry<T, V>> entries) {
		Object2ObjectMap<T, V> map = new Object2ObjectLinkedOpenHashMap<>();
		for (Object2ObjectMap.Entry<T, V> entry : entries) {
			map.put(entry.getKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2ObjectMap.Entry<T, V>> entries) {
		Map<T, V> map = new Object2ObjectLinkedOpenHashMap<>();
		for (Object2ObjectMap.Entry<T, V> entry : entries) {
			map.put(entry.getKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray((V[])new Object[map.size()]));
