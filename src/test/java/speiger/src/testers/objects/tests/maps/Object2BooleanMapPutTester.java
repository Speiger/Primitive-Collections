...
@Ignore
public class Object2BooleanMapPutTester<T> extends AbstractObject2BooleanMapTester<T>
{	
	...
	public void testPut_supportedNotPresent() {
		assertEquals("put(notPresent, value) should return false", false, put(e3()));
		expectAdded(e3());
	}
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Object2BooleanMap.Entry<T>> iterator = getMap().object2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Object2BooleanMap.Entry<T> entry) {
		return getMap().put(entry.getKey(), entry.getBooleanValue());
	}
}