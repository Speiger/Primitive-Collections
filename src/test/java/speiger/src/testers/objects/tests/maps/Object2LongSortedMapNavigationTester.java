...
public class Object2LongSortedMapNavigationTester<T> extends AbstractObject2LongMapTester<T>
{
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Object2LongSortedMap<T>) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getKey(), entries.get(j).getKey()).object2LongEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry<T>> entryItr = sortedMap.object2LongEntrySet().iterator();
