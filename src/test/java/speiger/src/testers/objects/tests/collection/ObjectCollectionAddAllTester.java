...
public class ObjectCollectionAddAllTester<T> extends AbstractObjectCollectionTester<T>
{
	...
	public void testAddAll_supportedSomePresent() {
		assertTrue("addAll(somePresent) should return true", collection.addAll(MinimalObjectCollection.of(e3(), e0())));
	...
	public void testAddAll_unsupportedSomePresent() {
		try {
			collection.addAll(MinimalObjectCollection.of(e3(), e0()));
	...
	public void testAddAllConcurrentWithIteration() {
		try {
			...
			assertTrue(collection.addAll(MinimalObjectCollection.of(e3(), e0())));
			iterator.next();
	...
	public void testAddAll_unsupportedAllPresent() {
		try {
			assertFalse("addAll(allPresent) should return false or throw", collection.addAll(MinimalObjectCollection.of(e0())));
		} catch (UnsupportedOperationException tolerated) {
		}
		expectUnchanged();
	}
