...
public class ObjectListSwapRemoveTester<T> extends AbstractObjectListTester<T>
{
	...
	public void testRemove_duplicate() {
		...
		T lastElement = arrayAndDuplicate.elements[arrayAndDuplicate.elements.length-1];
		
		int firstIndex = getList().indexOf(duplicate);
		int initialSize = getList().size();
		assertTrue("swapRemove(present) should return true", getList().swapRemove(duplicate));
		assertTrue("removed element should have moved the last element in its place", getList().get(firstIndex) == lastElement);
