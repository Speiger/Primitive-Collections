...
@Ignore
public class Object2ByteSortedMapNavigationTester<T> extends AbstractObject2ByteMapTester<T>
{
	private Object2ByteSortedMap<T> sortedMap;
	private Entry<T> a;
	private Entry<T> c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Object2ByteSortedMap<T>) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getKey(), entries.get(j).getKey()).object2ByteEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry<T>> entryItr = sortedMap.object2ByteEntrySet().iterator();
