...
public class Object2ShortMapPutAllArrayTester<T> extends AbstractObject2ShortMapTester<T>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Object2ShortMap.Entry<T>> iterator = getMap().object2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((T[])null, (short[])null);
	...
	private void putAll(ObjectIterable<Object2ShortMap.Entry<T>> entries) {
		Object2ShortMap<T> map = new Object2ShortLinkedOpenHashMap<>();
		for (Object2ShortMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Object2ShortMap.Entry<T>> entries) {
		Map<T, Short> map = new Object2ShortLinkedOpenHashMap<>();
		for (Object2ShortMap.Entry<T> entry : entries) {
			map.put(entry.getKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray((T[])new Object[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}