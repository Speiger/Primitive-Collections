...
public class Long2ObjectMapPutTester<V> extends AbstractLong2ObjectMapTester<V>
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2ObjectMap.Entry<V>> iterator = getMap().long2ObjectEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private V put(Long2ObjectMap.Entry<V> entry) {
		return getMap().put(entry.getLongKey(), entry.getValue());
	}
}