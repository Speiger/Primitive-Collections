...
public class Long2ObjectMapReplaceAllTester<V> extends AbstractLong2ObjectMapTester<V>
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Long2ObjectMap.Entry<V>> expectedEntries = new ObjectArrayList<>();
		for (Long2ObjectMap.Entry<V> entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getLongKey());
			expectedEntries.add(entry(entry.getLongKey(), values().asList().get(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Long2ObjectMap.Entry<V>> orderedEntries = getOrderedElements();
		int index = 0;
