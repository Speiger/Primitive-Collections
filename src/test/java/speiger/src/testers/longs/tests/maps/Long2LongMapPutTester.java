...
public class Long2LongMapPutTester extends AbstractLong2LongMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2LongMap.Entry> iterator = getMap().long2LongEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private long put(Long2LongMap.Entry entry) {
		return getMap().put(entry.getLongKey(), entry.getLongValue());
	}
}