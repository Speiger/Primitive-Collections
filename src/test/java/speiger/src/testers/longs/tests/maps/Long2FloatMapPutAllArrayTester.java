...
public class Long2FloatMapPutAllArrayTester extends AbstractLong2FloatMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (float[])null);
	...
	private void putAll(ObjectIterable<Long2FloatMap.Entry> entries) {
		...
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toFloatArray(new float[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2FloatMap.Entry> entries) {
		Map<Long, Float> map = new Long2FloatLinkedOpenHashMap();
		for (Long2FloatMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getFloatValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Float[map.size()]));
	}
}