...
@Ignore
public class Long2ObjectMapPutAllArrayTester<V> extends AbstractLong2ObjectMapTester<V>
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Long2ObjectMap.Entry<V>> iterator = getMap().long2ObjectEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (V[])null);
	...
	private void putAll(ObjectIterable<Long2ObjectMap.Entry<V>> entries) {
		Long2ObjectMap<V> map = new Long2ObjectLinkedOpenHashMap<>();
		for (Long2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getLongKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2ObjectMap.Entry<V>> entries) {
		Map<Long, V> map = new Long2ObjectLinkedOpenHashMap<>();
		for (Long2ObjectMap.Entry<V> entry : entries) {
			map.put(entry.getLongKey(), entry.getValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray((V[])new Object[map.size()]));
	}
}