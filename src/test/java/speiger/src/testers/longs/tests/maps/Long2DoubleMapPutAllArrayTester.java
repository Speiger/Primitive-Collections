...
public class Long2DoubleMapPutAllArrayTester extends AbstractLong2DoubleMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Long2DoubleMap.Entry> iterator = getMap().long2DoubleEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (double[])null);
	...
	private void putAll(ObjectIterable<Long2DoubleMap.Entry> entries) {
		Long2DoubleMap map = new Long2DoubleLinkedOpenHashMap();
		for (Long2DoubleMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toDoubleArray(new double[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2DoubleMap.Entry> entries) {
		Map<Long, Double> map = new Long2DoubleLinkedOpenHashMap();
		for (Long2DoubleMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getDoubleValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Double[map.size()]));
	}
}