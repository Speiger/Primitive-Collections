...
public class Long2LongMapPutAllArrayTester extends AbstractLong2LongMapTester
{
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (long[])null);
	...
	private void putAll(ObjectIterable<Long2LongMap.Entry> entries) {
		Long2LongMap map = new Long2LongLinkedOpenHashMap();
		for (Long2LongMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toLongArray(new long[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2LongMap.Entry> entries) {
		Map<Long, Long> map = new Long2LongLinkedOpenHashMap();
		for (Long2LongMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getLongValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Long[map.size()]));
	}
}