...
@Ignore
public class Long2BooleanMapPutTester extends AbstractLong2BooleanMapTester
{	
	...
	public void testPutAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2BooleanMap.Entry> iterator = getMap().long2BooleanEntrySet().iterator();
			put(e3());
			iterator.next();
	...
	private boolean put(Long2BooleanMap.Entry entry) {
		return getMap().put(entry.getLongKey(), entry.getBooleanValue());
	}
}