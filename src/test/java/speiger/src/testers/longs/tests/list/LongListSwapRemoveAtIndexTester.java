...
public class LongListSwapRemoveAtIndexTester extends AbstractLongListTester {
	...
	private void runRemoveTest(int index) {
		assertEquals(String.format("remove(%d) should return the element at index %d", index, index), getList().getLong(index), getList().swapRemove(index));
		LongList expected = LongHelpers.copyToList(createSamplesArray());
		expected.set(index, expected.getLong(expected.size()-1));
		expected.removeLong(expected.size()-1);
