...
public class Long2ByteMapAddToTester extends AbstractLong2ByteMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2ByteMap.Entry> iterator = getMap().long2ByteEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private byte addTo(Long2ByteMap.Entry entry) {
		return getMap().addTo(entry.getLongKey(), entry.getByteValue());
	}
}