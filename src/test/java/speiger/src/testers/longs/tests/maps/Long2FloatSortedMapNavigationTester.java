...
@Ignore
public class Long2FloatSortedMapNavigationTester extends AbstractLong2FloatMapTester
{
	private Long2FloatSortedMap sortedMap;
	private Entry a;
	private Entry c;
	...
	public void setUp() throws Exception {
		super.setUp();
		sortedMap = (Long2FloatSortedMap) getMap();
	...
	public void testSubMap() {
		...
		for (int i = 0; i < entries.size(); i++) {
			for (int j = i + 1; j < entries.size(); j++) {
				assertEqualInOrder(entries.subList(i, j), sortedMap.subMap(entries.get(i).getLongKey(), entries.get(j).getLongKey()).long2FloatEntrySet());
			}
		}
	}
	...
	public void testOrderedByComparator() {
		...
		Iterator<Entry> entryItr = sortedMap.long2FloatEntrySet().iterator();
