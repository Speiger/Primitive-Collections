...
@Ignore
public class Long2CharMapReplaceAllTester extends AbstractLong2CharMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Long2CharMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Long2CharMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getLongKey());
			expectedEntries.add(entry(entry.getLongKey(), values().asList().getChar(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Long2CharMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
