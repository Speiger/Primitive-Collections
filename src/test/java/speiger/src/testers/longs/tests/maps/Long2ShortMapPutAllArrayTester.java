...
@Ignore
public class Long2ShortMapPutAllArrayTester extends AbstractLong2ShortMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Long2ShortMap.Entry> iterator = getMap().long2ShortEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (short[])null);
	...
	private void putAll(ObjectIterable<Long2ShortMap.Entry> entries) {
		Long2ShortMap map = new Long2ShortLinkedOpenHashMap();
		for (Long2ShortMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toShortArray(new short[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2ShortMap.Entry> entries) {
		Map<Long, Short> map = new Long2ShortLinkedOpenHashMap();
		for (Long2ShortMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getShortValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Short[map.size()]));
	}
}