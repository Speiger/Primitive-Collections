...
@Ignore
public class Long2DoubleMapAddToTester extends AbstractLong2DoubleMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2DoubleMap.Entry> iterator = getMap().long2DoubleEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private double addTo(Long2DoubleMap.Entry entry) {
		return getMap().addTo(entry.getLongKey(), entry.getDoubleValue());
	}
}