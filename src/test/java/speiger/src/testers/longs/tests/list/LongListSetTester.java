...
public class LongListSetTester extends AbstractLongListTester {
	...
	private void doTestSet(long newValue) {
		int index = aValidIndex();
		long initialValue = getList().getLong(index);
		assertEquals("set(i, x) should return the old element at position i.", initialValue, getList().set(index, newValue));
		assertEquals("After set(i, x), get(i) should return x", newValue, getList().getLong(index));
