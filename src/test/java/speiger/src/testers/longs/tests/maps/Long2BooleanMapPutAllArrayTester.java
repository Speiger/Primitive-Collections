...
@Ignore
public class Long2BooleanMapPutAllArrayTester extends AbstractLong2BooleanMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Long2BooleanMap.Entry> iterator = getMap().long2BooleanEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (boolean[])null);
	...
	private void putAll(ObjectIterable<Long2BooleanMap.Entry> entries) {
		Long2BooleanMap map = new Long2BooleanLinkedOpenHashMap();
		for (Long2BooleanMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toBooleanArray(new boolean[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2BooleanMap.Entry> entries) {
		Map<Long, Boolean> map = new Long2BooleanLinkedOpenHashMap();
		for (Long2BooleanMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getBooleanValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Boolean[map.size()]));
	}
}