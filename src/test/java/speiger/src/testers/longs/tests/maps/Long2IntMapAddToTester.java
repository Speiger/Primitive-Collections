...
public class Long2IntMapAddToTester extends AbstractLong2IntMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2IntMap.Entry> iterator = getMap().long2IntEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private int addTo(Long2IntMap.Entry entry) {
		return getMap().addTo(entry.getLongKey(), entry.getIntValue());
	}
}