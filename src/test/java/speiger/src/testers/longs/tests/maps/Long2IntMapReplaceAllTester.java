...
public class Long2IntMapReplaceAllTester extends AbstractLong2IntMapTester
{
	...
	public void testReplaceAllRotate() {
		...
		ObjectList<Long2IntMap.Entry> expectedEntries = new ObjectArrayList<>();
		for (Long2IntMap.Entry entry : getSampleEntries()) {
			int index = keys().asList().indexOf(entry.getLongKey());
			expectedEntries.add(entry(entry.getLongKey(), values().asList().getInt(index + 1)));
		}
	...
	public void testReplaceAllPreservesOrder() {
		...
		ObjectList<Long2IntMap.Entry> orderedEntries = getOrderedElements();
		int index = 0;
