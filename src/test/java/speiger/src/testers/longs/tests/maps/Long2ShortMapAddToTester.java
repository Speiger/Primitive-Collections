...
@Ignore
public class Long2ShortMapAddToTester extends AbstractLong2ShortMapTester
{	@MapFeature.Require(SUPPORTS_PUT)
	...
	public void testAddToAbsentConcurrentWithEntrySetIteration() {
		try {
			Iterator<Long2ShortMap.Entry> iterator = getMap().long2ShortEntrySet().iterator();
			addTo(e3());
			iterator.next();
	...
	private short addTo(Long2ShortMap.Entry entry) {
		return getMap().addTo(entry.getLongKey(), entry.getShortValue());
	}
}