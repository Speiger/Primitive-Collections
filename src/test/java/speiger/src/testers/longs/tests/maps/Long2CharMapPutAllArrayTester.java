...
public class Long2CharMapPutAllArrayTester extends AbstractLong2CharMapTester
{
	...
	public void testPutAllArraySomePresentConcurrentWithEntrySetIteration() {
		try {
			ObjectIterator<Long2CharMap.Entry> iterator = getMap().long2CharEntrySet().iterator();
			putAll(MinimalObjectCollection.of(e3(), e0()));
			iterator.next();
	...
	public void testPutAllArray_nullCollectionReference() {
		try {
			getMap().putAll((long[])null, (char[])null);
	...
	public void testPutAllObjectArray_nullCollectionReference() {
		try {
			getMap().putAll((Long[])null, (Character[])null);
			fail("putAll(null) should throw NullPointerException");
		} catch (NullPointerException expected) {
		}
	}
	
	private void putAll(ObjectIterable<Long2CharMap.Entry> entries) {
		Long2CharMap map = new Long2CharLinkedOpenHashMap();
		for (Long2CharMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toLongArray(new long[map.size()]), map.values().toCharArray(new char[map.size()]));
	}
	
	private void putAllObjects(ObjectIterable<Long2CharMap.Entry> entries) {
		Map<Long, Character> map = new Long2CharLinkedOpenHashMap();
		for (Long2CharMap.Entry entry : entries) {
			map.put(entry.getLongKey(), entry.getCharValue());
		}
		getMap().putAll(map.keySet().toArray(new Long[map.size()]), map.values().toArray(new Character[map.size()]));
	}
}