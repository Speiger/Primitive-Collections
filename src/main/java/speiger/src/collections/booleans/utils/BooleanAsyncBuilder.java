...
public class BooleanAsyncBuilder
{
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(Boolean2ObjectFunction<E> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(Boolean2ObjectFunction<V> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E> ObjectAsyncBuilder<E> arrayflatMap(Boolean2ObjectFunction<E[]> mapper) {
	 ...
	 */
	public BooleanAsyncBuilder filter(Boolean2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAny(Boolean2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(Boolean2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(Boolean2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder findFirst(Boolean2BooleanFunction filter) {
	 ...
	 */
	public IntAsyncBuilder count(Boolean2BooleanFunction filter) {
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(BooleanIterator iter, Boolean2BooleanFunction filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
		...
		protected boolean execute() throws Exception {
			switch(type) {
				case 0:
					while(shouldRun() && iter.hasNext()) {
						if(filter.get(iter.nextBoolean())) {
							setResult(true);
							return true;
						}
					}
	...
	private static class FindFirstTask extends BaseBooleanTask
	{
		...
		public FindFirstTask(BooleanIterator iter, Boolean2BooleanFunction filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(BooleanIterator iter, Boolean2BooleanFunction filter) {
			this.iter = iter;
			this.filter = filter;
		}
