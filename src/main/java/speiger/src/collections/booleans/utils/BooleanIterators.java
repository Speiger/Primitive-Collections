...
public class BooleanIterators
{
	...
	private static class MappedIterator<T> implements ObjectIterator<T>
	{
		...
		MappedIterator(BooleanIterator iterator, BooleanFunction<T> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		@Override
		public T next() {
			return mapper.apply(iterator.nextBoolean());
		}
	...
	private static class FlatMappedIterator<T, V extends Iterable<T>> implements ObjectIterator<T>
	{
		...
		FlatMappedIterator(BooleanIterator iterator, BooleanFunction<V> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				...
				last = mapper.apply(iterator.nextBoolean()).iterator();
			}
		}
	...
	private static class FlatMappedArrayIterator<T> implements ObjectIterator<T>
	{
		...
		FlatMappedArrayIterator(BooleanIterator iterator, BooleanFunction<T[]> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				if(last != null && last.hasNext()) return;
				last = ObjectIterators.wrap(mapper.apply(iterator.nextBoolean()));
			}
		}
	...
	private static class RepeatingIterator implements BooleanIterator
	{
		...
		public RepeatingIterator(BooleanIterator iter, int repeat) {
			this.iter = iter;
			this.repeats = repeat;
		}
	...
	private static class SortedIterator implements BooleanIterator
	{
		...
		BooleanCollections.CollectionWrapper sortedElements = null;
		int index = 0;
		...
		public boolean hasNext() {
			if(sortedElements == null) {
				...
				if(hasNext) {
					sortedElements = BooleanCollections.wrapper();
					pour(iterator, sortedElements);
				}
				else sortedElements = BooleanCollections.wrapper();
	...
	private static class FilteredIterator implements BooleanIterator
	{
		...
		public FilteredIterator(BooleanIterator iterator, BooleanPredicate filter) {
			this.iterator = iterator;
			this.filter = filter;
		}
