...
public interface Char2ObjectMap<V> extends Map<Character, V>, CharFunction<V>
{
	...
	/**
	 * A Helper method that allows to put int a Char2ObjectMap.Entry into a map.
	 ...
	 * @return the last present value or default return	value.
	 */
	public default V put(Map.Entry<Character, V> entry) {
		return put(entry.getKey().charValue(), entry.getValue());
	}
	...
	@Override
	public default V apply(char key) {
		return get(key);
	}
	/**
	...
	public default V computeIfAbsent(Character key, Function<? super Character, ? extends V> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return computeIfAbsent(key.charValue(), mappingFunction instanceof CharFunction ? (CharFunction<V>)mappingFunction : K -> mappingFunction.apply(Character.valueOf(K)));
	}
