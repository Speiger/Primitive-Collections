...
public class Char2DoubleOpenHashMap extends AbstractChar2DoubleMap implements ITrimmable
{
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2DoubleMap.Entry> implements Char2DoubleMap.FastEntrySet {
		...
		@Override
		public Optional<Char2DoubleMap.Entry> reduce(ObjectObjectUnaryOperator<Char2DoubleMap.Entry, Char2DoubleMap.Entry> operator) {
			Objects.requireNonNull(operator);
		...
		@Override
		public Optional<Char2DoubleMap.Entry> findFirst(Predicate<Char2DoubleMap.Entry> filter) {
			Objects.requireNonNull(filter);
	...
	private class Values extends AbstractDoubleCollection {
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(values[i])) return OptionalDouble.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2DoubleOpenHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
