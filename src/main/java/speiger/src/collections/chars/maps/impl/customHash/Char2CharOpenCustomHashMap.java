...
public class Char2CharOpenCustomHashMap extends AbstractChar2CharMap implements ITrimmable
{
	...
	protected class MapEntry implements Char2CharMap.Entry, Map.Entry<Character, Character> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getCharKey()) ^ Character.hashCode(getCharValue());
		}
	...
	private class Values extends AbstractCharCollection {
		...
		public OptionalChar findFirst(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.test(values[i])) return OptionalChar.of(values[i]);
			}
			return OptionalChar.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2CharOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
