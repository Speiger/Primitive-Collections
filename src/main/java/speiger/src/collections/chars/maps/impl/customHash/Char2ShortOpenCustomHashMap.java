...
public class Char2ShortOpenCustomHashMap extends AbstractChar2ShortMap implements ITrimmable
{
	...
	protected class MapEntry implements Char2ShortMap.Entry, Map.Entry<Character, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getCharKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
