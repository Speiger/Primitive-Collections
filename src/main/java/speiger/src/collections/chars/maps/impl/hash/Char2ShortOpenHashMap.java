...
public class Char2ShortOpenHashMap extends AbstractChar2ShortMap implements ITrimmable
{
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ShortMap.Entry> implements Char2ShortMap.FastEntrySet {
		...
		public void forEach(Consumer<? super Char2ShortMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--)
				if(keys[i] != (char)0) action.accept(new ValueMapEntry(i));
		}
		...
		@Override
		public void forEachIndexed(IntObjectConsumer<Char2ShortMap.Entry> action) {
			Objects.requireNonNull(action);
			if(size() <= 0) return;
			if(containsNull) action.accept(0, new ValueMapEntry(nullIndex));
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(keys[i] != (char)0) action.accept(index++, new ValueMapEntry(i));
			}
		}
		...
		public <E> void forEach(E input, ObjectObjectConsumer<E, Char2ShortMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0) action.accept(input, new ValueMapEntry(i));
			}
		}
		
		@Override
		public boolean matchesAny(Predicate<Char2ShortMap.Entry> filter) {
		...
		@Override
		public boolean matchesNone(Predicate<Char2ShortMap.Entry> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public boolean matchesAll(Predicate<Char2ShortMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return true;
			MapEntry entry = new MapEntry();
			if(containsNull) {
		...
		@Override
		public int count(Predicate<Char2ShortMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return 0;
			MapEntry entry = new MapEntry();
			int result = 0;
	...
	private final class KeySet extends AbstractCharSet {
		...
		public void forEachIndexed(IntCharConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(keys[i] != (char)0) action.accept(index++, keys[i]);
			}
		}
		...
		public boolean matchesAny(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(keys[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(keys[i])) return false;
			}
			return true;
		}
		...
		public int count(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(keys[i])) result++;
			}
			return result;
		}
	}
	...
	private class Values extends AbstractShortCollection {
		...
		public void forEachIndexed(IntShortConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(keys[i] != (char)0) action.accept(index++, values[i]);
			}
		}
		...
		public boolean matchesAny(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(values[i])) return false;
			}
			return true;
		}
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
		...
		public int count(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.test(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ShortOpenHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
