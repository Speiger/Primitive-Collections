...
public class Char2FloatOpenCustomHashMap extends AbstractChar2FloatMap implements ITrimmable
{
	...
	protected class MapEntry implements Char2FloatMap.Entry, Map.Entry<Character, Float> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getCharKey()) ^ Float.hashCode(getFloatValue());
		}
	...
	private class Values extends AbstractFloatCollection {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.test(values[i])) return OptionalFloat.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2FloatOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
