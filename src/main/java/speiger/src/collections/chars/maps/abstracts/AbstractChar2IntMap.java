...
public abstract class AbstractChar2IntMap extends AbstractMap<Character, Integer> implements Char2IntMap
{
	...
	protected ObjectIterable<Char2IntMap.Entry> getFastIterable(Char2IntMap map) {
		return Char2IntMaps.fastIterable(map);
	}
	
	protected ObjectIterator<Char2IntMap.Entry> getFastIterator(Char2IntMap map) {
		return Char2IntMaps.fastIterator(map);
	}
	...
	public void putAll(Char2IntMap m) {
		for(ObjectIterator<Char2IntMap.Entry> iter = getFastIterator(m);iter.hasNext();) {
	...
	public void replaceInts(CharIntUnaryOperator mappingFunction) {
		...
		for(ObjectIterator<Char2IntMap.Entry> iter = getFastIterator(this);iter.hasNext();) {
			Char2IntMap.Entry entry = iter.next();
		...
		return new AbstractCharSet() {
			...
			public CharIterator iterator() {
				return new CharIterator() {
					ObjectIterator<Char2IntMap.Entry> iter = getFastIterator(AbstractChar2IntMap.this);
					@Override
	...
	public int hashCode() {
		int hash = 0;
		ObjectIterator<Char2IntMap.Entry> iter = getFastIterator(this);
