...
public class Char2ByteOpenCustomHashMap extends AbstractChar2ByteMap implements ITrimmable
{
	...
	protected class MapEntry implements Char2ByteMap.Entry, Map.Entry<Character, Byte> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getCharKey()) ^ Byte.hashCode(getByteValue());
		}
	...
	private class Values extends AbstractByteCollection {
		...
		public OptionalByte findFirst(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.test(values[i])) return OptionalByte.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ByteOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
