...
public class Char2ShortConcurrentOpenHashMap extends AbstractChar2ShortMap implements Char2ShortConcurrentMap, ITrimmable
{
		...
		public OptionalChar findFirst(CharPredicate filter) {
			...
			for(int i = 0,m=segments.length;i<m;i++) {
				...
				try {
					int index = seg.firstIndex;
					while(index != -1){
						if(filter.test(seg.keys[index])) return OptionalChar.of(seg.keys[index]);
						index = (int)seg.links[index];
					}
				}
				finally {
	...
	protected static class Segment extends StampedLock
	{
		...
		protected short computeIfPresentNonDefault(int hash, char key, CharShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
