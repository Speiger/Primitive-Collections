...
public interface Char2DoubleMap extends Map<Character, Double>, Char2DoubleFunction
{
	...
	/**
	 * A Helper method that allows to put int a Char2DoubleMap.Entry into a map.
	 ...
	 * @return the last present value or default return	value.
	 */
	public default Double put(Map.Entry<Character, Double> entry) {
		return put(entry.getKey().charValue(), entry.getValue().doubleValue());
	}
	...
	@Override
	public default double applyAsDouble(char key) {
		return get(key);
	}
	/**
