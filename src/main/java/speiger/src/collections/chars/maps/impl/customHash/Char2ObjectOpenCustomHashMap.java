...
public class Char2ObjectOpenCustomHashMap<V> extends AbstractChar2ObjectMap<V> implements ITrimmable
{
	...
	protected class MapEntry implements Char2ObjectMap.Entry<V>, Map.Entry<Character, V> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getCharKey()) ^ Objects.hashCode(getValue());
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public Optional<V> findFirst(Predicate<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.test(values[i])) return Optional.ofNullable(values[i]);
			}
			return Optional.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ObjectOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
