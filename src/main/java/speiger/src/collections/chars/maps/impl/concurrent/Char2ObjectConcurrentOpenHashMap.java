...
public class Char2ObjectConcurrentOpenHashMap<V> extends AbstractChar2ObjectMap<V> implements Char2ObjectConcurrentMap<V>, ITrimmable
{
	...
	protected static class Segment<V> extends StampedLock
	{
		...
		protected V computeNonDefault(int hash, char key, CharObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsent(int hash, char key, CharFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsentNonDefault(int hash, char key, CharFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V supplyIfAbsentNonDefault(int hash, char key, ObjectSupplier<V> valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfPresentNonDefault(int hash, char key, CharObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0 || Objects.equals(values[index], getDefaultReturnValue())) return getDefaultReturnValue();
