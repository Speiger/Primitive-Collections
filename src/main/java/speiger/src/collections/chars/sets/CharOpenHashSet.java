...
public class CharOpenHashSet extends AbstractCharSet implements ITrimmable
{
	...
	public char[] toCharArray(char[] a) {
		...
		if(containsNull) a[0] = (char)0;
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (char)0) a[index++] = keys[i];
		}
	...
	public Object[] toArray() {
		...
		if(containsNull) obj[0] = Character.valueOf((char)0);
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (char)0) obj[index++] = Character.valueOf(keys[i]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		if(containsNull) a[0] = (E)Character.valueOf((char)0);
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (char)0) a[index++] = (E)Character.valueOf(keys[i]);
		}
	...
	private class SetIterator implements CharIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				CharOpenHashSet.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
