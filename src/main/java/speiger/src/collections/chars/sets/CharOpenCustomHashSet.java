...
public class CharOpenCustomHashSet extends AbstractCharSet implements ITrimmable
{
	...
	public char[] toCharArray(char[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], (char)0)) a[index++] = keys[i];
		}
	...
	public Object[] toArray() {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], (char)0)) obj[index++] = Character.valueOf(keys[i]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], (char)0)) a[index++] = (E)Character.valueOf(keys[i]);
		}
	...
	private class SetIterator implements CharIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				CharOpenCustomHashSet.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
