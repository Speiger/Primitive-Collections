...
public class CharIterators
{
	 ...
	 */
	public static CharListIterator empty() {
		return EMPTY;
	}
	 ...
	 */
	public static CharIterator wrap(char... a) {
	 ...
	 * @return a Iterator that is wrapping a array.
	 */
	public static CharIterator wrap(char[] a, int start, int end) {
	...
	private static class FlatMappedIterator<T, V extends Iterable<T>> implements ObjectIterator<T>
	{
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				...
				last = mapper.apply(iterator.nextChar()).iterator();
			}
		}
	...
	private static class FlatMappedArrayIterator<T> implements ObjectIterator<T>
	{
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				if(last != null && last.hasNext()) return;
				last = ObjectIterators.wrap(mapper.apply(iterator.nextChar()));
			}
		}
	...
	private static class SortedIterator implements CharIterator
	{
		...
		public boolean hasNext() {
			if(sortedElements == null) {
				...
				if(hasNext) {
					sortedElements = CharCollections.wrapper();
					pour(iterator, sortedElements);
				}
				else sortedElements = CharCollections.wrapper();
