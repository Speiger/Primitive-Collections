...
public class CharAsyncBuilder
{
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(CharFunction<E> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(CharFunction<V> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E> ObjectAsyncBuilder<E> arrayflatMap(CharFunction<E[]> mapper) {
	 ...
	 */
	public CharAsyncBuilder filter(CharPredicate filter) {
	 ...
	 */
	public CharAsyncBuilder repeat(int repeats) {
		iterable = CharIterables.repeat(iterable, repeats);
		return this;
	}
	 ...
	 */
	public ObjectAsyncBuilder<char[]> toCharArray() {
	 ...
	 */
	public BooleanAsyncBuilder matchAny(CharPredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(CharPredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(CharPredicate filter) {
	 ...
	 */
	public IntAsyncBuilder count(CharPredicate filter) {
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(CharIterator iter, CharPredicate filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(CharIterator iter, CharPredicate filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class ArrayTask extends BaseObjectTask<char[]>
	{
		CharIterator iter;
		CharCollections.CollectionWrapper wrapper;
	
		public ArrayTask(CharIterable iterable) {
			iter = iterable.iterator();
			ISizeProvider prov = ISizeProvider.of(iterable);
			int size = prov == null ? -1 : prov.size();
			wrapper = size < 0 ? CharCollections.wrapper() : CharCollections.wrapper(size);
		}
		
		@Override
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				wrapper.add(iter.nextChar());
			}
			if(!iter.hasNext()) {
				setResult(wrapper.toCharArray());
				wrapper = null;
				return true;
			}
			return false;
