...
public class ShortIterators
{
	 ...
	 */
	public static ShortListIterator empty() {
		return EMPTY;
	}
	 ...
	 */
	public static <E> ObjectIterator<E> map(Iterator<? extends Short> iterator, ShortFunction<E> mapper) {
	 ...
	 * @return a iterator that is mapped to a new result
	 */
	public static <E> ObjectIterator<E> map(ShortIterator iterator, ShortFunction<E> mapper) {
	 ...
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(Iterator<? extends Short> iterator, ShortFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(ShortIterator iterator, ShortFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(Iterator<? extends Short> iterator, ShortFunction<E[]> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(ShortIterator iterator, ShortFunction<E[]> mapper) {
	 ...
	 */
	public static ShortIterator filter(Iterator<? extends Short> iterator, ShortPredicate filter) {
	 ...
	 * @return a filtered iterator
	 */
	public static ShortIterator filter(ShortIterator iterator, ShortPredicate filter) {
	 ...
	 */
	public static ShortIterator repeat(ShortIterator iterator, int repeats) {
	 ...
	 */
	public static ShortIterator wrap(short... a) {
	 ...
	 * @return a Iterator that is wrapping a array.
	 */
	public static ShortIterator wrap(short[] a, int start, int end) {
	...
	private static class MappedIterator<T> implements ObjectIterator<T>
	{
		...
		MappedIterator(ShortIterator iterator, ShortFunction<T> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		@Override
		public T next() {
			return mapper.apply(iterator.nextShort());
		}
	...
	private static class FlatMappedIterator<T, V extends Iterable<T>> implements ObjectIterator<T>
	{
		...
		FlatMappedIterator(ShortIterator iterator, ShortFunction<V> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				...
				last = mapper.apply(iterator.nextShort()).iterator();
			}
		}
	...
	private static class FlatMappedArrayIterator<T> implements ObjectIterator<T>
	{
		...
		FlatMappedArrayIterator(ShortIterator iterator, ShortFunction<T[]> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				if(last != null && last.hasNext()) return;
				last = ObjectIterators.wrap(mapper.apply(iterator.nextShort()));
			}
		}
	...
	private static class RepeatingIterator implements ShortIterator
	{
		...
		public RepeatingIterator(ShortIterator iter, int repeat) {
			this.iter = iter;
			this.repeats = repeat;
		}
	...
	private static class SortedIterator implements ShortIterator
	{
		...
		ShortCollections.CollectionWrapper sortedElements = null;
		int index = 0;
		...
		public boolean hasNext() {
			if(sortedElements == null) {
				...
				if(hasNext) {
					sortedElements = ShortCollections.wrapper();
					pour(iterator, sortedElements);
				}
				else sortedElements = ShortCollections.wrapper();
	...
	private static class FilteredIterator implements ShortIterator
	{
		...
		public FilteredIterator(ShortIterator iterator, ShortPredicate filter) {
			this.iterator = iterator;
			this.filter = filter;
		}
