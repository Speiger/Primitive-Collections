...
public class ShortCollections
{
	 ...
	 */
	public static ShortCollection singleton(short element) {
	...
	protected static class CollectionWrapper extends AbstractShortCollection implements ITrimmable {
		short[] elements;
		int size = 0;
		...
		public void sort(ShortComparator c) {
			if(c != null) ShortArrays.stableSort(elements, size, c);
			else ShortArrays.stableSort(elements, size);
		}
		
		public void unstableSort(ShortComparator c) {
			if(c != null) ShortArrays.unstableSort(elements, size, c);
			else ShortArrays.unstableSort(elements, size);		
		}
		...
		public boolean trim(int size) {
			...
			elements = value == 0 ? ShortArrays.EMPTY_ARRAY : Arrays.copyOf(elements, value);
			return true;
		}
		...
		public void clearAndTrim(int size) {
			...
			elements = size == 0 ? ShortArrays.EMPTY_ARRAY : new short[size];
			this.size = size;
		}
		...
		public Object[] toArray() {
			...
			for(int i = 0;i<size;i++)
				obj[i] = Short.valueOf(elements[i]);
			return obj;
		}
		...
		public <E> E[] toArray(E[] a) {
			...
			for(int i = 0;i<size;i++)
				a[i] = (E)Short.valueOf(elements[i]);
	...
	protected static class DistinctCollectionWrapper extends AbstractShortCollection {
		short[] keys;
		...
		private class SetIterator implements ShortIterator {
			...
			public void remove() {
				...
				if(lastReturned == nullIndex) {
					...
					DistinctCollectionWrapper.this.remShort(wrapped[-returnedPos - 1]);
					lastReturned = -1;
					return;
				}
				size--;
	...
	private static class SingletonCollection extends AbstractShortCollection
	{
		short element;
		
		SingletonCollection(short element) {
	...
	public static class SynchronizedCollection implements ShortCollection {
		...
		@Override
		public void forEachIndexed(IntShortConsumer action) { synchronized(mutex) { c.forEachIndexed(action); } }
		@Override
		...
		@Override
		public boolean matchesAny(ShortPredicate filter) { synchronized(mutex) { return c.matchesAny(filter); } }
		@Override
		public boolean matchesNone(ShortPredicate filter) { synchronized(mutex) { return c.matchesNone(filter); } }
		@Override
		public boolean matchesAll(ShortPredicate filter) { synchronized(mutex) { return c.matchesAll(filter); } }
		@Override
	...
	public static class UnmodifiableCollection implements ShortCollection {
		...
		@Override
		public void forEachIndexed(IntShortConsumer action) { c.forEachIndexed(action); }
		@Override
		...
		@Override
		public boolean matchesAny(ShortPredicate filter) { return c.matchesAny(filter); }
		@Override
		public boolean matchesNone(ShortPredicate filter) { return c.matchesNone(filter); }
		@Override
		public boolean matchesAll(ShortPredicate filter) { return c.matchesAll(filter); }
		@Override
