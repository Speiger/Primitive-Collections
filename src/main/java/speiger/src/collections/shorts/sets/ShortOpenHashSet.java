...
public class ShortOpenHashSet extends AbstractShortSet implements ITrimmable
{
	...
	public short[] toShortArray(short[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (short)0) a[index++] = keys[i];
		}
	...
	public Object[] toArray() {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (short)0) obj[index++] = Short.valueOf(keys[i]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(keys[i] != (short)0) a[index++] = (E)Short.valueOf(keys[i]);
		}
	...
	private class SetIterator implements ShortIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				ShortOpenHashSet.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
