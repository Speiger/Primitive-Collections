...
public abstract class AbstractShortList extends AbstractShortCollection implements ShortList 
{
	...
	private class ReversedList extends AbstractShortList
	{
		final AbstractShortList list;
		
		public ReversedList(AbstractShortList list) {
			this.list = list;
		}
		...
		public boolean addAll(int index, ShortList c) {
			if(c instanceof RandomAccess) {
				for(int i = 0,m=c.size();i<m;i++) {
					list.add(list.size() - index - i - 1, c.getShort(i));
				}
				return true;
			}
			return addCollection(index, c);
		...
		private boolean addCollection(int index, ShortCollection c) {
			int i = 0;
			for(ShortIterator iter = c.iterator();iter.hasNext();i++) {
				list.add(list.size() - index - i - 1, iter.nextShort());
			}
			return true;
		}
		...
		public boolean addAll(int index, Collection<? extends Short> c) {
			int i = 0;
			for(Iterator<? extends Short> iter = c.iterator();iter.hasNext();i++) {
		...
		public short getShort(int index) {
			return list.getShort(list.size() - index - 1);
		}
		...
		public short removeShort(int index) {
			return list.removeShort(list.size() - index - 1);
		}
		...
		private short[] reverse(short[] data) {
	...
	private class ListIndexedIterator implements ShortListIterator {
		IntList indecies;
		int index;
		int lastReturned = -1;
		...
		public short nextShort() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getShort((lastReturned = indecies.getInt(i)));
		}
		...
		public short previousShort() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getShort((lastReturned = indecies.getInt(index)));
		}
	...
	private class IndexedIterator implements ShortListIterator {
		int[] indecies;
		int index;
		int lastReturned = -1;
		...
		public short nextShort() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getShort((lastReturned = indecies[i]));
		}
		...
		public short previousShort() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getShort((lastReturned = indecies[index]));
		}
