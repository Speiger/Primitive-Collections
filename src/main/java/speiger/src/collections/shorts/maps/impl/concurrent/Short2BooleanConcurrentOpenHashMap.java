...
public class Short2BooleanConcurrentOpenHashMap extends AbstractShort2BooleanMap implements Short2BooleanConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected boolean computeIfAbsent(int hash, short key, ShortPredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeNonDefault(int hash, short key, ShortBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfAbsentNonDefault(int hash, short key, ShortPredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean supplyIfAbsentNonDefault(int hash, short key, BooleanSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfPresentNonDefault(int hash, short key, ShortBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
