...
public class Short2FloatConcurrentOpenHashMap extends AbstractShort2FloatMap implements Short2FloatConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected float computeNonDefault(int hash, short key, ShortFloatUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfAbsentNonDefault(int hash, short key, Short2FloatFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float supplyIfAbsentNonDefault(int hash, short key, FloatSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfPresentNonDefault(int hash, short key, ShortFloatUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
