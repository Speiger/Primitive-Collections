...
public class Short2LongOpenCustomHashMap extends AbstractShort2LongMap implements ITrimmable
{
	...
	protected class MapEntry implements Short2LongMap.Entry, Map.Entry<Short, Long> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getShortKey()) ^ Long.hashCode(getLongValue());
		}
	...
	private class Values extends AbstractLongCollection {
		...
		public OptionalLong findFirst(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.test(values[i])) return OptionalLong.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2LongOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
