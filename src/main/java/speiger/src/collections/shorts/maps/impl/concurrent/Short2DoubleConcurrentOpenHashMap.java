...
public class Short2DoubleConcurrentOpenHashMap extends AbstractShort2DoubleMap implements Short2DoubleConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected double computeNonDefault(int hash, short key, ShortDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfAbsentNonDefault(int hash, short key, Short2DoubleFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double supplyIfAbsentNonDefault(int hash, short key, DoubleSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfPresentNonDefault(int hash, short key, ShortDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
