 ...
 */
public interface Short2BooleanMap extends Map<Short, Boolean>, ShortPredicate
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Short2BooleanMap.Entry into a map.
	...
	@Override
	public default boolean test(short key) {
		return get(key);
	}
	/**
	...
	public default Boolean computeIfAbsent(Short key, Function<? super Short, ? extends Boolean> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Boolean.valueOf(computeBooleanIfAbsent(key.shortValue(), mappingFunction instanceof ShortPredicate ? (ShortPredicate)mappingFunction : K -> mappingFunction.apply(Short.valueOf(K)).booleanValue()));
	}
