...
public abstract class AbstractShort2ObjectMap<V> extends AbstractMap<Short, V> implements Short2ObjectMap<V>
{
	...
	protected ObjectIterable<Short2ObjectMap.Entry<V>> getFastIterable(Short2ObjectMap<V> map) {
		return Short2ObjectMaps.fastIterable(map);
	}
	
	protected ObjectIterator<Short2ObjectMap.Entry<V>> getFastIterator(Short2ObjectMap<V> map) {
		return Short2ObjectMaps.fastIterator(map);
	}
	...
	public void putAll(Short2ObjectMap<V> m) {
		for(ObjectIterator<Short2ObjectMap.Entry<V>> iter = getFastIterator(m);iter.hasNext();) {
	...
	public void replaceObjects(ShortObjectUnaryOperator<V> mappingFunction) {
		...
		for(ObjectIterator<Short2ObjectMap.Entry<V>> iter = getFastIterator(this);iter.hasNext();) {
			Short2ObjectMap.Entry<V> entry = iter.next();
		...
		return new AbstractShortSet() {
			...
			public ShortIterator iterator() {
				return new ShortIterator() {
					ObjectIterator<Short2ObjectMap.Entry<V>> iter = getFastIterator(AbstractShort2ObjectMap.this);
					@Override
	...
	public int hashCode() {
		int hash = 0;
		ObjectIterator<Short2ObjectMap.Entry<V>> iter = getFastIterator(this);
