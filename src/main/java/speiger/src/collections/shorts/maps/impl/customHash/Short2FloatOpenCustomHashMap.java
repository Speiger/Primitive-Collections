...
public class Short2FloatOpenCustomHashMap extends AbstractShort2FloatMap implements ITrimmable
{
	...
	protected class MapEntry implements Short2FloatMap.Entry, Map.Entry<Short, Float> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getShortKey()) ^ Float.hashCode(getFloatValue());
		}
	...
	private class Values extends AbstractFloatCollection {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.test(values[i])) return OptionalFloat.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2FloatOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
