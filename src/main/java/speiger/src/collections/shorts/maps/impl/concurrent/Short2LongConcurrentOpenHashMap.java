...
public class Short2LongConcurrentOpenHashMap extends AbstractShort2LongMap implements Short2LongConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected long computeNonDefault(int hash, short key, ShortLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsentNonDefault(int hash, short key, Short2LongFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long supplyIfAbsentNonDefault(int hash, short key, LongSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfPresentNonDefault(int hash, short key, ShortLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
