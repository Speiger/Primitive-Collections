...
public class Short2CharConcurrentOpenHashMap extends AbstractShort2CharMap implements Short2CharConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected char computeNonDefault(int hash, short key, ShortCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfAbsentNonDefault(int hash, short key, Short2CharFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char supplyIfAbsentNonDefault(int hash, short key, CharSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfPresentNonDefault(int hash, short key, ShortCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
