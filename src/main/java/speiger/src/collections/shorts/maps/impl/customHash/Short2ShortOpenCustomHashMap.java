...
public class Short2ShortOpenCustomHashMap extends AbstractShort2ShortMap implements ITrimmable
{
	...
	protected class MapEntry implements Short2ShortMap.Entry, Map.Entry<Short, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getShortKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
			return OptionalShort.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
