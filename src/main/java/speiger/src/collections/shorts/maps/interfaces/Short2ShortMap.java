 ...
 */
public interface Short2ShortMap extends Map<Short, Short>, ShortUnaryOperator
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Short2ShortMap.Entry into a map.
	...
	@Override
	public default short applyAsShort(short key) {
		return get(key);
	}
	/**
	...
	public default Short computeIfAbsent(Short key, Function<? super Short, ? extends Short> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Short.valueOf(computeShortIfAbsent(key.shortValue(), mappingFunction instanceof ShortUnaryOperator ? (ShortUnaryOperator)mappingFunction : K -> mappingFunction.apply(Short.valueOf(K)).shortValue()));
	}
