...
public class Short2ObjectOpenCustomHashMap<V> extends AbstractShort2ObjectMap<V> implements ITrimmable
{
	...
	protected class MapEntry implements Short2ObjectMap.Entry<V>, Map.Entry<Short, V> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getShortKey()) ^ Objects.hashCode(getValue());
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public Optional<V> findFirst(Predicate<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.test(values[i])) return Optional.ofNullable(values[i]);
			}
			return Optional.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2ObjectOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
