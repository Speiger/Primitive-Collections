...
public class Short2IntConcurrentOpenHashMap extends AbstractShort2IntMap implements Short2IntConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected int computeNonDefault(int hash, short key, ShortIntUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfAbsentNonDefault(int hash, short key, Short2IntFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int supplyIfAbsentNonDefault(int hash, short key, IntSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfPresentNonDefault(int hash, short key, ShortIntUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
