...
public class Short2ByteConcurrentOpenHashMap extends AbstractShort2ByteMap implements Short2ByteConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected byte computeNonDefault(int hash, short key, ShortByteUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfAbsentNonDefault(int hash, short key, Short2ByteFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte supplyIfAbsentNonDefault(int hash, short key, ByteSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfPresentNonDefault(int hash, short key, ShortByteUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
