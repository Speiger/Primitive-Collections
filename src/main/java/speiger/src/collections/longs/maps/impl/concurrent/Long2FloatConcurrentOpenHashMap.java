...
public class Long2FloatConcurrentOpenHashMap extends AbstractLong2FloatMap implements Long2FloatConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected float computeIfAbsentNonDefault(int hash, long key, Long2FloatFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfPresentNonDefault(int hash, long key, LongFloatUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
