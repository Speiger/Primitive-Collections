...
public class Long2IntConcurrentOpenHashMap extends AbstractLong2IntMap implements Long2IntConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected int computeIfAbsentNonDefault(int hash, long key, Long2IntFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int supplyIfAbsentNonDefault(int hash, long key, IntSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfPresentNonDefault(int hash, long key, LongIntUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
