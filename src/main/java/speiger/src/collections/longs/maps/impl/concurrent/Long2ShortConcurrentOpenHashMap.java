...
public class Long2ShortConcurrentOpenHashMap extends AbstractLong2ShortMap implements Long2ShortConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected short computeNonDefault(int hash, long key, LongShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsentNonDefault(int hash, long key, Long2ShortFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short supplyIfAbsentNonDefault(int hash, long key, ShortSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfPresentNonDefault(int hash, long key, LongShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
