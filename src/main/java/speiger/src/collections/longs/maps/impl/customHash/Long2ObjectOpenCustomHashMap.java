...
public class Long2ObjectOpenCustomHashMap<V> extends AbstractLong2ObjectMap<V> implements ITrimmable
{
	...
	protected class MapEntry implements Long2ObjectMap.Entry<V>, Map.Entry<Long, V> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Objects.hashCode(getValue());
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements Long2ObjectMap.FastEntrySet<V> {
		...
		@Override
		public void forEachIndexed(IntObjectConsumer<Long2ObjectMap.Entry<V>> action) {
		...
		@Override
		public boolean matchesAny(Predicate<Long2ObjectMap.Entry<V>> filter) {
		...
		@Override
		public boolean matchesNone(Predicate<Long2ObjectMap.Entry<V>> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public boolean matchesAll(Predicate<Long2ObjectMap.Entry<V>> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return true;
			MapEntry entry = new MapEntry();
			if(containsNull) {
		...
		@Override
		public int count(Predicate<Long2ObjectMap.Entry<V>> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return 0;
			MapEntry entry = new MapEntry();
			int result = 0;
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public Optional<V> findFirst(Predicate<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return Optional.ofNullable(values[i]);
			}
			return Optional.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2ObjectOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
