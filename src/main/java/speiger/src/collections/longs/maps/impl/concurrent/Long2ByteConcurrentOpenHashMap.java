...
public class Long2ByteConcurrentOpenHashMap extends AbstractLong2ByteMap implements Long2ByteConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected byte computeNonDefault(int hash, long key, LongByteUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfAbsentNonDefault(int hash, long key, Long2ByteFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte supplyIfAbsentNonDefault(int hash, long key, ByteSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfPresentNonDefault(int hash, long key, LongByteUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
