...
public class Long2DoubleConcurrentOpenHashMap extends AbstractLong2DoubleMap implements Long2DoubleConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected double computeIfAbsentNonDefault(int hash, long key, Long2DoubleFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfPresentNonDefault(int hash, long key, LongDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
