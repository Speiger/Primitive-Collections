...
public class Long2IntOpenCustomHashMap extends AbstractLong2IntMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2IntMap.Entry, Map.Entry<Long, Integer> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Integer.hashCode(getIntValue());
		}
	...
	private class Values extends AbstractIntCollection {
		...
		public OptionalInt findFirst(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalInt.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2IntOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
