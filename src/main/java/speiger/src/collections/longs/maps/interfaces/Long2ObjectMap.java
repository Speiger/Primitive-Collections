 ...
 */
public interface Long2ObjectMap<V> extends Map<Long, V>, LongFunction<V>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Long2ObjectMap.Entry into a map.
	...
	@Override
	public default V apply(long key) {
		return get(key);
	}
	/**
	...
	public default V computeIfAbsent(Long key, Function<? super Long, ? extends V> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return computeIfAbsent(key.longValue(), mappingFunction instanceof LongFunction ? (LongFunction<V>)mappingFunction : K -> mappingFunction.apply(Long.valueOf(K)));
	}
