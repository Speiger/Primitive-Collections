...
public class Long2ByteOpenCustomHashMap extends AbstractLong2ByteMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2ByteMap.Entry, Map.Entry<Long, Byte> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Byte.hashCode(getByteValue());
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ByteMap.Entry> implements Long2ByteMap.FastEntrySet {
		...
		public void forEach(Consumer<? super Long2ByteMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--)
				if(!strategy.equals(keys[i], 0L)) action.accept(new ValueMapEntry(i));
		}
		...
		@Override
		public void forEachIndexed(IntObjectConsumer<Long2ByteMap.Entry> action) {
			Objects.requireNonNull(action);
			if(size() <= 0) return;
			if(containsNull) action.accept(0, new ValueMapEntry(nullIndex));
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L)) action.accept(index++, new ValueMapEntry(i));
			}
		}
		...
		public <E> void forEach(E input, ObjectObjectConsumer<E, Long2ByteMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L)) action.accept(input, new ValueMapEntry(i));
			}
		}
		
		@Override
		public boolean matchesAny(Predicate<Long2ByteMap.Entry> filter) {
		...
		@Override
		public boolean matchesNone(Predicate<Long2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public boolean matchesAll(Predicate<Long2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return true;
			MapEntry entry = new MapEntry();
			if(containsNull) {
		...
		@Override
		public Optional<Long2ByteMap.Entry> reduce(ObjectObjectUnaryOperator<Long2ByteMap.Entry, Long2ByteMap.Entry> operator) {
			Objects.requireNonNull(operator);
		...
		@Override
		public Optional<Long2ByteMap.Entry> findFirst(Predicate<Long2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public int count(Predicate<Long2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return 0;
			MapEntry entry = new MapEntry();
			int result = 0;
	...
	private final class KeySet extends AbstractLongSet {
		...
		public void forEachIndexed(IntLongConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L)) action.accept(index++, keys[i]);
			}
		}
		...
		public boolean matchesAny(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(keys[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(keys[i])) return false;
			}
			return true;
		}
		...
		public int count(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(keys[i])) result++;
			}
			return result;
		}
	}
	...
	private class Values extends AbstractByteCollection {
		...
		public void forEachIndexed(IntByteConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L)) action.accept(index++, values[i]);
			}
		}
		...
		public boolean matchesAny(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return false;
			}
			return true;
		}
		...
		public OptionalByte findFirst(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalByte.of(values[i]);
			}
		...
		public int count(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2ByteOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
