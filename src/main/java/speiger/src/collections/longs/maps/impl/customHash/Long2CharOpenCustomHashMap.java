...
public class Long2CharOpenCustomHashMap extends AbstractLong2CharMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2CharMap.Entry, Map.Entry<Long, Character> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Character.hashCode(getCharValue());
		}
	...
	private class Values extends AbstractCharCollection {
		...
		public OptionalChar findFirst(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalChar.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2CharOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
