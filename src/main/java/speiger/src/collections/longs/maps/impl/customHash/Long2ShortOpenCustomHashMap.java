...
public class Long2ShortOpenCustomHashMap extends AbstractLong2ShortMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2ShortMap.Entry, Map.Entry<Long, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ShortMap.Entry> implements Long2ShortMap.FastEntrySet {
		...
		@Override
		public Optional<Long2ShortMap.Entry> reduce(ObjectObjectUnaryOperator<Long2ShortMap.Entry, Long2ShortMap.Entry> operator) {
			Objects.requireNonNull(operator);
		...
		@Override
		public Optional<Long2ShortMap.Entry> findFirst(Predicate<Long2ShortMap.Entry> filter) {
			Objects.requireNonNull(filter);
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
