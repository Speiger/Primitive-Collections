...
public class Long2LongOpenCustomHashMap extends AbstractLong2LongMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2LongMap.Entry, Map.Entry<Long, Long> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Long.hashCode(getLongValue());
		}
	...
	private class Values extends AbstractLongCollection {
		...
		public OptionalLong findFirst(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalLong.of(values[i]);
			}
			return OptionalLong.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2LongOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
