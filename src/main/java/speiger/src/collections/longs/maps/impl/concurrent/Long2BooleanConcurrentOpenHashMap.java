...
public class Long2BooleanConcurrentOpenHashMap extends AbstractLong2BooleanMap implements Long2BooleanConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected boolean computeIfAbsentNonDefault(int hash, long key, LongPredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfPresentNonDefault(int hash, long key, LongBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
