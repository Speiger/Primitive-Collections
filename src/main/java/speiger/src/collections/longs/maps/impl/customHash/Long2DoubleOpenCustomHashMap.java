...
public class Long2DoubleOpenCustomHashMap extends AbstractLong2DoubleMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2DoubleMap.Entry, Map.Entry<Long, Double> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Double.hashCode(getDoubleValue());
		}
	...
	private class Values extends AbstractDoubleCollection {
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalDouble.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2DoubleOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
