...
public class Long2BooleanOpenCustomHashMap extends AbstractLong2BooleanMap implements ITrimmable
{
	...
	protected class MapEntry implements Long2BooleanMap.Entry, Map.Entry<Long, Boolean> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getLongKey()) ^ Boolean.hashCode(getBooleanValue());
		}
	...
	private class Values extends AbstractBooleanCollection {
		...
		public OptionalBoolean findFirst(BooleanPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0L) && filter.test(values[i])) return OptionalBoolean.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2BooleanOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
