...
public class Long2ObjectConcurrentOpenHashMap<V> extends AbstractLong2ObjectMap<V> implements Long2ObjectConcurrentMap<V>, ITrimmable
{
	...
	protected static class Segment<V> extends StampedLock
	{
		...
		protected V computeNonDefault(int hash, long key, LongObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsent(int hash, long key, LongFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsentNonDefault(int hash, long key, LongFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V supplyIfAbsentNonDefault(int hash, long key, ObjectSupplier<V> valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfPresentNonDefault(int hash, long key, LongObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0 || Objects.equals(values[index], getDefaultReturnValue())) return getDefaultReturnValue();
