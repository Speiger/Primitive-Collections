...
public class Long2CharConcurrentOpenHashMap extends AbstractLong2CharMap implements Long2CharConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected char computeNonDefault(int hash, long key, LongCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfAbsentNonDefault(int hash, long key, Long2CharFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char supplyIfAbsentNonDefault(int hash, long key, CharSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfPresentNonDefault(int hash, long key, LongCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
