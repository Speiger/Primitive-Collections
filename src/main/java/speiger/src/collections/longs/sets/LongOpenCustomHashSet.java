...
public class LongOpenCustomHashSet extends AbstractLongSet implements ITrimmable
{
	 ...
	 */
	public static LongOpenCustomHashSet toSet(LongStream stream, LongStrategy strategy) {
		return stream.collect(() -> new LongOpenCustomHashSet(strategy), LongOpenCustomHashSet::add, LongOpenCustomHashSet::addAll);
	}
	...
	public long[] toLongArray(long[] a) {
		...
		if(containsNull) a[0] = 0L;
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0L)) a[index++] = keys[i];
		}
	...
	public Object[] toArray() {
		...
		if(containsNull) obj[0] = Long.valueOf(0L);
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0L)) obj[index++] = Long.valueOf(keys[i]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		if(containsNull) a[0] = (E)Long.valueOf(0L);
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0L)) a[index++] = (E)Long.valueOf(keys[i]);
		}
	...
	private class SetIterator implements LongIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				LongOpenCustomHashSet.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
