...
public class ByteAsyncBuilder
{
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(ByteFunction<E> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(ByteFunction<V> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E> ObjectAsyncBuilder<E> arrayflatMap(ByteFunction<E[]> mapper) {
	 ...
	 */
	public ByteAsyncBuilder filter(BytePredicate filter) {
	 ...
	 */
	public ByteAsyncBuilder repeat(int repeats) {
		iterable = ByteIterables.repeat(iterable, repeats);
		return this;
	}
	 ...
	 */
	public ObjectAsyncBuilder<byte[]> toByteArray() {
	 ...
	 */
	public BooleanAsyncBuilder matchAny(BytePredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(BytePredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(BytePredicate filter) {
	 ...
	 */
	public IntAsyncBuilder count(BytePredicate filter) {
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(ByteIterator iter, BytePredicate filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(ByteIterator iter, BytePredicate filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class ArrayTask extends BaseObjectTask<byte[]>
	{
		ByteIterator iter;
		ByteCollections.CollectionWrapper wrapper;
	
		public ArrayTask(ByteIterable iterable) {
			iter = iterable.iterator();
			ISizeProvider prov = ISizeProvider.of(iterable);
			int size = prov == null ? -1 : prov.size();
			wrapper = size < 0 ? ByteCollections.wrapper() : ByteCollections.wrapper(size);
		}
		
		@Override
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				wrapper.add(iter.nextByte());
			}
			if(!iter.hasNext()) {
				setResult(wrapper.toByteArray());
				wrapper = null;
				return true;
			}
			return false;
