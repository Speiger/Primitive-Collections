...
public interface ByteIterable extends Iterable<Byte>
{
	...
	*/
	 ...
	 */
	default <E> ObjectIterable<E> map(Byte2ObjectFunction<E> mapper) {
	 ...
	 * @return a new Iterable that returns the desired result
	 */
	default <E, V extends Iterable<E>> ObjectIterable<E> flatMap(Byte2ObjectFunction<V> mapper) {
	 ...
	 * @return a new Iterable that returns the desired result
	 */
	default <E> ObjectIterable<E> arrayflatMap(Byte2ObjectFunction<E[]> mapper) {
	 ...
	 */
	default ByteIterable filter(Byte2BooleanFunction filter) {
	 ...
	 */
	default boolean matchesAny(Byte2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(ByteIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextByte())) return true;
		}
		return false;
	}
	 ...
	 */
	default boolean matchesNone(Byte2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(ByteIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextByte())) return false;
		}
		return true;
	}
	 ...
	 */
	default boolean matchesAll(Byte2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(ByteIterator iter = iterator();iter.hasNext();) {
			if(!filter.get(iter.nextByte())) return false;
		}
	 ...
	 */
	default byte findFirst(Byte2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(ByteIterator iter = iterator();iter.hasNext();) {
	 ...
	 */
	default int count(Byte2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		int result = 0;
		for(ByteIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextByte())) result++;
		}
		return result;
	}
}