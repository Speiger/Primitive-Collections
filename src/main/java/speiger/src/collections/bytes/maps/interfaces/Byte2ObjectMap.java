 ...
 */
public interface Byte2ObjectMap<V> extends Map<Byte, V>, ByteFunction<V>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Byte2ObjectMap.Entry into a map.
	...
	@Override
	public default V apply(byte key) {
		return get(key);
	}
	/**
	...
	public default V computeIfAbsent(Byte key, Function<? super Byte, ? extends V> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return computeIfAbsent(key.byteValue(), mappingFunction instanceof ByteFunction ? (ByteFunction<V>)mappingFunction : K -> mappingFunction.apply(Byte.valueOf(K)));
	}
