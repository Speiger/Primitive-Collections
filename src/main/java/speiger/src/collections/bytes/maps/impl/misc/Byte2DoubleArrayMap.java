...
public class Byte2DoubleArrayMap extends AbstractByte2DoubleMap implements Byte2DoubleOrderedMap
{
	...
	public Byte2DoubleArrayMap(Byte2DoubleMap map) {
		this(map.size());
		for(ObjectIterator<Byte2DoubleMap.Entry> iter = getFastIterator(map);iter.hasNext();size++) {
	...
	private class MapEntrySet extends AbstractObjectSet<Byte2DoubleMap.Entry> implements Byte2DoubleOrderedMap.FastOrderedSet {
		...
		@Override
		public void forEachIndexed(IntObjectConsumer<Byte2DoubleMap.Entry> action) {
			Objects.requireNonNull(action);
			for(int i = 0;i<size;i++) {
		...
		@Override
		public boolean matchesAny(Predicate<Byte2DoubleMap.Entry> filter) {
		...
		@Override
		public boolean matchesNone(Predicate<Byte2DoubleMap.Entry> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public boolean matchesAll(Predicate<Byte2DoubleMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return true;
			MapEntry entry = new MapEntry();
			for(int i = 0;i<size;i++) {
		...
		@Override
		public int count(Predicate<Byte2DoubleMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return 0;
			int result = 0;
			MapEntry entry = new MapEntry();
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				if(obj instanceof Byte2DoubleMap.Entry) {
					...
					return keys[index] == entry.getByteKey() && Double.doubleToLongBits(getDoubleValue()) == Double.doubleToLongBits(entry.getDoubleValue());
				}
