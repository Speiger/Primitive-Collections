...
public class ObjectAsyncBuilder<T>
{
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(UnaryOperator<T, E> mapper) {
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(UnaryOperator<T, V> mapper) {
	 ...
	 */
	public ObjectAsyncBuilder<T> filter(Predicate<T> filter) {
	 ...
	 */
	public ObjectAsyncBuilder<T> repeat(int repeats) {
		iterable = ObjectIterables.repeat(iterable, repeats);
		return this;
	}
	 ...
	 */
	public ObjectAsyncBuilder<T[]> toArray(IntFunction<T[]> action) {
		return new ObjectAsyncBuilder<>(new ArrayTask<>(iterable, action));
	}
	 ...
	 */
	public BooleanAsyncBuilder matchAny(Predicate<T> filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(Predicate<T> filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(Predicate<T> filter) {
	 ...
	 */
	public IntAsyncBuilder count(Predicate<T> filter) {
	...
	private static class MatchTask<T> extends BaseBooleanTask
	{
		...
		public MatchTask(ObjectIterator<T> iter, Predicate<T> filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class CountTask<T> extends BaseIntTask
	{
		...
		public CountTask(ObjectIterator<T> iter, Predicate<T> filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class ArrayTask<T> extends BaseObjectTask<T[]>
	{
		ObjectIterator<T> iter;
		ObjectCollections.CollectionWrapper<T> wrapper;
		IntFunction<T[]> builder;
		
		public ArrayTask(ObjectIterable<T> iterable, IntFunction<T[]> builder) {
			this.builder = builder;
			iter = iterable.iterator();
			ISizeProvider prov = ISizeProvider.of(iterable);
			int size = prov == null ? -1 : prov.size();
			wrapper = size < 0 ? ObjectCollections.wrapper() : ObjectCollections.wrapper(size);
		}
		...
		protected boolean execute() throws Exception {
			...
			if(!iter.hasNext()) {
				setResult(wrapper.toArray(builder));
				wrapper = null;
				return true;
			}
			return false;
