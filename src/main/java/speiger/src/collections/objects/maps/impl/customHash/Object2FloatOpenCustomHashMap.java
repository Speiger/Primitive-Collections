...
public class Object2FloatOpenCustomHashMap<T> extends AbstractObject2FloatMap<T> implements ITrimmable
{
	...
	protected class MapEntry implements Object2FloatMap.Entry<T>, Map.Entry<T, Float> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Float.hashCode(getFloatValue());
		}
	...
	private class Values extends AbstractFloatCollection {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return OptionalFloat.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2FloatOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
