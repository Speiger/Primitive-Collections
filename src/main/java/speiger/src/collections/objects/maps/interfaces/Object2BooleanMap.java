...
public interface Object2BooleanMap<T> extends Map<T, Boolean>, Predicate<T>
{
	 ...
	 * @return the last present value or default return value.
	 */
	public default boolean put(Entry<T> entry) {
	...
	@Override
	public default boolean test(T key) {
	...
	public default Boolean computeIfAbsent(T key, Function<? super T, ? extends Boolean> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Boolean.valueOf(computeBooleanIfAbsent(key, mappingFunction instanceof Predicate ? (Predicate<T>)mappingFunction : K -> mappingFunction.apply(K).booleanValue()));
	}
