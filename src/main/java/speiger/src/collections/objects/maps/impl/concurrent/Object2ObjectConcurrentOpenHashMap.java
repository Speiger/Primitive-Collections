...
public class Object2ObjectConcurrentOpenHashMap<T, V> extends AbstractObject2ObjectMap<T, V> implements Object2ObjectConcurrentMap<T, V>, ITrimmable
{
	...
	protected static class Segment<T, V> extends StampedLock
	{
		...
		protected V computeNonDefault(int hash, T key, ObjectObjectUnaryOperator<T, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsent(int hash, T key, UnaryOperator<T, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsentNonDefault(int hash, T key, UnaryOperator<T, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V supplyIfAbsentNonDefault(int hash, T key, ObjectSupplier<V> valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfPresentNonDefault(int hash, T key, ObjectObjectUnaryOperator<T, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0 || Objects.equals(values[index], getDefaultReturnValue())) return getDefaultReturnValue();
