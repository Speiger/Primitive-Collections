...
public class Object2ShortConcurrentOpenHashMap<T> extends AbstractObject2ShortMap<T> implements Object2ShortConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected short computeNonDefault(int hash, T key, ObjectShortUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsent(int hash, T key, ToShortFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsentNonDefault(int hash, T key, ToShortFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short supplyIfAbsentNonDefault(int hash, T key, ShortSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfPresentNonDefault(int hash, T key, ObjectShortUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
