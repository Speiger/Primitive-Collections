 ...
 */
public class Reference2ShortHashMap<T> extends AbstractObject2ShortMap<T> implements ITrimmable
{
	 ...
	 */
	public Reference2ShortHashMap(int minCapacity) {
	 ...
	 */
	public Reference2ShortHashMap(int minCapacity, float loadFactor) {
		...
		values = new short[this.capacity];
	}
	 ...
	 */
	public Reference2ShortHashMap(T[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2ShortHashMap(T[] keys, Short[] values, float loadFactor) {
	 ...
	 */
	public Reference2ShortHashMap(Map<? extends T, ? extends Short> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2ShortHashMap(Map<? extends T, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Reference2ShortHashMap(Object2ShortMap<T> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Reference2ShortHashMap(Object2ShortMap<T> map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean remove(Object key, Object value) {
		...
		WeakKey<T> current = keys[pos];
		if(current == null) return false;
		if(Objects.equals(key, current.get()) && Objects.equals(value, Short.valueOf(values[pos]))) {
			removeIndex(pos);
			return true;
		}
		while(true) {
			if((current = keys[pos = (++pos & mask)]) == null) return false;
			else if(Objects.equals(key, current.get()) && Objects.equals(value, Short.valueOf(values[pos]))){
				removeIndex(pos);
				return true;
			}
		}
	...
	@Override
	public Reference2ShortHashMap<T> copy() {
		Reference2ShortHashMap<T> map = new Reference2ShortHashMap<>(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2ShortMap.Entry<T>> implements Object2ShortMap.FastEntrySet<T> {
		...
		@Override
		public int size() {
			return Reference2ShortHashMap.this.size();
		}
		
		@Override
		public void clear() {
			Reference2ShortHashMap.this.clear();
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2ShortMap.Entry) {
					...
					int index = Reference2ShortHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return entry.getShortValue() == Reference2ShortHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Reference2ShortHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Reference2ShortHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2ShortMap.Entry) {
					Object2ShortMap.Entry<T> entry = (Object2ShortMap.Entry<T>)o;
					return Reference2ShortHashMap.this.remove(entry.getKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Reference2ShortHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractObjectSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size;
			Reference2ShortHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(returnedPos >= 0) shiftKeys(returnedPos);
				...
				Reference2ShortHashMap.this.remove(wrapped[-returnedPos - 1].get());
				lastReturned = -1;
				return;
			}
			size--;
