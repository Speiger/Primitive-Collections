...
public class Object2FloatConcurrentOpenHashMap<T> extends AbstractObject2FloatMap<T> implements Object2FloatConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected float computeIfAbsent(int hash, T key, ToFloatFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeNonDefault(int hash, T key, ObjectFloatUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfAbsentNonDefault(int hash, T key, ToFloatFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float supplyIfAbsentNonDefault(int hash, T key, FloatSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfPresentNonDefault(int hash, T key, ObjectFloatUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
