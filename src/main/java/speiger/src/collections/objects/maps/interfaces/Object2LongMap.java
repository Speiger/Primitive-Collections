 ...
 */
public interface Object2LongMap<T> extends Map<T, Long>, ToLongFunction<T>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Object2LongMap.Entry into a map.
	 * @param entry then Entry that should be inserted.
	 * @return the last present value or default return value.
	 */
	public default long put(Entry<T> entry) {
	 ...
	 * @return the last present value or default return	value.
	 */
	public default Long put(Map.Entry<T, Long> entry) {
		return put(entry.getKey(), entry.getValue().longValue());
	}
	...
	@Override
	public default long applyAsLong(T key) {
		return getLong(key);
	}
	/**
	...
	public default Long computeIfAbsent(T key, Function<? super T, ? extends Long> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Long.valueOf(computeLongIfAbsent(key, mappingFunction instanceof ToLongFunction ? (ToLongFunction<T>)mappingFunction : K -> mappingFunction.apply(K).longValue()));
	}
