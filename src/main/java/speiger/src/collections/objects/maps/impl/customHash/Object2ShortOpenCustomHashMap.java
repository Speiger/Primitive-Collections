...
public class Object2ShortOpenCustomHashMap<T> extends AbstractObject2ShortMap<T> implements ITrimmable
{
	...
	protected class MapEntry implements Object2ShortMap.Entry<T>, Map.Entry<T, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
