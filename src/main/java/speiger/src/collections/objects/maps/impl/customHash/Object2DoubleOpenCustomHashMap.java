...
public class Object2DoubleOpenCustomHashMap<T> extends AbstractObject2DoubleMap<T> implements ITrimmable
{
	...
	protected class MapEntry implements Object2DoubleMap.Entry<T>, Map.Entry<T, Double> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Double.hashCode(getDoubleValue());
		}
	...
	private class Values extends AbstractDoubleCollection {
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return OptionalDouble.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2DoubleOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
