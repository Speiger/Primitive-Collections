...
public class Object2ByteConcurrentOpenHashMap<T> extends AbstractObject2ByteMap<T> implements Object2ByteConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected byte computeNonDefault(int hash, T key, ObjectByteUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfAbsent(int hash, T key, ToByteFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfAbsentNonDefault(int hash, T key, ToByteFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte supplyIfAbsentNonDefault(int hash, T key, ByteSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfPresentNonDefault(int hash, T key, ObjectByteUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
