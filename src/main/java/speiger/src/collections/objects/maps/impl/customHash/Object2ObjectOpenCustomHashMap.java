...
public class Object2ObjectOpenCustomHashMap<T, V> extends AbstractObject2ObjectMap<T, V> implements ITrimmable
{
	...
	protected class MapEntry implements Object2ObjectMap.Entry<T, V>, Map.Entry<T, V> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Objects.hashCode(getValue());
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public Optional<V> findFirst(Predicate<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return Optional.ofNullable(values[i]);
			}
			return Optional.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2ObjectOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
