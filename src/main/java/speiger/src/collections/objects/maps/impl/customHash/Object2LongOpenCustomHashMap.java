...
public class Object2LongOpenCustomHashMap<T> extends AbstractObject2LongMap<T> implements ITrimmable
{
	...
	protected class MapEntry implements Object2LongMap.Entry<T>, Map.Entry<T, Long> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Long.hashCode(getLongValue());
		}
	...
	private class Values extends AbstractLongCollection {
		...
		public OptionalLong findFirst(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return OptionalLong.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2LongOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
