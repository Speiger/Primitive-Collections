...
public class Object2IntConcurrentOpenHashMap<T> extends AbstractObject2IntMap<T> implements Object2IntConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected int computeIfAbsent(int hash, T key, ToIntFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeNonDefault(int hash, T key, ObjectIntUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfAbsentNonDefault(int hash, T key, ToIntFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int supplyIfAbsentNonDefault(int hash, T key, IntSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfPresentNonDefault(int hash, T key, ObjectIntUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
