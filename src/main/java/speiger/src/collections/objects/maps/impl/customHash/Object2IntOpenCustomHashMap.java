...
public class Object2IntOpenCustomHashMap<T> extends AbstractObject2IntMap<T> implements ITrimmable
{
	...
	protected class MapEntry implements Object2IntMap.Entry<T>, Map.Entry<T, Integer> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getKey()) ^ Integer.hashCode(getIntValue());
		}
	...
	private class Values extends AbstractIntCollection {
		...
		public OptionalInt findFirst(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], null) && filter.test(values[i])) return OptionalInt.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Object2IntOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
