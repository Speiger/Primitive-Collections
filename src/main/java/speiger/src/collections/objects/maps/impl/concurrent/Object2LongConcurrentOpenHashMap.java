...
public class Object2LongConcurrentOpenHashMap<T> extends AbstractObject2LongMap<T> implements Object2LongConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected long computeNonDefault(int hash, T key, ObjectLongUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsent(int hash, T key, ToLongFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsentNonDefault(int hash, T key, ToLongFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long supplyIfAbsentNonDefault(int hash, T key, LongSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfPresentNonDefault(int hash, T key, ObjectLongUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
