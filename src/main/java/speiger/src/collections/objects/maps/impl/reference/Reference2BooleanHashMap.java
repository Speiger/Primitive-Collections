 ...
 */
public class Reference2BooleanHashMap<T> extends AbstractObject2BooleanMap<T> implements ITrimmable
{
	 ...
	 * Default Constructor
	 */
	public Reference2BooleanHashMap() {
	 ...
	 */
	public Reference2BooleanHashMap(int minCapacity) {
	 ...
	 */
	public Reference2BooleanHashMap(int minCapacity, float loadFactor) {
		...
		values = new boolean[this.capacity];
	}
	 ...
	 */
	public Reference2BooleanHashMap(T[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2BooleanHashMap(T[] keys, Boolean[] values, float loadFactor) {
	 ...
	 */
	public Reference2BooleanHashMap(Map<? extends T, ? extends Boolean> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2BooleanHashMap(Map<? extends T, ? extends Boolean> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Reference2BooleanHashMap(Object2BooleanMap<T> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Reference2BooleanHashMap(Object2BooleanMap<T> map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean remove(Object key, Object value) {
		...
		WeakKey<T> current = keys[pos];
		if(current == null) return false;
		if(Objects.equals(key, current.get()) && Objects.equals(value, Boolean.valueOf(values[pos]))) {
			removeIndex(pos);
			return true;
		}
		while(true) {
			if((current = keys[pos = (++pos & mask)]) == null) return false;
			else if(Objects.equals(key, current.get()) && Objects.equals(value, Boolean.valueOf(values[pos]))){
				removeIndex(pos);
				return true;
			}
		}
	...
	@Override
	public Reference2BooleanHashMap<T> copy() {
		Reference2BooleanHashMap<T> map = new Reference2BooleanHashMap<>(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2BooleanMap.Entry<T>> implements Object2BooleanMap.FastEntrySet<T> {
		...
		@Override
		public int size() {
			return Reference2BooleanHashMap.this.size();
		}
		
		@Override
		public void clear() {
			Reference2BooleanHashMap.this.clear();
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2BooleanMap.Entry) {
					...
					int index = Reference2BooleanHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return entry.getBooleanValue() == Reference2BooleanHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Reference2BooleanHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Boolean.valueOf(Reference2BooleanHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2BooleanMap.Entry) {
					Object2BooleanMap.Entry<T> entry = (Object2BooleanMap.Entry<T>)o;
					return Reference2BooleanHashMap.this.remove(entry.getKey(), entry.getBooleanValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Reference2BooleanHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractObjectSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size;
			Reference2BooleanHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractBooleanCollection {
		...
		public OptionalBoolean findFirst(BooleanPredicate filter) {
			...
			for(int i = capacity-1;i>=0;i--) {
				if(keys[i] != null && filter.test(values[i])) return OptionalBoolean.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(returnedPos >= 0) shiftKeys(returnedPos);
				...
				Reference2BooleanHashMap.this.remove(wrapped[-returnedPos - 1].get());
				lastReturned = -1;
				return;
			}
			size--;
