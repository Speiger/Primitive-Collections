 ...
 */
public interface Object2ShortMap<T> extends Map<T, Short>, ToShortFunction<T>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Object2ShortMap.Entry into a map.
	 * @param entry then Entry that should be inserted.
	 * @return the last present value or default return value.
	 */
	public default short put(Entry<T> entry) {
	...
	@Override
	public default short applyAsShort(T key) {
		return getShort(key);
	}
	/**
	...
	public default Short computeIfAbsent(T key, Function<? super T, ? extends Short> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Short.valueOf(computeShortIfAbsent(key, mappingFunction instanceof ToShortFunction ? (ToShortFunction<T>)mappingFunction : K -> mappingFunction.apply(K).shortValue()));
	}
