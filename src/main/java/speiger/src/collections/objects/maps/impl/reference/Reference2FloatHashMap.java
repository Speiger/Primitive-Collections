 ...
 */
public class Reference2FloatHashMap<T> extends AbstractObject2FloatMap<T> implements ITrimmable
{
	 ...
	 */
	public Reference2FloatHashMap(int minCapacity) {
	 ...
	 */
	public Reference2FloatHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Reference2FloatHashMap(T[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2FloatHashMap(T[] keys, Float[] values, float loadFactor) {
	 ...
	 */
	public Reference2FloatHashMap(Map<? extends T, ? extends Float> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2FloatHashMap(Map<? extends T, ? extends Float> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Reference2FloatHashMap(Object2FloatMap<T> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Reference2FloatHashMap(Object2FloatMap<T> map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean remove(T key, float value) {
		...
		if(Objects.equals(current.get(), key) && Float.floatToIntBits(value) == Float.floatToIntBits(values[pos])) {
			removeIndex(pos);
			return true;
		}
		while(true) {
			if((current = keys[pos = (++pos & mask)]) == null) return false;
			else if(Objects.equals(current.get(), key) && Float.floatToIntBits(value) == Float.floatToIntBits(values[pos])) {
				removeIndex(pos);
				return true;
			}
		}
	...
	public boolean remove(Object key, Object value) {
		...
		WeakKey<T> current = keys[pos];
		if(current == null) return false;
		if(Objects.equals(key, current.get()) && Objects.equals(value, Float.valueOf(values[pos]))) {
			removeIndex(pos);
			return true;
		}
		while(true) {
			if((current = keys[pos = (++pos & mask)]) == null) return false;
			else if(Objects.equals(key, current.get()) && Objects.equals(value, Float.valueOf(values[pos]))){
				removeIndex(pos);
				return true;
			}
		}
	...
	@Override
	public Reference2FloatHashMap<T> copy() {
		Reference2FloatHashMap<T> map = new Reference2FloatHashMap<>(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2FloatMap.Entry<T>> implements Object2FloatMap.FastEntrySet<T> {
		...
		@Override
		public int size() {
			return Reference2FloatHashMap.this.size();
		}
		
		@Override
		public void clear() {
			Reference2FloatHashMap.this.clear();
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2FloatMap.Entry) {
					...
					int index = Reference2FloatHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Float.floatToIntBits(entry.getFloatValue()) == Float.floatToIntBits(Reference2FloatHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Reference2FloatHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float.valueOf(Reference2FloatHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2FloatMap.Entry) {
					Object2FloatMap.Entry<T> entry = (Object2FloatMap.Entry<T>)o;
					return Reference2FloatHashMap.this.remove(entry.getKey(), entry.getFloatValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Reference2FloatHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractObjectSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size;
			Reference2FloatHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractFloatCollection {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = capacity-1;i>=0;i--) {
				if(keys[i] != null && filter.test(values[i])) return OptionalFloat.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(returnedPos >= 0) shiftKeys(returnedPos);
				...
				Reference2FloatHashMap.this.remove(wrapped[-returnedPos - 1].get());
				lastReturned = -1;
				return;
			}
			size--;
