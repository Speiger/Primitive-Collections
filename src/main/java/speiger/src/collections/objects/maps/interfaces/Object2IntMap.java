 ...
 */
public interface Object2IntMap<T> extends Map<T, Integer>, ToIntFunction<T>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Object2IntMap.Entry into a map.
	 * @param entry then Entry that should be inserted.
	 * @return the last present value or default return value.
	 */
	public default int put(Entry<T> entry) {
	...
	@Override
	public default int applyAsInt(T key) {
		return getInt(key);
	}
	/**
	...
	public default Integer computeIfAbsent(T key, Function<? super T, ? extends Integer> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Integer.valueOf(computeIntIfAbsent(key, mappingFunction instanceof ToIntFunction ? (ToIntFunction<T>)mappingFunction : K -> mappingFunction.apply(K).intValue()));
	}
