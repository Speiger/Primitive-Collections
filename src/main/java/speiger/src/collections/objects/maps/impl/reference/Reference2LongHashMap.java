 ...
 */
public class Reference2LongHashMap<T> extends AbstractObject2LongMap<T> implements ITrimmable
{
	 ...
	 */
	public Reference2LongHashMap(int minCapacity) {
	 ...
	 */
	public Reference2LongHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Reference2LongHashMap(T[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2LongHashMap(T[] keys, Long[] values, float loadFactor) {
	 ...
	 */
	public Reference2LongHashMap(Map<? extends T, ? extends Long> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Reference2LongHashMap(Map<? extends T, ? extends Long> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Reference2LongHashMap(Object2LongMap<T> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Reference2LongHashMap(Object2LongMap<T> map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean remove(Object key, Object value) {
		...
		WeakKey<T> current = keys[pos];
		if(current == null) return false;
		if(Objects.equals(key, current.get()) && Objects.equals(value, Long.valueOf(values[pos]))) {
			removeIndex(pos);
			return true;
		}
		while(true) {
			if((current = keys[pos = (++pos & mask)]) == null) return false;
			else if(Objects.equals(key, current.get()) && Objects.equals(value, Long.valueOf(values[pos]))){
				removeIndex(pos);
				return true;
			}
		}
	...
	@Override
	public Reference2LongHashMap<T> copy() {
		Reference2LongHashMap<T> map = new Reference2LongHashMap<>(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2LongMap.Entry<T>> implements Object2LongMap.FastEntrySet<T> {
		...
		@Override
		public int size() {
			return Reference2LongHashMap.this.size();
		}
		
		@Override
		public void clear() {
			Reference2LongHashMap.this.clear();
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2LongMap.Entry) {
					...
					int index = Reference2LongHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return entry.getLongValue() == Reference2LongHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Reference2LongHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Long.valueOf(Reference2LongHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2LongMap.Entry) {
					Object2LongMap.Entry<T> entry = (Object2LongMap.Entry<T>)o;
					return Reference2LongHashMap.this.remove(entry.getKey(), entry.getLongValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Reference2LongHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractObjectSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size;
			Reference2LongHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractLongCollection {
		...
		public OptionalLong findFirst(LongPredicate filter) {
			...
			for(int i = capacity-1;i>=0;i--) {
				if(keys[i] != null && filter.test(values[i])) return OptionalLong.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(returnedPos >= 0) shiftKeys(returnedPos);
				...
				Reference2LongHashMap.this.remove(wrapped[-returnedPos - 1].get());
				lastReturned = -1;
				return;
			}
			size--;
