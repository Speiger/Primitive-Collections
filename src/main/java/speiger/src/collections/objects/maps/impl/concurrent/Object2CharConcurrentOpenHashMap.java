...
public class Object2CharConcurrentOpenHashMap<T> extends AbstractObject2CharMap<T> implements Object2CharConcurrentMap<T>, ITrimmable
{
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected char computeIfAbsent(int hash, T key, ToCharFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeNonDefault(int hash, T key, ObjectCharUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfAbsentNonDefault(int hash, T key, ToCharFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char supplyIfAbsentNonDefault(int hash, T key, CharSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfPresentNonDefault(int hash, T key, ObjectCharUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
