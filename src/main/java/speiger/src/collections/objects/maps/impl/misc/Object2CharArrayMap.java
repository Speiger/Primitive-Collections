...
public class Object2CharArrayMap<T> extends AbstractObject2CharMap<T> implements Object2CharOrderedMap<T>
{
	...
	public Object2CharArrayMap(Object2CharMap<T> map) {
		this(map.size());
		for(ObjectIterator<Object2CharMap.Entry<T>> iter = getFastIterator(map);iter.hasNext();size++) {
	...
	public Object2CharMap.Entry<T> pollFirstEntry() {
		if(size == 0) throw new NoSuchElementException();
		BasicEntry<T> result = new BasicEntry<>(keys[0], values[0]);
		removeIndex(0);
		return result;
	}
	...
	public Object2CharMap.Entry<T> pollLastEntry() {
		if(size == 0) throw new NoSuchElementException();
		BasicEntry<T> result = new BasicEntry<>(keys[size-1], values[size-1]);
		removeIndex(size-1);
		return result;
	}
	...
	private class KeySet extends AbstractObjectSet<T> implements ObjectOrderedSet<T> {
		...
		@Override
		public ObjectListIterator<T> iterator() { return new KeyIterator(true); }
		@Override
		public ObjectListIterator<T> reverseIterator() { return new KeyIterator(false); }
		@Override
		...
		@Override
		public T getFirst() { return firstKey(); }
		@Override
		public T removeFirst() { return pollFirstKey(); }
		@Override
		public T getLast() { return lastKey(); }
		@Override
		public T removeLast() { return pollLastKey(); }
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Object2CharMap.Entry<T>> {
		...
		public FastEntryIterator(T element) { super(element); }
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Object2CharMap.Entry<T>> {
		...
		public EntryIterator(T element) { super(element); }
	...
	private class KeyIterator extends MapIterator implements ObjectListIterator<T> {
		...
		public KeyIterator(T element) { super(element); }
	...
	private class ValueIterator extends MapIterator implements CharListIterator {
		...
		public ValueIterator(T element) { super(element); }
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				if(obj instanceof Object2CharMap.Entry) {
					...
					return Objects.equals(keys[index], entry.getKey()) && getCharValue() == entry.getCharValue();
				}
