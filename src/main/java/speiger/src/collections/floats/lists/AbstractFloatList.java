...
public abstract class AbstractFloatList extends AbstractFloatCollection implements FloatList 
{
	...
	private class ReversedList extends AbstractFloatList
	{
		final AbstractFloatList list;
		
		public ReversedList(AbstractFloatList list) {
			this.list = list;
		}
		...
		public boolean addAll(int index, FloatList c) {
			if(c instanceof RandomAccess) {
				for(int i = 0,m=c.size();i<m;i++) {
					list.add(list.size() - index - i - 1, c.getFloat(i));
				}
				return true;
			}
			return addCollection(index, c);
		...
		private boolean addCollection(int index, FloatCollection c) {
			int i = 0;
			for(FloatIterator iter = c.iterator();iter.hasNext();i++) {
				list.add(list.size() - index - i - 1, iter.nextFloat());
			}
			return true;
		}
		...
		public boolean addAll(int index, Collection<? extends Float> c) {
			int i = 0;
			for(Iterator<? extends Float> iter = c.iterator();iter.hasNext();i++) {
		...
		public float getFloat(int index) {
			return list.getFloat(list.size() - index - 1);
		}
		...
		public float removeFloat(int index) {
			return list.removeFloat(list.size() - index - 1);
		}
	...
	private class ListIndexedIterator implements FloatListIterator {
		IntList indecies;
		int index;
		int lastReturned = -1;
		...
		public float nextFloat() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getFloat((lastReturned = indecies.getInt(i)));
		}
		...
		public float previousFloat() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getFloat((lastReturned = indecies.getInt(index)));
		}
	...
	private class IndexedIterator implements FloatListIterator {
		int[] indecies;
		int index;
		int lastReturned = -1;
		...
		public float nextFloat() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getFloat((lastReturned = indecies[i]));
		}
		...
		public float previousFloat() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getFloat((lastReturned = indecies[index]));
		}
