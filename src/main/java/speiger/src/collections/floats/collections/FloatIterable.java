...
public interface FloatIterable extends Iterable<Float>
{
	...
	*/
	 ...
	 */
	default <E> ObjectIterable<E> map(Float2ObjectFunction<E> mapper) {
	 ...
	 * @return a new Iterable that returns the desired result
	 */
	default <E, V extends Iterable<E>> ObjectIterable<E> flatMap(Float2ObjectFunction<V> mapper) {
	 ...
	 * @return a new Iterable that returns the desired result
	 */
	default <E> ObjectIterable<E> arrayflatMap(Float2ObjectFunction<E[]> mapper) {
	 ...
	 */
	default FloatIterable filter(Float2BooleanFunction filter) {
	 ...
	 */
	default boolean matchesAny(Float2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(FloatIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextFloat())) return true;
		}
		return false;
	}
	 ...
	 */
	default boolean matchesNone(Float2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(FloatIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextFloat())) return false;
		}
		return true;
	}
	 ...
	 */
	default boolean matchesAll(Float2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(FloatIterator iter = iterator();iter.hasNext();) {
			if(!filter.get(iter.nextFloat())) return false;
		}
	 ...
	 */
	default float findFirst(Float2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		for(FloatIterator iter = iterator();iter.hasNext();) {
	 ...
	 */
	default int count(Float2BooleanFunction filter) {
		Objects.requireNonNull(filter);
		int result = 0;
		for(FloatIterator iter = iterator();iter.hasNext();) {
			if(filter.get(iter.nextFloat())) result++;
		}
		return result;
	}
}