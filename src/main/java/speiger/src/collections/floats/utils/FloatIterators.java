...
public class FloatIterators
{
	 ...
	 */
	public static <E> ObjectIterator<E> map(Iterator<? extends Float> iterator, FloatFunction<E> mapper) {
	 ...
	 * @return a iterator that is mapped to a new result
	 */
	public static <E> ObjectIterator<E> map(FloatIterator iterator, FloatFunction<E> mapper) {
	 ...
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(Iterator<? extends Float> iterator, FloatFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(FloatIterator iterator, FloatFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(Iterator<? extends Float> iterator, FloatFunction<E[]> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(FloatIterator iterator, FloatFunction<E[]> mapper) {
	 ...
	 */
	public static FloatIterator filter(Iterator<? extends Float> iterator, FloatPredicate filter) {
	 ...
	 * @return a filtered iterator
	 */
	public static FloatIterator filter(FloatIterator iterator, FloatPredicate filter) {
	 ...
	 */
	public static FloatIterator repeat(FloatIterator iterator, int repeats) {
	...
	private static class MappedIterator<T> implements ObjectIterator<T>
	{
		...
		MappedIterator(FloatIterator iterator, FloatFunction<T> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		@Override
		public T next() {
			return mapper.apply(iterator.nextFloat());
		}
	...
	private static class FlatMappedIterator<T, V extends Iterable<T>> implements ObjectIterator<T>
	{
		...
		FlatMappedIterator(FloatIterator iterator, FloatFunction<V> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				...
				last = mapper.apply(iterator.nextFloat()).iterator();
			}
		}
	...
	private static class FlatMappedArrayIterator<T> implements ObjectIterator<T>
	{
		...
		FlatMappedArrayIterator(FloatIterator iterator, FloatFunction<T[]> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				if(last != null && last.hasNext()) return;
				last = ObjectIterators.wrap(mapper.apply(iterator.nextFloat()));
			}
		}
	...
	private static class RepeatingIterator implements FloatIterator
	{
		...
		public RepeatingIterator(FloatIterator iter, int repeat) {
			this.iter = iter;
			this.repeats = repeat;
		}
	...
	private static class SortedIterator implements FloatIterator
	{
		...
		FloatCollections.CollectionWrapper sortedElements = null;
		int index = 0;
		...
		public boolean hasNext() {
			if(sortedElements == null) {
				...
				if(hasNext) {
					sortedElements = FloatCollections.wrapper();
					pour(iterator, sortedElements);
				}
				else sortedElements = FloatCollections.wrapper();
	...
	private static class FilteredIterator implements FloatIterator
	{
		...
		public FilteredIterator(FloatIterator iterator, FloatPredicate filter) {
			this.iterator = iterator;
			this.filter = filter;
		}
