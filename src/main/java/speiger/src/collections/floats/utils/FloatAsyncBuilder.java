...
import speiger.src.collections.floats.functions.FloatTask;
...
public class FloatAsyncBuilder
{
	 ...
	 */
	public FloatAsyncBuilder(BaseFloatTask task) {
		this.task = task;
	}
	...
	public static FloatAsyncBuilder of(Iterable<Float> iterable) {
		return new FloatAsyncBuilder(FloatIterables.wrap(iterable));
	}
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(Float2ObjectFunction<E> mapper) {
		return new ObjectAsyncBuilder<>(FloatIterables.map(iterable, mapper));
	}
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(Float2ObjectFunction<V> mapper) {
		return new ObjectAsyncBuilder<>(FloatIterables.flatMap(iterable, mapper));
	}
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E> ObjectAsyncBuilder<E> arrayflatMap(Float2ObjectFunction<E[]> mapper) {
		return new ObjectAsyncBuilder<>(FloatIterables.arrayFlatMap(iterable, mapper));
	}
	 ...
	 */
	public FloatAsyncBuilder filter(Float2BooleanFunction filter) {
		iterable = FloatIterables.filter(iterable, filter);
		return this;
	}
	...
	public FloatAsyncBuilder distinct() {
		iterable = FloatIterables.distinct(iterable);
		return this;
	}
	...
	public FloatAsyncBuilder limit(long limit) {
		iterable = FloatIterables.limit(iterable, limit);
		return this;
	}
	...
	public FloatAsyncBuilder sorted(FloatComparator sorter) {
		iterable = FloatIterables.sorted(iterable, sorter);
		return this;
	}
	...
	public FloatAsyncBuilder peek(FloatConsumer action) {
		iterable = FloatIterables.peek(iterable, action);
		return this;
	}
	...
	public FloatAsyncBuilder reduce(float identity, FloatFloatUnaryOperator operator) {
		return new FloatAsyncBuilder(new ReduceTask(iterable.iterator(), operator, identity));
	}
	 ...
	 */
	public BooleanAsyncBuilder matchAny(Float2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(Float2BooleanFunction filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(Float2BooleanFunction filter) {
	 ...
	 */
	public FloatAsyncBuilder findFirst(Float2BooleanFunction filter) {
	 ...
	 */
	public IntAsyncBuilder count(Float2BooleanFunction filter) {
	...
	public float join(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
		...
		task = null;
		toRun.begin();
		return toRun.getFloat(timeout, unit);
	}
	
	private static class ReduceTask extends BaseFloatTask
	{
		FloatIterator iter;
		FloatFloatUnaryOperator operator;
		float value;
		
		public ReduceTask(FloatIterator iter, FloatFloatUnaryOperator operator, float value) {
			this.iter = iter;
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				value = operator.apply(value, iter.nextFloat());
			}
			if(!iter.hasNext()) {
				setResult(value);
				return true;
	...
	private static class SimpleReduceTask extends BaseFloatTask
	{
		FloatIterator iter;
		FloatFloatUnaryOperator operator;
		boolean first = true;
		...
		public SimpleReduceTask(FloatIterator iter, FloatFloatUnaryOperator operator) {
			this.iter = iter;
			this.operator = operator;
		}
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				if(first) {
					first = false;
					value = iter.nextFloat();
				}
				else {
					value = operator.applyAsFloat(value, iter.nextFloat());
				}
			}
			if(!iter.hasNext()) {
				setResult(value);
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(FloatIterator iter, Float2BooleanFunction filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class FindFirstTask extends BaseFloatTask
	{
		FloatIterator iter;
		Float2BooleanFunction filter;
		
		public FindFirstTask(FloatIterator iter, Float2BooleanFunction filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(FloatIterator iter, Float2BooleanFunction filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class CollectTask<T extends FloatCollection> extends BaseObjectTask<T>
	{
		FloatIterator iter;
		T collection;
		
		public CollectTask(FloatIterator iter, T collection) {
			this.iter = iter;
	...
	{
		FloatIterator iter;
		FloatConsumer listener;
		
		public ForEachTask(FloatIterator iter, FloatConsumer listener) {
			this.iter = iter;
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				listener.accept(iter.nextFloat());
			}
			return !iter.hasNext();
		}
	 ...
	 */
	public abstract static class BaseFloatTask implements FloatTask
	{
		...
		Consumer<FloatTask> callback;
		Executor executor = SanityChecks::invokeAsyncTask;
		float result;
		Throwable excpetion;
		
		void withCallback(Consumer<FloatTask> callback) {
		...
		private float report(int s) throws ExecutionException {
