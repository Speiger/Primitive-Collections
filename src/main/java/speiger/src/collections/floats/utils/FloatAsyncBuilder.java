...
public class FloatAsyncBuilder
{
	...
	public FloatAsyncBuilder repeat(int repeats) {
		iterable = FloatIterables.repeat(iterable, repeats);
		return this;
	}
	 ...
	 */
	public ObjectAsyncBuilder<OptionalFloat> reduce(FloatFloatUnaryOperator operator) {
	 ...
	 */
	public ObjectAsyncBuilder<OptionalFloat> findFirst(FloatPredicate filter) {
	...
	private static class SimpleReduceTask extends BaseObjectTask<OptionalFloat>
	{
		FloatIterator iter;
		FloatFloatUnaryOperator operator;
		boolean first = true;
		...
		public SimpleReduceTask(FloatIterator iter, FloatFloatUnaryOperator operator) {
			this.iter = iter;
			this.operator = operator;
			this.setResult(OptionalFloat.empty());
		}
		...
		protected boolean execute() throws Exception {
			...
			if(!iter.hasNext()) {
				setResult(OptionalFloat.of(value));
				return true;
			}
			return false;
		}
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(FloatIterator iter, FloatPredicate filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class FindFirstTask extends BaseObjectTask<OptionalFloat>
	{
		FloatIterator iter;
		FloatPredicate filter;
		
		public FindFirstTask(FloatIterator iter, FloatPredicate filter) {
			this.iter = iter;
			this.filter = filter;
			this.setResult(OptionalFloat.empty());
		}
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				...
				if(filter.test(iter.nextFloat())) {
					setResult(OptionalFloat.of(entry));
					return true;
				}
			}
			return !iter.hasNext();
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(FloatIterator iter, FloatPredicate filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class ArrayTask extends BaseObjectTask<float[]>
	{
		FloatIterator iter;
		FloatCollections.CollectionWrapper wrapper;
	
		public ArrayTask(FloatIterable iterable) {
			iter = iterable.iterator();
			ISizeProvider prov = ISizeProvider.of(iterable);
			int size = prov == null ? -1 : prov.size();
			wrapper = size < 0 ? FloatCollections.wrapper() : FloatCollections.wrapper(size);
		}
		
		@Override
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				wrapper.add(iter.nextFloat());
			}
			if(!iter.hasNext()) {
				setResult(wrapper.toFloatArray());
				wrapper = null;
				return true;
			}
			return false;
