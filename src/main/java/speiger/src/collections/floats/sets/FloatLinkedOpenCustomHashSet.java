...
public class FloatLinkedOpenCustomHashSet extends FloatOpenCustomHashSet implements FloatOrderedSet
{
	...
	public boolean moveToFirst(float o) {
		if(isEmpty() || strategy.equals(getFirstFloat(), o)) return false;
		if(strategy.equals(o, 0F)) {
			if(containsNull) {
				moveToFirstIndex(nullIndex);
				return true;
	...
	public boolean moveToLast(float o) {
		if(isEmpty() || strategy.equals(getLastFloat(), o)) return false;
		if(strategy.equals(o, 0F)) {
			if(containsNull) {
				moveToLastIndex(nullIndex);
				return true;
	...
	public Object[] toArray() {
		...
		for(int i = 0, index = firstIndex;index != -1;i++,index = (int)links[index]) {
			obj[i] = Float.valueOf(keys[index]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		for(int i = 0, index = firstIndex;index != -1;i++,index = (int)links[index]) {
			a[i] = (E)Float.valueOf(keys[index]);
		}
