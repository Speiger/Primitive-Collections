...
public class Float2LongConcurrentOpenHashMap extends AbstractFloat2LongMap implements Float2LongConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected long computeNonDefault(int hash, float key, FloatLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsentNonDefault(int hash, float key, Float2LongFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long supplyIfAbsentNonDefault(int hash, float key, LongSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfPresentNonDefault(int hash, float key, FloatLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
