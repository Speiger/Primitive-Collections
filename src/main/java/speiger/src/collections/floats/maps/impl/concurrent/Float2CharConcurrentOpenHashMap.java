...
public class Float2CharConcurrentOpenHashMap extends AbstractFloat2CharMap implements Float2CharConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected char computeNonDefault(int hash, float key, FloatCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfAbsentNonDefault(int hash, float key, Float2CharFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char supplyIfAbsentNonDefault(int hash, float key, CharSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfPresentNonDefault(int hash, float key, FloatCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
