...
public interface Float2CharMap extends Map<Float, Character>, Float2CharFunction
{
	...
	/**
	 * A Helper method that allows to put int a Float2CharMap.Entry into a map.
	 ...
	 * @return the last present value or default return	value.
	 */
	public default Character put(Map.Entry<Float, Character> entry) {
		return put(entry.getKey().floatValue(), entry.getValue().charValue());
	}
	...
	@Override
	public default char applyAsChar(float key) {
		return get(key);
	}
	/**
