...
public class Float2BooleanOpenCustomHashMap extends AbstractFloat2BooleanMap implements ITrimmable
{
	...
	protected class MapEntry implements Float2BooleanMap.Entry, Map.Entry<Float, Boolean> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getFloatKey()) ^ Boolean.hashCode(getBooleanValue());
		}
	...
	private class Values extends AbstractBooleanCollection {
		...
		public OptionalBoolean findFirst(BooleanPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.test(values[i])) return OptionalBoolean.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2BooleanOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
