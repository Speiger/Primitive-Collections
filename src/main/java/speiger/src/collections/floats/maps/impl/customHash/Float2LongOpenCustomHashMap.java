...
public class Float2LongOpenCustomHashMap extends AbstractFloat2LongMap implements ITrimmable
{
	...
	protected class MapEntry implements Float2LongMap.Entry, Map.Entry<Float, Long> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getFloatKey()) ^ Long.hashCode(getLongValue());
		}
	...
	private class Values extends AbstractLongCollection {
		...
		public OptionalLong findFirst(LongPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.test(values[i])) return OptionalLong.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2LongOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
