...
public class Float2LongArrayMap extends AbstractFloat2LongMap implements Float2LongOrderedMap
{
	...
	public Float2LongArrayMap(Float2LongMap map) {
		this(map.size());
		for(ObjectIterator<Float2LongMap.Entry> iter = getFastIterator(map);iter.hasNext();size++) {
	...
	@Override
	public long putFirst(float key, long value) {
		int index = findIndex(key);
		if(index < 0) {
			insertIndex(0, key, value);
			size++;
	...
	@Override
	public long putLast(float key, long value) {
		int index = findIndex(key);
		if(index < 0) {
			insertIndex(size++, key, value);
			return getDefaultReturnValue();
	...
	private class MapEntrySet extends AbstractObjectSet<Float2LongMap.Entry> implements Float2LongOrderedMap.FastOrderedSet {
		@Override
		public void addFirst(Float2LongMap.Entry o) { throw new UnsupportedOperationException(); }
		@Override
		public void addLast(Float2LongMap.Entry o) { throw new UnsupportedOperationException(); }
		@Override
		...
		@Override
		public Float2LongMap.Entry getFirst() {
			return new BasicEntry(firstFloatKey(), firstLongValue());
		}
		
		@Override
		public Float2LongMap.Entry getLast() {
			return new BasicEntry(lastFloatKey(), lastLongValue());
		}
		
		@Override
		public Float2LongMap.Entry removeFirst() {
		...
		@Override
		public Float2LongMap.Entry removeLast() {
		...
		@Override
		public ObjectBidirectionalIterator<Float2LongMap.Entry> reverseIterator() {
		...
		@Override
		public Optional<Float2LongMap.Entry> reduce(ObjectObjectUnaryOperator<Float2LongMap.Entry, Float2LongMap.Entry> operator) {
			Objects.requireNonNull(operator);
		...
		@Override
		public Optional<Float2LongMap.Entry> findFirst(Predicate<Float2LongMap.Entry> filter) {
			Objects.requireNonNull(filter);
	...
	private class KeySet extends AbstractFloatSet implements FloatOrderedSet {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			Objects.requireNonNull(filter);
			for(int i = 0;i<size;i++) {
				if(filter.test(keys[i])) return OptionalFloat.of(keys[i]);
			}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				if(obj instanceof Float2LongMap.Entry) {
					...
					return Float.floatToIntBits(keys[index]) == Float.floatToIntBits(entry.getFloatKey()) && getLongValue() == entry.getLongValue();
				}
