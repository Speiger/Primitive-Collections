...
public class Float2DoubleOpenCustomHashMap extends AbstractFloat2DoubleMap implements ITrimmable
{
	...
	protected class MapEntry implements Float2DoubleMap.Entry, Map.Entry<Float, Double> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getFloatKey()) ^ Double.hashCode(getDoubleValue());
		}
	...
	private class Values extends AbstractDoubleCollection {
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.test(values[i])) return OptionalDouble.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2DoubleOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
