...
public class Float2ByteOpenCustomHashMap extends AbstractFloat2ByteMap implements ITrimmable
{
	...
	protected class MapEntry implements Float2ByteMap.Entry, Map.Entry<Float, Byte> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getFloatKey()) ^ Byte.hashCode(getByteValue());
		}
	...
	private class Values extends AbstractByteCollection {
		...
		public OptionalByte findFirst(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.test(values[i])) return OptionalByte.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2ByteOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
