...
public class Float2ShortConcurrentOpenHashMap extends AbstractFloat2ShortMap implements Float2ShortConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected short computeNonDefault(int hash, float key, FloatShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsentNonDefault(int hash, float key, Float2ShortFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short supplyIfAbsentNonDefault(int hash, float key, ShortSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfPresentNonDefault(int hash, float key, FloatShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
