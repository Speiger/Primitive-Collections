...
public class Float2DoubleConcurrentOpenHashMap extends AbstractFloat2DoubleMap implements Float2DoubleConcurrentMap, ITrimmable
{
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = 0,m=segments.length;i<m;i++) {
				...
				try {
					int index = seg.firstIndex;
					while(index != -1){
						if(filter.test(seg.keys[index])) return OptionalFloat.of(seg.keys[index]);
						index = (int)seg.links[index];
					}
				}
				finally {
	...
	protected static class Segment extends StampedLock
	{
		...
		protected double computeNonDefault(int hash, float key, FloatDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfAbsentNonDefault(int hash, float key, Float2DoubleFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double supplyIfAbsentNonDefault(int hash, float key, DoubleSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfPresentNonDefault(int hash, float key, FloatDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
