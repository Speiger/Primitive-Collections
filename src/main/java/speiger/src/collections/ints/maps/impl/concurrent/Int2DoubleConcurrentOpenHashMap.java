...
public class Int2DoubleConcurrentOpenHashMap extends AbstractInt2DoubleMap implements Int2DoubleConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected double computeNonDefault(int hash, int key, IntDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfAbsentNonDefault(int hash, int key, Int2DoubleFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double supplyIfAbsentNonDefault(int hash, int key, DoubleSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfPresentNonDefault(int hash, int key, IntDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
