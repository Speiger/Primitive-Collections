...
public class Int2BooleanConcurrentOpenHashMap extends AbstractInt2BooleanMap implements Int2BooleanConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected boolean computeIfAbsent(int hash, int key, IntPredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeNonDefault(int hash, int key, IntBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfAbsentNonDefault(int hash, int key, IntPredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean supplyIfAbsentNonDefault(int hash, int key, BooleanSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfPresentNonDefault(int hash, int key, IntBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
