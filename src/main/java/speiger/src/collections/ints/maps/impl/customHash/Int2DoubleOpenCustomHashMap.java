...
public class Int2DoubleOpenCustomHashMap extends AbstractInt2DoubleMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2DoubleMap.Entry, Map.Entry<Integer, Double> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Double.hashCode(getDoubleValue());
		}
	...
	private class Values extends AbstractDoubleCollection {
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalDouble.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2DoubleOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
