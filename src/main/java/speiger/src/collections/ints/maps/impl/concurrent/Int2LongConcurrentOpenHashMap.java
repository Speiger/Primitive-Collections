...
public class Int2LongConcurrentOpenHashMap extends AbstractInt2LongMap implements Int2LongConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected long computeNonDefault(int hash, int key, IntLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsentNonDefault(int hash, int key, Int2LongFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long supplyIfAbsentNonDefault(int hash, int key, LongSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfPresentNonDefault(int hash, int key, IntLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
