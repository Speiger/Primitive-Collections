...
public class Int2CharOpenCustomHashMap extends AbstractInt2CharMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2CharMap.Entry, Map.Entry<Integer, Character> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Character.hashCode(getCharValue());
		}
	...
	private class Values extends AbstractCharCollection {
		...
		public OptionalChar findFirst(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalChar.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2CharOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
