...
public class Int2ByteOpenCustomHashMap extends AbstractInt2ByteMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2ByteMap.Entry, Map.Entry<Integer, Byte> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Byte.hashCode(getByteValue());
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ByteMap.Entry> implements Int2ByteMap.FastEntrySet {
		...
		public void forEach(Consumer<? super Int2ByteMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--)
				if(!strategy.equals(keys[i], 0)) action.accept(new ValueMapEntry(i));
		}
		...
		@Override
		public void forEachIndexed(IntObjectConsumer<Int2ByteMap.Entry> action) {
			Objects.requireNonNull(action);
			if(size() <= 0) return;
			if(containsNull) action.accept(0, new ValueMapEntry(nullIndex));
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0)) action.accept(index++, new ValueMapEntry(i));
			}
		}
		...
		public <E> void forEach(E input, ObjectObjectConsumer<E, Int2ByteMap.Entry> action) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0)) action.accept(input, new ValueMapEntry(i));
			}
		}
		
		@Override
		public boolean matchesAny(Predicate<Int2ByteMap.Entry> filter) {
		...
		@Override
		public boolean matchesNone(Predicate<Int2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
		...
		@Override
		public boolean matchesAll(Predicate<Int2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return true;
			MapEntry entry = new MapEntry();
			if(containsNull) {
		...
		@Override
		public int count(Predicate<Int2ByteMap.Entry> filter) {
			Objects.requireNonNull(filter);
			if(size() <= 0) return 0;
			MapEntry entry = new MapEntry();
			int result = 0;
	...
	private final class KeySet extends AbstractIntSet {
		...
		public void forEachIndexed(IntIntConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0)) action.accept(index++, keys[i]);
			}
		}
		...
		public boolean matchesAny(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(keys[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(keys[i])) return false;
			}
			return true;
		}
		...
		public int count(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(keys[i])) result++;
			}
			return result;
		}
	}
	...
	private class Values extends AbstractByteCollection {
		...
		public void forEachIndexed(IntByteConsumer action) {
			...
			for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
				if(!strategy.equals(keys[i], 0)) action.accept(index++, values[i]);
			}
		}
		...
		public boolean matchesAny(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return false;
			}
			return true;
		}
		...
		public int count(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2ByteOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
