...
public class Int2BooleanOpenCustomHashMap extends AbstractInt2BooleanMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2BooleanMap.Entry, Map.Entry<Integer, Boolean> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Boolean.hashCode(getBooleanValue());
		}
	...
	private class Values extends AbstractBooleanCollection {
		...
		public OptionalBoolean findFirst(BooleanPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalBoolean.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2BooleanOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
