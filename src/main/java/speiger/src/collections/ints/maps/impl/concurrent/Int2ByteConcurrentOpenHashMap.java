...
public class Int2ByteConcurrentOpenHashMap extends AbstractInt2ByteMap implements Int2ByteConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected byte computeIfAbsentNonDefault(int hash, int key, Int2ByteFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected byte computeIfPresentNonDefault(int hash, int key, IntByteUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
