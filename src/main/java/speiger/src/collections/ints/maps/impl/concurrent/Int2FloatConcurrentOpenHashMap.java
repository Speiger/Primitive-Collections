...
public class Int2FloatConcurrentOpenHashMap extends AbstractInt2FloatMap implements Int2FloatConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected float computeNonDefault(int hash, int key, IntFloatUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfAbsentNonDefault(int hash, int key, Int2FloatFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float supplyIfAbsentNonDefault(int hash, int key, FloatSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected float computeIfPresentNonDefault(int hash, int key, IntFloatUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
