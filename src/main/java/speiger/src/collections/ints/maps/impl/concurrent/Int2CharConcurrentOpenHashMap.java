...
public class Int2CharConcurrentOpenHashMap extends AbstractInt2CharMap implements Int2CharConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected char computeIfAbsentNonDefault(int hash, int key, Int2CharFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected char computeIfPresentNonDefault(int hash, int key, IntCharUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
