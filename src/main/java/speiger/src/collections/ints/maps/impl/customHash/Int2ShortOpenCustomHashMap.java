...
public class Int2ShortOpenCustomHashMap extends AbstractInt2ShortMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2ShortMap.Entry, Map.Entry<Integer, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ShortMap.Entry> implements Int2ShortMap.FastEntrySet {
		...
		@Override
		public Optional<Int2ShortMap.Entry> reduce(ObjectObjectUnaryOperator<Int2ShortMap.Entry, Int2ShortMap.Entry> operator) {
			Objects.requireNonNull(operator);
		...
		@Override
		public Optional<Int2ShortMap.Entry> findFirst(Predicate<Int2ShortMap.Entry> filter) {
			Objects.requireNonNull(filter);
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
