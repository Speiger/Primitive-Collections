...
public class Int2ShortConcurrentOpenHashMap extends AbstractInt2ShortMap implements Int2ShortConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected short computeNonDefault(int hash, int key, IntShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsentNonDefault(int hash, int key, Int2ShortFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short supplyIfAbsentNonDefault(int hash, int key, ShortSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfPresentNonDefault(int hash, int key, IntShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
