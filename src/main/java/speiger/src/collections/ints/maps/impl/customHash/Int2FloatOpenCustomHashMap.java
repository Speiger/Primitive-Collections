...
public class Int2FloatOpenCustomHashMap extends AbstractInt2FloatMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2FloatMap.Entry, Map.Entry<Integer, Float> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Float.hashCode(getFloatValue());
		}
	...
	private class Values extends AbstractFloatCollection {
		...
		public OptionalFloat findFirst(FloatPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalFloat.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2FloatOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
