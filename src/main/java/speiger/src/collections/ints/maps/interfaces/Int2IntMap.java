 ...
 */
public interface Int2IntMap extends Map<Integer, Integer>, IntUnaryOperator
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Int2IntMap.Entry into a map.
	...
	@Override
	public default int applyAsInt(int key) {
		return get(key);
	}
	/**
	...
	public default Integer computeIfAbsent(Integer key, Function<? super Integer, ? extends Integer> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Integer.valueOf(computeIntIfAbsent(key.intValue(), mappingFunction instanceof IntUnaryOperator ? (IntUnaryOperator)mappingFunction : K -> mappingFunction.apply(Integer.valueOf(K)).intValue()));
	}
