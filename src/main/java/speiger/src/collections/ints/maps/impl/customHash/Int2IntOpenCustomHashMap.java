...
public class Int2IntOpenCustomHashMap extends AbstractInt2IntMap implements ITrimmable
{
	...
	protected class MapEntry implements Int2IntMap.Entry, Map.Entry<Integer, Integer> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getIntKey()) ^ Integer.hashCode(getIntValue());
		}
	...
	private class Values extends AbstractIntCollection {
		...
		public OptionalInt findFirst(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0) && filter.test(values[i])) return OptionalInt.of(values[i]);
			}
			return OptionalInt.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2IntOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
