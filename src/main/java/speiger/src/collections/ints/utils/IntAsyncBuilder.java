...
import speiger.src.collections.ints.functions.IntTask;
import speiger.src.collections.ints.functions.function.IntPredicate;
...
public class IntAsyncBuilder
{
	 ...
	 */
	public IntAsyncBuilder(IntIterable iterable) {
	 ...
	 */
	public IntAsyncBuilder(BaseIntTask task) {
		this.task = task;
	}
	 ...
	 */
	public static IntAsyncBuilder of(Iterable<Integer> iterable) {
		return new IntAsyncBuilder(IntIterables.wrap(iterable));
	}
	 ...
	 */
	public <E> ObjectAsyncBuilder<E> map(IntFunction<E> mapper) {
		return new ObjectAsyncBuilder<>(IntIterables.map(iterable, mapper));
	}
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E, V extends Iterable<E>> ObjectAsyncBuilder<E> flatMap(IntFunction<V> mapper) {
		return new ObjectAsyncBuilder<>(IntIterables.flatMap(iterable, mapper));
	}
	 ...
	 * @return a new Builder Object with the mapped Iterable
	 */
	public <E> ObjectAsyncBuilder<E> arrayflatMap(IntFunction<E[]> mapper) {
		return new ObjectAsyncBuilder<>(IntIterables.arrayFlatMap(iterable, mapper));
	}
	 ...
	 */
	public IntAsyncBuilder filter(IntPredicate filter) {
		iterable = IntIterables.filter(iterable, filter);
		return this;
	}
	 ...
	 */
	public IntAsyncBuilder distinct() {
		iterable = IntIterables.distinct(iterable);
		return this;
	}
	 ...
	 */
	public IntAsyncBuilder repeat(int repeats) {
		iterable = IntIterables.repeat(iterable, repeats);
		return this;
	}
	 ...
	 */
	public IntAsyncBuilder limit(long limit) {
		iterable = IntIterables.limit(iterable, limit);
		return this;
	}
	 ...
	 */
	public IntAsyncBuilder sorted(IntComparator sorter) {
		iterable = IntIterables.sorted(iterable, sorter);
		return this;
	}
	 ...
	 */
	public IntAsyncBuilder peek(IntConsumer action) {
		iterable = IntIterables.peek(iterable, action);
		return this;
	}
	 ...
	 */
	public ObjectAsyncBuilder<Void> forEach(IntConsumer action) {
	 ...
	 */
	public IntAsyncBuilder reduce(int identity, IntIntUnaryOperator operator) {
		return new IntAsyncBuilder(new ReduceTask(iterable.iterator(), operator, identity));
	}
	 ...
	 */
	public ObjectAsyncBuilder<int[]> toIntArray() {
	 ...
	 */
	public ObjectAsyncBuilder<IntList> pourAsList() {
	 ...
	 * @return a new Builder with the pour function applied
	 */
	public ObjectAsyncBuilder<IntSet> pourAsSet() {
	 ...
	 * @return a new Builder with the pour function applied
	 */
	public <E extends IntCollection> ObjectAsyncBuilder<E> pour(E collection) {
	 ...
	 */
	public BooleanAsyncBuilder matchAny(IntPredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchNone(IntPredicate filter) {
	 ...
	 */
	public BooleanAsyncBuilder matchAll(IntPredicate filter) {
	 ...
	 */
	public IntAsyncBuilder count(IntPredicate filter) {
	 ...
	 */
	public IntAsyncBuilder executor(Executor executor) {
	 ...
	 * @note can be null
	 */
	public IntAsyncBuilder onCompletion(Consumer<IntTask> callback) {
		if(task == null) throw new IllegalStateException("Action is missing");
	 ...
	 */
	public IntTask execute() {
		BaseIntTask toRun = task;
		toRun.begin();
		task = null;
		return toRun;
	}
	 ...
	 */
	public int join() throws ExecutionException, InterruptedException {
		BaseIntTask toRun = task;
		task = null;
		toRun.begin();
		return toRun.getInt();
	 ...
	 */
	public int join(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
		BaseIntTask toRun = task;
		task = null;
		toRun.begin();
		return toRun.getInt(timeout, unit);
	}
	
	private static class ReduceTask extends BaseIntTask
	{
		IntIterator iter;
		IntIntUnaryOperator operator;
		int value;
		
		public ReduceTask(IntIterator iter, IntIntUnaryOperator operator, int value) {
			this.iter = iter;
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				value = operator.apply(value, iter.nextInt());
			}
			if(!iter.hasNext()) {
				setResult(value);
				return true;
		...
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				if(first) {
					...
					value = operator.applyAsInt(value, iter.nextInt());
				}
			}
			if(!iter.hasNext()) {
				setResult(value);
	...
	private static class MatchTask extends BaseBooleanTask
	{
		...
		public MatchTask(IntIterator iter, IntPredicate filter, int type) {
			this.iter = iter;
			this.filter = filter;
			this.type = type;
	...
	private static class CountTask extends BaseIntTask
	{
		...
		public CountTask(IntIterator iter, IntPredicate filter) {
			this.iter = iter;
			this.filter = filter;
		}
	...
	private static class CollectTask<T extends IntCollection> extends BaseObjectTask<T>
	{
		IntIterator iter;
		T collection;
		
		public CollectTask(IntIterator iter, T collection) {
			this.iter = iter;
	...
	private static class ArrayTask extends BaseObjectTask<int[]>
	{
		IntIterator iter;
		IntCollections.CollectionWrapper wrapper;
	
		public ArrayTask(IntIterable iterable) {
			iter = iterable.iterator();
			ISizeProvider prov = ISizeProvider.of(iterable);
			int size = prov == null ? -1 : prov.size();
			wrapper = size < 0 ? IntCollections.wrapper() : IntCollections.wrapper(size);
		}
		
		@Override
		protected boolean execute() throws Exception {
			while(shouldRun() && iter.hasNext()) {
				wrapper.add(iter.nextInt());
			}
			if(!iter.hasNext()) {
				setResult(wrapper.toIntArray());
				wrapper = null;
				return true;
			}
			return false;
	...
	private static class ForEachTask<T> extends BaseObjectTask<Void>
	{
		...
		public ForEachTask(IntIterator iter, IntConsumer listener) {
			this.iter = iter;
	 ...
	 */
	public abstract static class BaseIntTask implements IntTask
	{
		...
		Consumer<IntTask> callback;
		Executor executor = SanityChecks::invokeAsyncTask;
		int result;
		Throwable excpetion;
		
		void withCallback(Consumer<IntTask> callback) {
		...
		@Override
		public int getInt() throws InterruptedException, ExecutionException {
	        int s = state;
	        return report(s <= FINISHING ? awaitDone(false, 0L) : s);
		}
		
		@Override
		public int getInt(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
		...
		private int report(int s) throws ExecutionException {
