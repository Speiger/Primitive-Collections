...
public class IntOpenCustomHashSet extends AbstractIntSet implements ITrimmable
{
	...
	public int[] toIntArray(int[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0)) a[index++] = keys[i];
		}
	...
	public Object[] toArray() {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0)) obj[index++] = Integer.valueOf(keys[i]);
		}
		return obj;
	}
	...
	public <E> E[] toArray(E[] a) {
		...
		for(int i = nullIndex-1, index = containsNull ? 1 : 0;i>=0;i--) {
			if(!strategy.equals(keys[i], 0)) a[index++] = (E)Integer.valueOf(keys[i]);
		}
	...
	private class SetIterator implements IntIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				IntOpenCustomHashSet.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
