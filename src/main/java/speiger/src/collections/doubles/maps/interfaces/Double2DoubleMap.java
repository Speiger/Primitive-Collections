 ...
 */
public interface Double2DoubleMap extends Map<Double, Double>, DoubleUnaryOperator
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Double2DoubleMap.Entry into a map.
	...
	@Override
	public default double applyAsDouble(double key) {
		return get(key);
	}
	/**
	...
	public default Double computeIfAbsent(Double key, Function<? super Double, ? extends Double> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Double.valueOf(computeDoubleIfAbsent(key.doubleValue(), mappingFunction instanceof DoubleUnaryOperator ? (DoubleUnaryOperator)mappingFunction : K -> mappingFunction.apply(Double.valueOf(K)).doubleValue()));
	}
