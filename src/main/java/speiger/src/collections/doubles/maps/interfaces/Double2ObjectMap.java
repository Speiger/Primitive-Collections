 ...
 */
public interface Double2ObjectMap<V> extends Map<Double, V>, DoubleFunction<V>
{
	/**
	...
	/**
	 * A Helper method that allows to put int a Double2ObjectMap.Entry into a map.
	...
	@Override
	public default V apply(double key) {
		return get(key);
	}
	/**
	...
	public default V computeIfAbsent(Double key, Function<? super Double, ? extends V> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return computeIfAbsent(key.doubleValue(), mappingFunction instanceof DoubleFunction ? (DoubleFunction<V>)mappingFunction : K -> mappingFunction.apply(Double.valueOf(K)));
	}
