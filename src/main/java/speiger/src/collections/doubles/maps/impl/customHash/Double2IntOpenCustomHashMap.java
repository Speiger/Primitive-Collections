...
public class Double2IntOpenCustomHashMap extends AbstractDouble2IntMap implements ITrimmable
{
	...
	protected class MapEntry implements Double2IntMap.Entry, Map.Entry<Double, Integer> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getDoubleKey()) ^ Integer.hashCode(getIntValue());
		}
	...
	private class Values extends AbstractIntCollection {
		...
		public OptionalInt findFirst(IntPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.test(values[i])) return OptionalInt.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2IntOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
