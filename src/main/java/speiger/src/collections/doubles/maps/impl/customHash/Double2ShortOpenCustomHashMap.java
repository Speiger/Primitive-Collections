...
public class Double2ShortOpenCustomHashMap extends AbstractDouble2ShortMap implements ITrimmable
{
	...
	protected class MapEntry implements Double2ShortMap.Entry, Map.Entry<Double, Short> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getDoubleKey()) ^ Short.hashCode(getShortValue());
		}
	...
	private class Values extends AbstractShortCollection {
		...
		public OptionalShort findFirst(ShortPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.test(values[i])) return OptionalShort.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2ShortOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
