...
public class Double2IntConcurrentOpenHashMap extends AbstractDouble2IntMap implements Double2IntConcurrentMap, ITrimmable
{
		...
		public OptionalDouble findFirst(DoublePredicate filter) {
			...
			for(int i = 0,m=segments.length;i<m;i++) {
				...
				try {
					int index = seg.firstIndex;
					while(index != -1){
						if(filter.test(seg.keys[index])) return OptionalDouble.of(seg.keys[index]);
						index = (int)seg.links[index];
					}
				}
				finally {
	...
	protected static class Segment extends StampedLock
	{
		...
		protected int computeNonDefault(int hash, double key, DoubleIntUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfAbsentNonDefault(int hash, double key, Double2IntFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int supplyIfAbsentNonDefault(int hash, double key, IntSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected int computeIfPresentNonDefault(int hash, double key, DoubleIntUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
