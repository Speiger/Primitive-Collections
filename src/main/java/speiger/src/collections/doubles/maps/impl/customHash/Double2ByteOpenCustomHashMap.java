...
public class Double2ByteOpenCustomHashMap extends AbstractDouble2ByteMap implements ITrimmable
{
	...
	protected class MapEntry implements Double2ByteMap.Entry, Map.Entry<Double, Byte> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getDoubleKey()) ^ Byte.hashCode(getByteValue());
		}
	...
	private class Values extends AbstractByteCollection {
		...
		public OptionalByte findFirst(BytePredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.test(values[i])) return OptionalByte.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2ByteOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
