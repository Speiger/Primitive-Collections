...
public class Double2LongConcurrentOpenHashMap extends AbstractDouble2LongMap implements Double2LongConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected long computeNonDefault(int hash, double key, DoubleLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfAbsentNonDefault(int hash, double key, Double2LongFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long supplyIfAbsentNonDefault(int hash, double key, LongSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected long computeIfPresentNonDefault(int hash, double key, DoubleLongUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
