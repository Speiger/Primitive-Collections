...
public class Double2DoubleConcurrentOpenHashMap extends AbstractDouble2DoubleMap implements Double2DoubleConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected double computeIfAbsent(int hash, double key, DoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeNonDefault(int hash, double key, DoubleDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfAbsentNonDefault(int hash, double key, DoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double supplyIfAbsentNonDefault(int hash, double key, DoubleSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected double computeIfPresentNonDefault(int hash, double key, DoubleDoubleUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
