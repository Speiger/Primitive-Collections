...
public class Double2BooleanConcurrentOpenHashMap extends AbstractDouble2BooleanMap implements Double2BooleanConcurrentMap, ITrimmable
{
	...
	protected static class Segment extends StampedLock
	{
		...
		protected boolean computeIfAbsent(int hash, double key, DoublePredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeNonDefault(int hash, double key, DoubleBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfAbsentNonDefault(int hash, double key, DoublePredicate mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean supplyIfAbsentNonDefault(int hash, double key, BooleanSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected boolean computeIfPresentNonDefault(int hash, double key, DoubleBooleanUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
