...
public class Double2ObjectConcurrentOpenHashMap<V> extends AbstractDouble2ObjectMap<V> implements Double2ObjectConcurrentMap<V>, ITrimmable
{
	...
	protected static class Segment<V> extends StampedLock
	{
		...
		protected V computeIfAbsent(int hash, double key, DoubleFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeNonDefault(int hash, double key, DoubleObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
