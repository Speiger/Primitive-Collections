...
public class Double2CharOpenCustomHashMap extends AbstractDouble2CharMap implements ITrimmable
{
	...
	protected class MapEntry implements Double2CharMap.Entry, Map.Entry<Double, Character> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getDoubleKey()) ^ Character.hashCode(getCharValue());
		}
	...
	private class Values extends AbstractCharCollection {
		...
		public OptionalChar findFirst(CharPredicate filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.test(values[i])) return OptionalChar.of(values[i]);
			}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2CharOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
