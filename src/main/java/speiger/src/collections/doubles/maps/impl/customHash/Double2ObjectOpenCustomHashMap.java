...
public class Double2ObjectOpenCustomHashMap<V> extends AbstractDouble2ObjectMap<V> implements ITrimmable
{
	...
	protected class MapEntry implements Double2ObjectMap.Entry<V>, Map.Entry<Double, V> {
		...
		@Override
		public int hashCode() {
			return strategy.hashCode(getDoubleKey()) ^ Objects.hashCode(getValue());
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public Optional<V> findFirst(Predicate<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.test(values[i])) return Optional.ofNullable(values[i]);
			}
			return Optional.empty();
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2ObjectOpenCustomHashMap.this.remove(wrapped[-returnedPos - 1]);
				lastReturned = -1;
				return;
			}
			size--;
