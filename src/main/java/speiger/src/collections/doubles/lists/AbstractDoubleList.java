...
public abstract class AbstractDoubleList extends AbstractDoubleCollection implements DoubleList 
{
	...
	private class ReversedList extends AbstractDoubleList
	{
		final AbstractDoubleList list;
		
		public ReversedList(AbstractDoubleList list) {
			this.list = list;
		}
		...
		public boolean addAll(int index, DoubleList c) {
			if(c instanceof RandomAccess) {
				for(int i = 0,m=c.size();i<m;i++) {
					list.add(list.size() - index - i - 1, c.getDouble(i));
				}
				return true;
			}
			return addCollection(index, c);
		...
		private boolean addCollection(int index, DoubleCollection c) {
			int i = 0;
			for(DoubleIterator iter = c.iterator();iter.hasNext();i++) {
				list.add(list.size() - index - i - 1, iter.nextDouble());
			}
			return true;
		}
		...
		public boolean addAll(int index, Collection<? extends Double> c) {
			int i = 0;
			for(Iterator<? extends Double> iter = c.iterator();iter.hasNext();i++) {
		...
		public double removeDouble(int index) {
			return list.removeDouble(list.size() - index - 1);
		}
	...
	private class ListIndexedIterator implements DoubleListIterator {
		IntList indecies;
		int index;
		int lastReturned = -1;
		...
		public double nextDouble() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getDouble((lastReturned = indecies.getInt(i)));
		}
		...
		public double previousDouble() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getDouble((lastReturned = indecies.getInt(index)));
		}
	...
	private class IndexedIterator implements DoubleListIterator {
		int[] indecies;
		int index;
		int lastReturned = -1;
		...
		public double nextDouble() {
			if(!hasNext()) throw new NoSuchElementException();
			int i = index++;
			return getDouble((lastReturned = indecies[i]));
		}
		...
		public double previousDouble() {
			if(!hasPrevious()) throw new NoSuchElementException();
			index--;
			return getDouble((lastReturned = indecies[index]));
		}
